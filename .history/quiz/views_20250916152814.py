...
def submit_quiz_attempt(request, job_id):
    ...
    raw_answers = request.data.get('answers', {})  # could be list or dict
    resume_id = request.data.get('resume_id')  # optional

    # --- Normalize answers to a dict: { "<qid>": "<choice>" } ---
    answers = {}
    try:
        if isinstance(raw_answers, dict):
            ...
            for item in raw_answers:
                if not isinstance(item, dict):
                    continue
                # common possible keys for question id and answer
                key = item.get('qid') or item.get('question_id') or item.get('id') or item.get('q') or item.get('question')
                val = item.get('answer') or item.get('choice') or item.get('value') or item.get('selected')
                if key is None:
                    # try if list item itself is like ["q1","A"] -> skip (rare)
                    continue
                answers[str(key)] = val
        else:
            # fallback: try to coerce JSON-stringified payload (if accidental)
            answers = {}
    except Exception as e:
        # don't crash — keep answers empty
        answers = {}
        print("answers normalization failed:", e)
    ...
    # scoring: compare against stored questions' 'answer' key
    correct = 0
    total = 0
    qmap = {}
    for q in quiz.questions_json or []:
        # normalize question id to string
        qid_raw = q.get('id') or q.get('qid') or q.get('question_id') or (total + 1)
        qid = str(qid_raw)
        qmap[qid] = q
        total += 1

    for qid, q in qmap.items():
        # check candidate's answer: try different keys
        selected = None
        if isinstance(answers, dict):
            selected = answers.get(qid) or answers.get(str(qid))
        # stored correct answer
        correct_answer = q.get('answer')
        if selected is not None and correct_answer:
            try:
                if str(selected).strip().upper() == str(correct_answer).strip().upper():
                    correct += 1
            except Exception:
                # if comparison fails, ignore and continue
                pass

    score_percent = (correct / total * 100) if total else 0.0
    passed = score_percent >= (quiz.passing_percent or 0.0)

    # Save attempt - store normalized answers (dict)
