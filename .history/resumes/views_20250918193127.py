...
{# --- inside the job/applications loop --- #}
{# ensure we pick an interview id safely: use in-order: interview_id, job_default_interview_id, job.default_interview_id #}
{% with iv=interview_id|default:job_default_interview_id|default:job.default_interview_id|default:"" %}
  <div class="application-row d-flex align-items-start justify-content-between py-2 mb-2 border rounded"
       data-application-id="{{ app.id }}"
       data-candidate-id="{{ app.candidate_id }}">
    <div>
      <strong>{{ app.candidate_name }}</strong><br>
      <small class="small-muted">Applied: {{ app.applied_at }}</small><br>
      <small class="small-muted">Message: {{ app.message|default:"-" }}</small>
    </div>

    <div class="text-end">
      {# Invite button - enabled only if we have an interview id #}
      {% if iv %}
        <button class="btn btn-sm btn-outline-primary invite-btn"
                data-interview-id="{{ iv }}"
                data-candidate-id="{{ app.candidate_id }}"
                data-candidate-name="{{ app.candidate_name|escape }}">
          Invite
        </button>
      {% else %}
        <button class="btn btn-sm btn-outline-secondary" disabled>No interview</button>
      {% endif %}

      <span class="badge bg-secondary ms-2 invite-badge">
        {{ app.invite_status|default:"-" }}
      </span>
    </div>
  </div>
{% endwith %}
