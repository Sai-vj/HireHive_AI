...
async function showShortlistsForSelectedJob() {
  ...
  for (const s of list) {
    ...
    if (typeof s.resume === 'number') {
      resumeId = s.resume;
      // fetch resume detail
      const r = await apiFetch(`/api/resumes/${resumeId}/`);
      if (r.ok && r.data) {
        candidateName = r.data.candidate_name || r.data.user_name || r.data.uploaded_by || `Resume ${resumeId}`;
      } else {
        candidateName = `Resume ${resumeId}`;
      }
    } else if (s.resume && typeof s.resume === 'object') {
      resumeId = s.resume.id || '';
      candidateName = s.resume.candidate_name || s.resume.user_name || `Resume ${resumeId}`;
    }
    ...
    div.innerHTML = `
      <div class="d-flex justify-content-between align-items-start">
        <div style="min-width:0">
          ...
          <div class="small-muted">Shortlisted by: ${escapeHtml(s.shortlisted_by || '')}</div>
        </div>
        <div>
          <button class="btn btn-sm btn-outline-primary resend-btn" data-job="${escapeHtml(String(s.job))}" data-resume="${escapeHtml(String(resumeId))}">Resend</button>
