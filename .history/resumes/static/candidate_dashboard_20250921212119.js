...
(function () {
 ...
 try {
  ...
  async function loadInvites() {
    ...
    if (!res.ok) {
      console.log('loadInvites failed response:', res);
    ...
    console.log("Invites debug →", res.data, arr);
    ...
    arr.forEach(inv => {
      ...
      const canStartNow = (status === 'accepted') && isScheduledNowOrPast(scheduledRaw);
      const startBtnHtml = (status === 'accepted' && (interview.id||inv.interview_id))
        ? `<button class="btn btn-sm btn-primary start-invite" data-interview="${escapeHtml(interview.id||inv.interview_id)}" data-invite="${escapeHtml(id)}" ${canStartNow ? '' : 'disabled'}>Start</button>`
        : '';

      div.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center">
        <div style="min-width:0">
          <strong>${escapeHtml(title)}</strong><div class="small-muted">${escapeHtml(recruiter)} • ${escapeHtml(scheduledLocal)}</div>
        </div>
        <div style="text-align:right">
      ...
      div.querySelectorAll('.accept-invite').forEach(b => b.addEventListener('click', async () => {
        ...
        if (r.ok) { showToast('Accepted', 'success'); loadInvites(); } else { console.error('accept failed', r); showToast('Accept failed','error'); b.disabled=false; }
      }));
      div.querySelectorAll('.decline-invite').forEach(b => b.addEventListener('click', async () => {
        ...
        if (r.ok) { showToast('Declined', 'success'); loadInvites(); } else { console.error('decline failed', r); showToast('Decline failed','error'); b.disabled=false; }
      }));

      const startBtn = div.querySelector('.start-invite');
      if (startBtn) {
        startBtn.addEventListener('click', async (e) => {
          ...
          // handle blocked/forbidden / closed window gracefully
          if (r && (r.status === 403 || r.status === 400)) {
            ...
            } catch (e) { showToast('Start blocked by server', 'error'); }
        ...
        if (!canStartNow) {
          const schedDate = parseDateTime(scheduledRaw);
          if (schedDate) {
            const msToStart = schedDate.getTime() - Date.now();
            if (msToStart > 0 && msToStart < 7 * 24 * 60 * 60 * 1000) {
              setTimeout(() => { startBtn.disabled = false; }, msToStart + 1000);
            }
          }
        }
      }
...
async function viewInvite(inviteId) {
  ...
  // create or reuse modal quickly (minimal DOM ops so UI stays responsive)
  ...
  if (!content) return;
  // show immediate lightweight loading UI so user sees it's working
  content.innerHTML = `<div style="padding:16px;text-align:center">
      ...
      <div style="margin-top:8px">Loading invite…</div>
    </div>`;

  try {
    // show modal quickly (so browser paints loading state)
    try {
  // remove focus from any element so bootstrap can safely manage focus/aria
  try { document.activeElement && document.activeElement.blur(); } catch(e){}
  bootstrap.Modal.getOrCreateInstance(modal).show();
  ...
  modal.style.display = 'block';
    ...
    // build fragment URL using API base (safe)
    const apiBase = (API.INVITES || '/api/interviews/candidate/invites/').replace(/\/candidate\/.*$/, '');
    const fragUrl = `${apiBase}/fragments/invite_row/${inviteId}/`;

    // use AbortController to timeout fragment fetch in 5s
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 5000);

    let fragResp = null;
    try {
      fragResp = await _fetchWithAuth(fragUrl, { method: 'GET', credentials: 'same-origin', signal: controller.signal });
    } catch (err) {
      fragResp = null;
    } finally {
    ...
    if (fragResp && fragResp.ok) {
      // guard: limit length to avoid heavy DOM freeze
      const txt = await fragResp.text().catch(()=>null);
      if (typeof txt === 'string' && txt.length > 300000) {
        // too large — show summary link instead of inserting huge HTML
        content.innerHTML = `<div class="text-muted">Invite content is large. <a href="${fragUrl}" target="_blank" rel="noopener">Open in new tab</a></div>`;
      } else {
        content.innerHTML = txt || '<div class="text-muted">No content</div>';
      }
      return;
    }
  } catch (err) {
    console.debug('fragment attempt error', err);
    // fallthrough to JSON fallback
  }

  // JSON fallback (non-blocking)
  try {
    ...
    let arr = res.data?.invites || res.data?.results || (Array.isArray(res.data) ? res.data : []);
    ...
    const startDisabled = isScheduledNowOrPast(scheduledRaw) ? '' : 'disabled';

    // render lightweight HTML (avoid very large blocks or heavy loops)
    content.innerHTML = `
      <div><strong>${escapeHtml((interview.title||inv.title||'Interview'))}</strong></div>
      <div style="margin-top:6px"><strong>When:</strong> ${escapeHtml(formatLocalDateTime(scheduledRaw))}</div>
      ...
      <div style="margin-top:10px;text-align:right">
        ...
        ${(status === 'accepted') && (interview.id || inv.interview_id) ? `<button id="inviteStartBtn" class="btn btn-primary btn-sm" ${startDisabled}>Start</button>` : ''}
      </div>
    `;

    // wire buttons (lightweight)
    modal.querySelector('#inviteAcceptBtn')?.addEventListener('click', async () => {
      await respondInvite(inviteId, 'accept');
      try { const b = bootstrap.Modal.getInstance(modal); if (b) b.hide(); else modal.style.display = 'none'; } catch(e) { modal.style.display='none'; }
    });
    ...
    modal.querySelector('#inviteStartBtn')?.addEventListener('click', () => {
      ...
      (async () => {
        ...
        let url = null;
        if (r && r.ok && r.data) url = r.data.redirect_url || r.data.join_url || r.data.url || r.data.attempt_url;
        if (!url) {
          const pageBase = (API.INVITES || '/api/interviews/candidate/invites/').replace(/\/candidate\/.*$/, '');
          url = `${pageBase}/page/candidate/${encodeURIComponent(iid)}/?invite=${encodeURIComponent(inviteId)}`;
        }
    ...
    console.error('viewInvite final fallback error', err);
  ...
  if (m) {
    try { document.activeElement && document.activeElement.blur(); } catch(e){}
    try { bootstrap.Modal.getOrCreateInstance(m).show(); } catch(e) { m.style.display = 'flex'; }
  }
  loadInvites().catch(e => { console.error('loadInvites err', e); });
});
