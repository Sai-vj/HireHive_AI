...
// top of file - replace your current first import with this
import { apiFetchAsJson, fetchWithAuth, clearTokens, getAccessToken, isTokenExpired } from "./utils.js";
...
// --- jwt status UI (no duplicate imports) ---
(function jwtAuthStatus() {
  ...
  function checkAuth() {
    ...
    if (isTokenExpired(token)) { setStatus('Token expired', true); return false; }
    setStatus('Authenticated with JWT'); return true;
  }
  ...
  function createLogoutBtn() {
    ...
    btn.addEventListener('click', () => { clearTokens(); location.href='/'; });
...
(function () {
  ...
  async function loadJobs(){
    ...
    el.innerHTML = '<div class="small-muted">Loading jobs...</div>';
    const res = await _apiFetch(API.JOBS);
    if(!res || !res.ok) { el.innerHTML = `<div class="small-muted">Failed to load jobs (${res?.status||'error'})</div>`; return; }
    jobs = Array.isArray(res.data)? res.data : (res.data?.results || []);
    if(!jobs || jobs.length===0) { el.innerHTML = '<div class="small-muted">No jobs</div>'; return; }
    el.innerHTML = '';
    jobs.forEach(j => {
      ...
      card.innerHTML = `
        ...
        <div class="job-actions">
          ...
          <button class="btn btn-sm btn-success apply-btn" data-id="${id}" disabled>Apply</button>
          <div><small id="quiz-status-${id}" class="text-muted d-block" style="margin-top:6px;text-align:right">Not attempted</small></div>
        </div>`;
      el.appendChild(card);
    ...
    // load attempt summaries (non-blocking)
    setTimeout(()=> jobs.forEach(j => loadAttemptSummary(j.id)), 400);
  }
  ...
  function openJobDetailModal(job){
    ...
    modal.querySelector('#jobDetailTitle').innerText = job.title || `Job ${job.id||''}`;
    const body = modal.querySelector('#jobDetailBody');
    const company = job.company || '', desc = job.description || job.summary || '', skills = job.skills_required || job.skills || '';
    body.innerHTML = `<div><strong>Company:</strong> ${escapeHtml(company)}</div><div style="margin-top:8px"><strong>Skills:</strong> ${escapeHtml(skills)}</div><hr><div style="white-space:pre-wrap">${escapeHtml(desc)}</div>`;
    modal.querySelector('#jobDetailApplyBtn').onclick = ()=> { try{ bootstrap.Modal.getInstance(modal).hide(); }catch(e){}; openApplyModal(job.id || job.pk); };
  ...
  function createQuizModalIfMissing(){
    if(document.getElementById('quizModal')) return;
    const modal = document.createElement('div'); modal.id='quizModal';
    modal.style = 'position:fixed;inset:0;display:none;align-items:center;justify-content:center;background:rgba(0,0,0,0.45);z-index:99999';
    modal.innerHTML = `<div style="background:#fff;padding:18px;border-radius:8px;max-width:900px;width:96%;max-height:88vh;overflow:auto;position:relative;">
  ...
  function renderQuizQuestions(questions){
    ...
    qWrap.innerHTML = questions.slice(0,50).map(q => {
      ...
      const opts = choices.map((c,i)=> `<div><label><input type="radio" name="q-${q.id}" value="${escapeHtml(String(i))}"> ${escapeHtml(String(c))}</label></div>`).join('');
      return `<div class="quiz-question" data-qid="${q.id}" style="margin-bottom:12px"><div style="font-weight:600">${escapeHtml(q.question||q.title||'')}</div><div style="margin-left:8px">${opts}</div></div>`;
    }).join('');
    const meta = document.getElementById('quizMeta'); if(meta) meta.textContent = `Questions: ${questions.length}`;
  }
  ...
  async function submitQuizAttempt(auto=false){
    const modal = document.getElementById('quizModal'); if(!modal) return;
    const jobId = modal.dataset.jobId; const ans = {};
    modal.querySelectorAll('.quiz-question').forEach(q => { const qid=q.dataset.qid; const sel = q.querySelector('input[type="radio"]:checked'); ans[qid]= sel? sel.value : null; });
    const btn = document.getElementById('quizSubmit'); if(btn) btn.disabled=true;
    try {
      const payload = { job_id: jobId, answers: ans };
      let r = await _fetchWithAuth(API.QUIZ_ATTEMPT_SUBMIT, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) }).catch(()=>null);
      let body = null;
      ...
      const passed = body && (body.passed === true || body.passed === 'true');
      const score = body && (body.score || body.total ? `${body.score||0}/${body.total||''}` : null);
      if (passed) showToast(`Passed — ${score||''}`,'success'); else showToast(`Quiz submitted — ${score||''}`,'info');
      await loadAttemptSummary(jobId); await loadMyApplications(); closeQuizModal();
    } catch(e){ console.error('quiz submit', e); showToast('Quiz submit failed','error'); }
