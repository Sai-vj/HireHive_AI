...
(function () {
  ...
  // Correct for your backend: /api/interviews/recruiter/<job-id>/invite/
...
// Replace your ensureInviteModal() with this version
function ensureInviteModal() {
  // If already created return it
  let modal = document.getElementById('inviteModal');
  if (modal) return modal;

  // Create proper Bootstrap modal structure (modal > modal-dialog > modal-content)
  modal = document.createElement('div');
  modal.id = 'inviteModal';
  modal.className = 'modal fade';
  modal.setAttribute('tabindex', '-1');
  modal.setAttribute('role', 'dialog');
  modal.style = ''; // don't force display here
  ...
  // Build the modal inner markup (header/body/footer)
  content.innerHTML = `
  ...
  // safe: return the created modal element
  return modal;
}

// Ensure attempts modal exists (bootstrap-friendly)
function ensureAttemptsModal() {
  ...
  // safe bindings for fallback (if bootstrap not present)
  ...
  function safeShowModal(modal) {
    ...
    try {
      if (typeof bootstrap !== 'undefined' && modal instanceof HTMLElement) {
        // get or create an instance and show — safe API usage
