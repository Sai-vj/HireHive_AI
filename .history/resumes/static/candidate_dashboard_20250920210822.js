...
async function startInterview(interviewId, inviteId) {
  ...
  try {
    ...
    // Normalize response -> { ok, status, data } or Response
    let ok = false, status = 0, body = null;
    if (res instanceof Response) {
      status = res.status;
      ok = res.ok;
      try { body = await res.json(); } catch(e) { body = await res.text().catch(()=>null); }
    } else if (res && typeof res === 'object') {
      // apiFetch might already return normalized object
      ok = !!res.ok;
      status = res.status || 0;
      body = res.data ?? res;
    } else {
    ...
    if (!ok) {
      ...
      const detail = (body && (body.detail || body.error || body.message)) || `Status ${status}`;
      const scheduled = (body && body.scheduled_start) ? `Starts at: ${new Date(body.scheduled_start).toLocaleString()}` : '';
      const msg = `${detail}${scheduled ? ' — ' + scheduled : ''}`;

      // show to user
      if (typeof showToast === 'function') showToast(msg, 'error', 6000);
      const m = document.getElementById('message');
      if (m) m.innerHTML = `<div class="alert alert-danger">Cannot start interview: ${escapeHtml(detail)}${scheduled ? '<br/>' + escapeHtml(scheduled) : ''}</div>`;

      return { ok: false, res };
    }

    // success path
    // if backend returned a direct join url, use it; else open candidate page
    const joinUrl = (body && (body.redirect_url || body.join_url || body.url)) || null;
    if (joinUrl) {
      window.location.href = joinUrl;
      return { ok: true, url: joinUrl, method: 'api_join_url' };
    }

    // else go to candidate page (your in-app interview page)
    ...
    const m = document.getElementById('message');
    if (m) m.innerHTML = `<div class="alert alert-danger">Network error while starting interview.</div>`;
