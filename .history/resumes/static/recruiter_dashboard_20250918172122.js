...
// recruiter_dashboard.js (replace your file with this)
(function(){
  ...
  // DOM refs (update ids if your template differs)
  const jobsList = el('jobsList') || el('interviews-container') || el('interviewsContainer');
  ...
  if (!jobsList) console.warn('jobsList element not found - update the id in the script to match your template.');

  // apiFetch: adds Authorization header, tries refresh on 401 and retries once
        ...
        try {
          ...
          if (r.ok) {
            ...
            if (j.access) {
              ...
              res = await fetch(url, opts); // retry original
            }
          } else {
            // refresh failed; remove tokens to force login
  ...
  // show/hide invite modal (global since used on buttons)
  window.showInviteModal = function(interviewId, title){
    window.__inviteInterviewId = interviewId;
    const titleEl = el('inviteModalTitle');
    if (titleEl) titleEl.innerText = `Invite for: ${title || interviewId}`;
    el('invite_candidate_id').value = '';
    el('invite_scheduled_at').value = '';
    el('invite_message').value = '';
  ...
  // save token from UI input (optional)
  ...
  // render one interview card
  function renderInterviewCard(i){
    const titleSafe = escapeHtml(i.title || 'Interview').replace(/'/g,"\\'");
    const scheduled = i.scheduled_at ? new Date(i.scheduled_at).toLocaleString() : 'Not set';
    const inviteStatus = i.invite_status || '-';
    return `
      <div class="job-card" data-interview-id="${i.id}">
        <div class="d-flex justify-content-between">
          <div>
            <h6 class="job-title mb-0">${escapeHtml(i.title || 'Interview')}</h6>
            <div class="small-muted">Scheduled: ${scheduled}</div>
            <div class="small-muted">Status: <span class="invite-status">${inviteStatus}</span></div>
          </div>
          <div class="text-end">
            <button class="btn btn-sm btn-outline-primary me-1" onclick="window.location.href='/api/interviews/recruiter/${i.id}/'">View</button>
            <button class="btn btn-sm btn-secondary" onclick="showInviteModal(${i.id}, '${titleSafe}')">Invite Candidate</button>
          </div>
        </div>
      </div>
    `;
  ...
  async function loadInterviews(){
    ...
    try {
      ...
      if (!res.ok) {
        console.error('Failed to load interviews', res.status);
        jobsList.innerHTML = '<div class="text-danger">Failed to load interviews</div>';
        return;
      }
      ...
      jobsList.innerHTML = data.map(renderInterviewCard).join('');
    } catch (e) {
      console.error('Network error loading interviews', e);
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (current && tokenStatus) tokenStatus.innerText = 'Token saved';
    if (tokenInput && current) tokenInput.value = current;
    loadInterviews();
  });
