...
(function () {
  ...
  async function loadInvites() {
    ...
    // accept many shapes: array OR { results: [] } OR { invites: [] }
    ...
    arr.forEach(inv => {
      ...
      // compute scheduled local and start enablement
      ...
      // start-invite (open new tab, server redirect_url preferred)
      ...
      if (startBtn) {
        ...
        // schedule enable if not yet started
        if (!canStartNow) {
          ...
          if (schedDate) {
            ...
            if (msToStart > 0 && msToStart < 7 * 24 * 60 * 60 * 1000) {
              setTimeout(() => {
                // only enable visually; we won't re-check server here beyond status string
                startBtn.disabled = false;
              }, msToStart + 1000);
            }
          }
  ...
  async function viewInvite(inviteId) {
    ...
    // try HTML fragment first (if your server provides it) - use same API base as INVITES to avoid /api mismatch
    try {
      ...
      const fragResp = await _fetchWithAuth(fragUrl, { method: 'GET', credentials: 'same-origin' });
      ...
      console.debug('fragment fetch failed', e);
    }

    // fallback to JSON invite
    try {
      ...
      modal.querySelector('#inviteStartBtn')?.addEventListener('click', () => {
        ...
        try { const b = bootstrap.Modal.getInstance(modal); if (b) b.hide(); else modal.style.display = 'none'; } catch(e) { modal.style.display='none'; }
        // open in new tab via same logic as row start
        (async () => {
          const r = await _apiFetch(API.START_INTERVIEW(iid), { method: 'POST', headers: { 'Content-Type':'application/json' }, body: JSON.stringify({ invite: inviteId }) });
          let url = null;
