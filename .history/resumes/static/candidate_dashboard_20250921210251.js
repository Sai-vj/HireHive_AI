...
(function () {
 ...
 try {
  ...
  /* ================= Invites ================= */
// DEBUG VERSION - replace your loadInvites() temporarily with this
async function loadInvites() {
  const wrap = document.getElementById('invitesList'); if (!wrap) return;
  wrap.innerHTML = '<div class="small-muted">Loading invites (debug)...</div>';
  ...
  pre.textContent = 'DEBUG fetch result:\n' + JSON.stringify(res, null, 2);
  wrap.innerHTML = '';
  ...
  if (res.status && (res.status === 401 || res.status === 403)) {
    wrap.appendChild(document.createTextNode(`\nAuth problem: status ${res.status}. Check token or session.`));
  }
}
