...
if (sendBtn && !sendBtn._bound) {
  ...
  sendBtn.addEventListener('click', async (ev) => {
    ...
    const candidateId = parseInt(candidateIdRaw, 10);
     if ((!candidateId || Number.isNaN(candidateId)) && !resumeId) {
...
document.body.addEventListener('click', (ev) => {
  ...
  let candidateId = resolveCandidateIdFromElement(btn) || resolveCandidateIdFromElement(btn.closest && btn.closest('.application-row'));
   ...
   if (!candidateId) {
    ...
    // pass resumeId to modal
    showInviteModal({ jobId: jobId || null, interviewId: interviewId || null, candidateId: '', candidateName: displayName, resumeId: resume || '' });
    return;
  }
