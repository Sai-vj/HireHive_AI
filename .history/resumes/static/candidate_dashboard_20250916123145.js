...
(function(){
  ...
  // Candidate side: GET quiz
  fetchWithAuth(`/api/quiz/${jobId}/`, { method: 'GET' })
    .then(async r => {
      if (!r.ok) {
        const err = await r.json().catch(()=>null);
        wrap.innerHTML = `<div class="text-danger">Error: ${err?.detail || r.statusText}</div>`;
        return;
      }
      const data = await r.json();
      // 👇 backend returns {"id":..,"questions_json":[..]} via serializer
      if (data && Array.isArray(data.questions_json)) {
        renderQuestionsInModal(data.questions_json);
      } else {
