...
try {
    ...
    const payload = { job_id:Number( jobId), resume_id:Number (resumeId), message };
    

    // 1) Try JSON first (Content-Type: application/json)
    
console.log('Apply payload (JSON):',payload,APPLY_URL=',APPLY_URL)
    ...
    // If server returned 400, capture server body text for debugging
    if (res && res.status === 400) {
      console.warn('Apply: server returned 400. Response data:', res.data);
      // If res.data is text/html or string, show it in console for debugging
    }

    // If JSON failed (400/415) then try FormData fallback
    if (!res.ok && (res.status === 400 || res.status === 415 || res.status === 415)) {
      ...
      // Use fetchWithAuth directly for FormData fallback (utils helper)
    ...
    console.log('Final apply response:', res);

    // existing success / already applied / error handling
    if (res.ok) {
      ...
      if (res.status === 409) {
        ...
        const d = res.data || {};
        const msg = (typeof d === 'object' && d.detail) ? d.detail : JSON.stringify(d);
        showToast('Already applied: ' + msg, 'info', 7000);
