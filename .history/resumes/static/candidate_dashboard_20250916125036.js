...
async function onQuizSubmit(jobId, answers) {
  // answers: array of { question_id, choice } OR object { q1: 'A', q2: 'B' }
  try {
    // normalize answers to mapping { qid: choice }
    let payloadAnswers = {};
    if (Array.isArray(answers)) {
      answers.forEach(a => {
        if (!a) return;
        const qid = a.question_id ?? a.qid ?? a.id;
        const choice = a.choice ?? a.value ?? a.answer;
        if (qid !== undefined) payloadAnswers[String(qid)] = choice;
      });
    } else if (typeof answers === 'object' && answers !== null) {
      // might already be mapping
      payloadAnswers = { ...answers };
    }

    // optional resume id - try to read if form stores it
    const resumeId = document.getElementById('applyResumeSelect')?.value || null;

    // endpoints to try (most likely first)
    const tried = [];
    const endpoints = [
      `/api/quiz/${jobId}/attempt/`,
      `/api/quiz/attempt/`,
      `/api/quiz/${jobId}/submit/`,
      `/api/quiz/submit/`
    ];

    // helper to POST JSON and return { ok, status, json, text }
    async function postJson(url, body) {
      ...
      try {
        ...
        return { ok: r.ok, status: r.status, json, text, raw: r };
      } catch (e) {
    ...
    // build body
    const body = { answers: payloadAnswers };
    ...
    for (const ep of endpoints) {
      ...
      if (r.ok) { finalResp = r; break; }
      // if 404 try next; if 401/403 -> stop and show auth message
      if (r.status === 401 || r.status === 403) {
        showToast('Authentication error while submitting quiz. Paste token and Save.', 'error', 7000);
        console.warn('Quiz submit auth error', r);
        return;
      }
      // else continue trying
      console.warn('Quiz submit attempt failed for', ep, 'status', r.status, r.text || r.error);
    }

    // if still null => all tried and failed
    if (!finalResp) {
      showToast('Submit failed — endpoint not found. Check console for tried URLs.', 'error', 7000);
      console.error('Submit failed, tried endpoints:', tried);
      return;
    }
    ...
    if (!finalResp.ok) {
      ...
      const msg = finalResp.json?.detail || finalResp.json?.message || finalResp.text || `Status ${finalResp.status}`;
      showToast('Quiz submit error: ' + msg, 'error', 7000);
      console.warn('Quiz submit final error', finalResp);
      return;
    }

    // success
    const data = finalResp.json || {};
    // data expected: { attempt_id, score, passed, correct, total }
    handleQuizSubmitResponse(jobId, data);
    // hide modal
    try { closeQuizModal(); } catch(e){}
    showToast(`Submitted — score: ${data.score ?? data.correct ?? '?'} / ${data.total ?? '?'} `, 'success', 4000);

  } catch (err) {
    console.error('onQuizSubmit exception', err);
    showToast('Network error while submitting quiz', 'error', 5000);
  }
}
