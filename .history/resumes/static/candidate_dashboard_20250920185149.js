...
const res = await apiFetch(apiStart, {
  ...
  body: JSON.stringify(payload),
    ...
    if (!res.ok) {
      // auth or other error
      if (res.status === 401 || res.status === 403) {
        showToast('Authentication required. Save token or login in browser.', 'error', 6000);
      } else {
        const msg = res.data?.detail || res.data?.error || `Status ${res.status}`;
        showToast('Failed to start interview: ' + msg, 'error', 6000);
      }
      return { ok: false, res };
    }
