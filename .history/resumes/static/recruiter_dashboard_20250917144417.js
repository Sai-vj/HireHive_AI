...
function openEditJobPrompt(jobId) { 
  ...
  const newTitle = prompt('New title for job?'); if (newTitle === null) return; updateJobTitle(jobId, newTitle); } async function updateJobTitle(jobId, title) { const res = await apiFetch(/api/resumes/jobs/${jobId}/, { method:'PATCH', body: JSON.stringify({ title }) }); if (!res.ok) { showToast('Update failed', 'error'); return; } showToast('Job updated', 'success'); // update DOM quickly const card = document.querySelector(.job-card[data-job-id="${jobId}"]); if (card) card.querySelector('h6') && (card.querySelector('h6').textContent = title); }
