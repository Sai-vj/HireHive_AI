...
async function startInterview(interviewId, inviteId) {
  ...
  // Option A: attempt to call API start (for non-browser clients) — optional
  try {
    // If you want to use API start for token-based clients, uncomment below:
    // const apiRes = await apiFetch(`/api/interviews/candidate/${interviewId}/start/`, { method: 'POST' });
    // if (apiRes && apiRes.ok && (apiRes.data?.url || apiRes.data?.join_url)) {
    //   const joinUrl = apiRes.data?.url || apiRes.data?.join_url;
    //   window.open(joinUrl, '_blank', 'noopener,noreferrer');
    //   return { ok: true, url: joinUrl };
    // }

    // Primary: open login-protected frontend helper route that creates attempt and redirects
    const frontend = FRONTEND_INTERVIEW_START_HELPER(interviewId, inviteId);
    window.open(frontend, '_blank', 'noopener,noreferrer');
    return { ok: true, url: frontend };
  } catch (e) {
    console.error('startInterview error', e);
    showToast('Failed to open interview', 'error');
    return { ok: false, error: e };
  }
}
