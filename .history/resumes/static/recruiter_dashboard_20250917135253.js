...
const API_ROOT = '/api';
const JOBS_ENDPOINT = `${API_ROOT}/resumes/re/`; // adjust if different
...
async function loadJobs() {
  ...
  if (!res.ok || !Array.isArray(res.data)) {
    container.innerHTML = `<div class="small-muted">No jobs or error (${res.status})</div>`;
    return;
  }
  const jobs = res.data;
  container.innerHTML = '';
  jobs.forEach(j => {
    // job-card with edit/delete and generate buttons included
    const div = document.createElement('div');
    div.className = 'list-group-item job-card';
    div.dataset.jobId = j.id;
    div.innerHTML = `
      <div class="d-flex w-100 justify-content-between align-items-start">
        <div style="flex:1;min-width:0;cursor:pointer" data-action="select" data-job-id="${j.id}">
          <h6 class="mb-1" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(j.title || '')}</h6>
          <div class="small-muted" style="font-size:.9rem">${escapeHtml(j.company || '')} • ${escapeHtml(j.skills_required || j.skills || '')}</div>
        </div>
  ...
  // wire click on job title to select
...
function attachJobCardEvents() {
  qsa('.view-job-btn').forEach(b=>{ if (b._bound) return; b._bound=true; b.addEventListener('click', ()=> openJobDetail(b.dataset.jobId)); });
  qsa('.edit-job-btn').forEach(b=>{ if (b._bound) return; b._bound=true; b.addEventListener('click', ()=> openEditJobPrompt(b.dataset.jobId)); });
  qsa('.delete-job-btn').forEach(b=>{ if (b._bound) return; b._bound=true; b.addEventListener('click', ()=> confirmAndDeleteJob(b.dataset.jobId)); });
  qsa('.generate-quiz-btn').forEach(b=>{ if (b._bound) return; b._bound=true; b.addEventListener('click', ()=> generateQuizForJob(Number(b.dataset.jobId))); });
}
