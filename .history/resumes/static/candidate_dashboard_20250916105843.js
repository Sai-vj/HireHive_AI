...
(function(){
  ...
  function openQuizModal(jobId, questionPayload) {
   ...
   // --- robust generate fetch (tries multiple likely endpoints) ---
async function tryGenerateQuestions(jobId) {
  ...
  for (const url of tries) {
    try {
      const r = await fetch(url, { method:'POST', headers, body: JSON.stringify({ job_id: jobId }) });
      const text = await r.text().catch(()=>null);
      let data = null;
      try { data = text ? JSON.parse(text) : null; } catch(e){ data = text; }

      if (r.ok && data && data.questions) {
        return { ok:true, url, data };
      }
      // if 404 continue trying - but surface useful 4xx/5xx text if first found
      if (!r.ok && r.status !== 404) {
        return { ok:false, url, status: r.status, data };
      }
      // else try next
    } catch (err) {
  ...
  return { ok:false, error: 'no-endpoint-found' };
}

// replace the original fetchWithAuth(...) block with:
tryGenerateQuestions(jobId).then(result => {
  const wrap = document.querySelector('#quiz-questions-wrap');
  if (!result.ok) {
    if (result.status) {
      wrap.innerHTML = `<div class="text-danger">Error loading questions: ${result.status} ${result.data ? (result.data.detail || JSON.stringify(result.data)) : ''} (tried multiple endpoints)</div>`;
    } else {
      wrap.innerHTML = `<div class="text-danger">Error loading questions: ${result.error || 'Not Found'} (tried multiple endpoints)</div>`;
    }
    console.warn('quiz generate try result', result);
    return;
  }
  console.log('quiz generate using', result.url);
  renderQuestionsInModal(result.data.questions);
}).catch(e => {
  const wrap = document.querySelector('#quiz-questions-wrap');
  ...
  body: JSON.stringify({ resume_id, answers })
});
