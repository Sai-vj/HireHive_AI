...
import { saveTokens, apiFetchAsJson } from './utils.js';

const JOBS_URL = '/resumes/jobs/';   // corrected path

// example usage
async function loadJobs() {
  const r = await apiFetchAsJson(JOBS_URL);
  if (!r.ok) {
    console.error("Job fetch failed", r);
    return;
  }
  console.log("Jobs loaded", r.data);
}
