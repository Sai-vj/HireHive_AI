...
async function viewInvite(inviteId) {
  ...
  try {
    // Use bootstrap Modal for proper focus handling if available
    try { new bootstrap.Modal(modal, { backdrop: true }).show(); }
    catch (e) { modal.style.display = 'block'; document.body.style.overflow = 'hidden'; }
  } catch (e) { modal.style.display = 'block'; document.body.style.overflow = 'hidden'; }
