// candidate_dashboard.js (updated - first chunk ~500 lines)
...
async function loadJobs() {
  ...
  jobs.forEach(j => {
    ...
    card._job = j;
    // set a consistent data attribute for other helpers
...
(function () {
  ...
  // ---- continuation of IIFE from previous chunk ----
  ...
  function renderQuestionsInModal(questions) {
    ...
    wrap.innerHTML = questions.map(q => {
      // choices could be array or object; normalize to entries
      let choices = q.choices || q.options || {};
      if (Array.isArray(choices)) {
        // convert ['A','B'] -> {0:'A',1:'B'}
        const obj = {};
        choices.forEach((c, i) => obj[i] = c);
        choices = obj;
      }
      const opts = Object.entries(choices || {}).map(([key, val]) => {
      ...
      return `<div class="quiz-question" data-qid="${q.id}" style="margin-bottom:12px;"><div style="font-weight:600;margin-bottom:6px">${escapeHtml(q.question || q.prompt || q.title)}</div><div style="margin-left:6px;">${opts}</div></div>`;
    }).join('');
  }
  ...
  function renderJobAttemptSummaryFromAttempt(jobCardEl, attempt) {
    ...
    wrap.innerHTML = `Attempt ${escapeHtml(String(attemptId))}: ${escapeHtml(String(score))}/${escapeHtml(String(total))} — <strong style="color:${passed ? 'green' : 'crimson'}">${passed ? 'Passed' : 'Failed'}</strong> <small style="color:#666">(${escapeHtml(whenText)})</small>`;
  }
  ...
  // delegation — works even for dynamically added buttons
  document.addEventListener('click', function(e){
    const btn = e.target.closest && e.target.closest('.view-attempts-btn');
    if (!btn) return;
    e.preventDefault && e.preventDefault();

    const jid = Number(btn.dataset.jobId || btn.getAttribute('data-job-id'));
    if (!jid) { alert('Missing job id'); return; }
    if (typeof openAttemptHistoryModal === 'function') openAttemptHistoryModal(jid, btn);
  });
...
// ---------------- Attempt-history modal helper (separate IIFE) ----------------
(function(){
  function showModal() {
  ...
  function renderAttemptList(attempts){
    ...
    attempts.slice().sort((a,b)=> new Date(b.finished_at||b.started_at||0) - new Date(a.finished_at||a.started_at||0)).forEach(at => {
      ...
      let score = '';
      try {
        const sc = at.score ?? at.correct ?? 0;
        const tot = at.total ?? at.total_questions ?? at.total_questions_count ?? '';
        score = `${sc}${tot ? ` / ${tot}` : ''}`;
      } catch (e) { score = at.score || ''; }
      const passed = at.passed ? '<strong style="color:green">Passed</strong>' : '<strong style="color:crimson">Failed</strong>';

      // answers: try to pretty-print if available
      let answersHtml = '';
      if (at.answers) {
        try {
          answersHtml = '<pre style="white-space:pre-wrap;margin:0;padding:0;font-size:.9rem;">' + escapeHtml(typeof at.answers === 'string' ? at.answers : JSON.stringify(at.answers, null, 2)) + '</pre>';
        } catch(e){ answersHtml = escapeHtml(String(at.answers)); }
      } else if (at.data && at.data.answers) {
        answersHtml = '<pre style="white-space:pre-wrap;margin:0;padding:0;font-size:.9rem;">' + escapeHtml(JSON.stringify(at.data.answers, null, 2)) + '</pre>';
      } else {
  ...
  document.addEventListener('click', function(e){
    ...
    // also clicking outside inner box closes
    const modal = document.getElementById('attempts-modal');
...
async function loadInvites() {
  ...
  // Use candidate invites endpoint (corrected)
  const res = await apiFetch('/api/interviews/candidate/invites/', { method: 'GET' });
  if (res && (res.status === 401 || res.status === 403)) {
  ...
  if (!res || !res.ok) {
    listEl.innerHTML = `<div class="text-danger">Failed to load invites (${res ? res.status : 'no response'})</div>`;
    return;
  }

  const invites = Array.isArray(res.data) ? res.data : (res.data?.results || []);
  if (!invites.length) {
  ...
  invites.forEach(inv => {
    ...
    // try to find interview id in common fields
    ...
    div.innerHTML = `
      <div class="d-flex justify-content-between align-items-start">
        <div style="min-width:0">
          <strong style="display:block">${escapeHtml(interview.title || 'Interview')}</strong>
          <div class="small-muted" style="margin-top:4px">Scheduled: ${interview.scheduled_at ? new Date(interview.scheduled_at).toLocaleString() : '—'}</div>
      ...
      <div class="invite-details mt-2" style="display:${status==='accepted'?'block':'none'}">
        ...
        <div>${escapeHtml(interview.description || interview.instructions || 'No description')}</div>
        <div class="small-muted">Mode: ${escapeHtml(interview.mode || '—')} • Duration: ${escapeHtml(String(interview.duration_minutes || interview.duration || '—'))} mins</div>
      </div>
    `;
  ...
  listEl.querySelectorAll('.accept-invite-btn').forEach(btn => {
    if (btn.__acceptAttached) return;
    btn.__acceptAttached = true;
  ...
  listEl.querySelectorAll('.decline-invite-btn').forEach(btn => {
    if (btn.__declineAttached) return;
    btn.__declineAttached = true;
    btn.addEventListener('click', async () => {
      btn.disabled = true;
  ...
  listEl.querySelectorAll('.start-interview-btn').forEach(btn => {
    if (btn.__startAttached) return;
    btn.__startAttached = true;
    btn.addEventListener('click', (e) => {
      const iid = btn.dataset.interviewId;
      const inviteId = btn.dataset.inviteId;
      if (!iid) return showToast('Interview id missing', 'error');
      // navigate to dedicated interview page (you can customize route)
...
async function respondInvite(inviteId, answer) {
  ...
  // POST to respond endpoint - many backends expect /invites/{id}/respond/ or candidate/{id}/respond/
  // Use the endpoint your backend exposes; this implementation tries the common patterns.
  const tries = [
    `/api/interviews/candidate/invites/${inviteId}/respond/`,
    `/api/interviews/candidate/${inviteId}/respond/`,
    `/api/interviews/invites/${inviteId}/respond/`
  ];

  let lastRes = null;
  for (const u of tries) {
    try {
      const res = await apiFetch(u, { method: 'POST', body: JSON.stringify({ response: answer }) });
      lastRes = res;
      if (res && res.ok) break;
    } catch (e) {
      console.warn('respondInvite try failed', u, e);
    }
  }

  const res = lastRes;
  if (res && res.ok) {
    showToast(`Invite ${answer}ed`, 'success');
    // refresh list so Start button shows after accept
...
async function startInterview(interviewId) {
  ...
  const res = await apiFetch(`/api/interviews/candidate/${interviewId}/start/`, { method: 'POST' });
  if (!res || !res.ok) return showToast('Failed to start interview', 'error');
  const data = res.data || {};
  // Show in modal if you have one (optional)
  let modalEl = document.getElementById('interviewModal');
  if (!modalEl) {
    modalEl = document.createElement('div');
    modalEl.className = 'modal fade';
    modalEl.id = 'interviewModal';
    modalEl.innerHTML = `
  ...
  if (bodyEl) bodyEl.innerHTML = `<pre style="white-space:pre-wrap">${escapeHtml(JSON.stringify(data, null, 2))}</pre>`;
  try {
    const m = new bootstrap.Modal(modalEl, { backdrop: 'static' });
    m.show();
  } catch (e) {
    // fallback just open new tab with interview URL if available
    if (data.url) window.open(data.url, '_blank');
    else showToast('Interview started (response shown in console)', 'info');
    console.log('startInterview data:', data);
  }
}

// expose invites loader globally so HTML can call refresh
window.loadInvites = loadInvites;
window.respondInvite = respondInvite;
window.startInterview = startInterview;

// run invites loader if invites area exists
document.addEventListener('DOMContentLoaded', () => {
  try { loadInvites(); } catch (e) { /* ignore */ }
});

// ----------------- END of candidate_dashboard.js remainder -----------------

  // ... rest of file continues (quiz rendering, attempts handlers, attempt history modal, invites)
  // I stopped here to keep the first chunk under the size you requested.
  // The remaining code includes:
  //  - renderQuestionsInModal, fetchAttemptsForJob, loadAttemptHistoryAndRender
  //  - attachQuizButtons, attempt-history modal helper, autoSubmitQuiz/onQuizSubmit
  //  - Invites section (loadInvites, respondInvite, startInterview) with corrected endpoints:
  //      * list:   GET  /api/interviews/candidate/invites/
  //      * respond: POST /api/interviews/candidate/invites/{inviteId}/respond/
  //      * start:  POST /api/interviews/candidate/{interviewId}/start/
  //
  // If you want I will paste the next chunk right away (continuation from here),
  // or paste the whole remainder in smaller pieces (you asked for separate pieces).
  //
  // Changes made in invites area:
  //  - uses `/api/interviews/candidate/invites/` for list
  //  - uses `/api/interviews/candidate/invites/${inviteId}/respond/` for responding
  //  - on accept we call loadInvites() to refresh and show Start Interview button
  //  - Start Interview navigates to `/interviews/candidate/${interviewId}/?invite=${inviteId}`
  //
  // Tell me: paste the next ~400-500 lines now? I can continue right away with the rest.
})();
