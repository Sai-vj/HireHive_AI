...
`;
  ...
  btn.addEventListener('click', ev => { 
    ...
    if (!jid) return showToast('No job selected', 'error');
    // Call the review function
    openReviewAttempts(jid);  // 👉 you must define this
  });
});
...
// put this BEFORE window.rdash = { ... }
async function openReviewAttempts(jobId) {
  if (!jobId) return showToast('No job id', 'error');
  try {
    const modal = ensureAttemptsModal();
    ensureAttemptBindings();
    const loading = qs('#attempts-loading'), list = qs('#attempts-list');
    qs('#attempts-modal-title').textContent = `Attempts — job ${jobId}`;
    loading && (loading.style.display = 'block');
    list && (list.style.display = 'none');
    safeShowBootstrapModal(modal);
    const data = await fetchAttempts(jobId, null);
    loading && (loading.style.display = 'none');
    if (!data) { list && (list.style.display = 'block', list.innerHTML = '<div class="text-danger">Error fetching attempts</div>'); return; }
    renderAttemptList(data);
    list && (list.style.display = 'block');
  } catch(e) { console.error(e); showToast('Failed to load attempts', 'error'); }
}
// then expose
window.openReviewAttempts = openReviewAttempts;
...
async function openReviewAttempts(jobId) {
  ...
  // show attempts modal (re-uses the existing attempts modal + renderer)
  try {
    // show spinner while loading
    const modal = ensureAttemptsModal();
    ensureAttemptBindings();
    const loading = qs('#attempts-loading');
    ...
    // fetch attempts (uses your existing fetchAttempts util)
    const data = await fetchAttempts(jobId, null);
    loading && (loading.style.display = 'none');

    if (!data) {
      list && (list.style.display = 'block', list.innerHTML = '<div class="text-danger">Error fetching attempts</div>');
      return;
    }
    ...
    console.error('openReviewAttempts error', e);
