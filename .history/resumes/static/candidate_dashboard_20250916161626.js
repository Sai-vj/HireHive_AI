...
(function () {
  ...
  // call this after modal & questions are rendered
(async () => {
  const modal = document.querySelector('#quiz-modal');
  if (!modal) return;
  const jobId = parseInt(modal.dataset.jobId, 10);
  const res = await fetchAttemptCount(jobId);

  // handle special returns
  if (res && typeof res === 'object' && res.error) {
    if (res.status === 401) {
      showToast('Authentication required (token expired). Please login again.', 'error', 4000);
      // optionally prompt token input
      return;
    }
    if (res.status === 403) {
      // backend said "Maximum 3 attempts reached"
      showToast(res.detail || 'Maximum attempts reached', 'error', 4000);
      // disable UI
      modal.querySelectorAll('input[type="radio"]').forEach(i => i.disabled = true);
      const sb = modal.querySelector('#quiz-submit-btn');
      if (sb) { sb.disabled = true; sb.innerText = 'Max attempts reached'; }
      // add small text
      let s = modal.querySelector('#attempt-summary');
      if (!s) {
        s = document.createElement('div'); s.id = 'attempt-summary';
        s.style = 'margin-top:10px;color:#666;font-size:.9rem';
        modal.querySelector('#quiz-questions-wrap').after(s);
      }
      s.textContent = `Attempts: ${res.detail || 'max reached'}`;
      return;
    }
  }

  // ok numeric result
  const attemptsCount = Number(res) || 0;
  const MAX = 3;
  let summaryEl = modal.querySelector('#attempt-summary');
  if (!summaryEl) {
    summaryEl = document.createElement('div'); summaryEl.id = 'attempt-summary';
    summaryEl.style = 'margin-top:10px;color:#666;font-size:.9rem';
    modal.querySelector('#quiz-questions-wrap').after(summaryEl);
  }
  summaryEl.textContent = `Attempts: ${attemptsCount} / ${MAX}`;

  if (attemptsCount >= MAX) {
    modal.querySelectorAll('input[type="radio"]').forEach(i => i.disabled = true);
    const sb = modal.querySelector('#quiz-submit-btn');
    if (sb) { sb.disabled = true; sb.innerText = 'Max attempts reached'; }
    showToast('Maximum attempts reached — retake not allowed', 'error', 5000);
    return;
  }
})();
...
// robust fetchAttemptCount - place in same JS file (candidate_dashboard.js)
async function fetchAttemptCount(jobId) {
  const tries = [
    `/api/quiz/${jobId}/attempts/`,             // candidate-specific preferred
  ...
  for (const u of tries) {
    try {
      ...
      // If auth error (expired token) -> return special object
      if (status === 401) {
        return { error: 'unauthorized', status: 401, raw: data || text };
      }
      // If backend explicitly forbids because attempts reached, return indicator
      if (status === 403) {
        // some backends send {"detail":"Maximum 3 attempts reached"}
        let detail = null;
        if (data && typeof data === 'object') detail = data.detail || data.message || JSON.stringify(data);
        return { error: 'forbidden', status: 403, detail: detail || text || 'Forbidden' };
      }
      ...
      if (r.ok) {
        ...
        // sometimes backend returns object list as plain object -> try to count its keys
        if (data && typeof data === 'object' && !Array.isArray(data)) {
          // if it's something like { count: 5, results: [...] } handle results above
          // fallback: no attempts array present
          return 0;
        }
  ...
  // nothing worked -> return 0 attempts
  return 0;
}
