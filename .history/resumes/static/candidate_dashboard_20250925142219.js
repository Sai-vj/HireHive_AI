...
// Try to resolve a friendly job/title from several sources:
// 1) explicit interview title (inv.interview_title / interview.title / inv.title)
// 2) job id referenced on interview/invite -> lookup in global `jobs[]`
// 3) nested job object on invite (inv.job or interview.job)
// 4) fallback to "Interview <id>"
let title = inv.interview_title || interview.title || inv.title || null;
...
if (!title) {
  ...
  const nestedJob = inv.job && typeof inv.job === 'object' ? inv.job : (interview.job && typeof interview.job === 'object' ? interview.job : null);
