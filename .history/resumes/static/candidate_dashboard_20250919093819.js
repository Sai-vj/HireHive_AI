/* candidate_dashboard.js - cleaned & consolidated (non-module friendly)
   Save as /static/candidate_dashboard.js (loaded as a normal script, not type="module")
*/

/* ---------- small polyfills / fallbacks & shared helpers ---------- */

// Use global functions from utils.js if available, else provide simple fallbacks
const fetchWithAuth = window.fetchWithAuth || (async (url, opts = {}) => fetch(url, opts));
const apiFetch = window.apiFetchAsJson || window.apiFetch || (async function (path, opts = {}) {
  // minimal normalize wrapper returning { ok, status, data, text }
  try {
...
const saveTokens = window.saveTokens || (v => { if (v) localStorage.setItem('token', v); });
const getAccessToken = window.getAccessToken || (() => localStorage.getItem('token') || '');
const clearTokens = window.clearTokens || (() => localStorage.removeItem('token'));

// escapeHtml early so used everywhere
...
function uploadWithXHR(file) {
  return new Promise(resolve => {
    ...
    const token = getAccessToken(); if (token) xhr.setRequestHeader('Authorization', 'Bearer ' + token);
...
function updateAttemptHistoryRender(jobId, attempts) {
  ...
  wrap.innerHTML = attempts.slice(0, 5).map(a => {
    return `<div>Attempt ${a.id || a.attempt_id}: ${a.score || 0}/${a.total || a.total_questions || ''} — ${a.passed ? 'Passed' : 'Failed'} <small>(${new Date(a.created_at || a.created || Date.now()).toLocaleString()})</small></div>`;
  }).join('');
}
...
/* Fetch job detail and render modal */
...
/* Quiz helpers: enable/disable apply */
function enableApplyButton(jobId) {
  const btn = document.querySelector(`#apply-btn-${jobId}`) || document.querySelector(`.apply-btn[data-job-id="${jobId}"]`);
...
/* handle quiz submit responses (single canonical) */
async function handleQuizSubmitResponse(jobId, responseData) {
  // update attempt history area
  try { await loadAttemptHistoryAndRender(jobId); } catch (e) { console.warn(e); }

  if (responseData && responseData.passed) {
    enableApplyButton(jobId);
    showToast(`Passed ✅ Score: ${responseData.score || responseData.correct || 0}/${responseData.total || 0}`, 'success');
    const lbl = document.querySelector(`#quiz-status-${jobId}`);
    if (lbl) { lbl.textContent = 'Passed'; lbl.style.color = 'green'; }
    const retake = document.getElementById(`retake-btn-${jobId}`);
    if (retake) retake.style.display = 'none';
  } else if (responseData) {
    disableApplyButton(jobId);
    showToast(`Failed ❌ Score: ${responseData.score || responseData.correct || 0}/${responseData.total || 0}`, 'info');
    const retake = document.getElementById(`retake-btn-${jobId}`);
    if (retake) retake.style.display = 'inline-block';
    const lbl = document.querySelector(`#quiz-status-${jobId}`);
    if (lbl) { lbl.textContent = 'Failed'; lbl.style.color = 'crimson'; }
  }

  // ensure apply button toggled on job card if present
  const jobCard = document.querySelector(`.job-card[data-job-id="${jobId}"]`) || document.querySelector(`[data-job-id="${jobId}"]`);
  if (jobCard && responseData && responseData.passed) {
    const applyBtn = jobCard.querySelector('.apply-btn') || jobCard.querySelector(`#apply-btn-${jobId}`);
...
/* onQuizSubmit used by quiz modal */
async function onQuizSubmit(jobId, answers) {
  try {
    ...
    if (res.ok) {
      ...
      console.warn('Quiz submit error', res.status, data);
    }
  } catch (err) {
...
async function showMatchesForSelectedJob() {
  ...
  matches.forEach(m => {
    ...
    let score = 0;
    try {
      const raw = m.score ?? m.score_percent ?? m.score_resume_for_job ?? m.embedding_score ?? (Array.isArray(m.scores) ? m.scores[0] : undefined) ?? 0;
      score = parseScoreValue(raw);
...
/* ---------- Quiz + attempts client (self-contained) ---------- */
(function () {
  ...
  document.addEventListener('click', function(e){
    ...
    if (typeof openAttemptHistoryModal === 'function') openAttemptHistoryModal(jid);
  });
  ...
  console.log('candidate_dashboard.js loaded - quiz attachers registered.');
})();

/* ---------- Attempt-history modal helper (self-contained) ---------- */
(function(){
  function showModal() {
  ...
  function escapeHtmlLocal(s){ if (s==null) return ''; return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }
  ...
  function renderAttemptList(attempts){
    ...
    attempts.slice().sort((a,b)=> new Date(b.finished_at||b.started_at||0) - new Date(a.finished_at||a.started_at||0)).forEach(at => {
      ...
      if (at.answers) {
        try {
          answersHtml = '<pre style="white-space:pre-wrap;margin:0;padding:0;font-size:.9rem;">' + escapeHtmlLocal(typeof at.answers === 'string' ? at.answers : JSON.stringify(at.answers, null, 2)) + '</pre>';
        } catch(e){ answersHtml = escapeHtmlLocal(String(at.answers)); }
      } else if (at.data && at.data.answers) {
        answersHtml = '<pre style="white-space:pre-wrap;margin:0;padding:0;font-size:.9rem;">' + escapeHtmlLocal(JSON.stringify(at.data.answers, null, 2)) + '</pre>';
      } else {
      ...
      tr.innerHTML = `
        <td style="padding:8px;border-bottom:1px solid #f2f2f2;vertical-align:top">${escapeHtmlLocal(id)}</td>
        <td style="padding:8px;border-bottom:1px solid #f2f2f2;vertical-align:top">${escapeHtmlLocal(finished)}</td>
        <td style="padding:8px;border-bottom:1px solid #f2f2f2;vertical-align:top">${escapeHtmlLocal(score)}</td>
