...
// --- review attempts delegation (add to recruiter_dashboard.js) ---
(function() {
  const parent = document.querySelector('#reviewTable') || document.body;

  parent.addEventListener('click', (ev) => {
    const btn = ev.target.closest && ev.target.closest('.review-attempts-btn');
    if (!btn) return;

    ev.preventDefault();
    console.log('[debug] review button clicked, dataset:', btn.dataset);

    const interviewId = btn.dataset.interviewId || btn.dataset.id;
    if (!interviewId) {
      console.warn('No interview id on review button', btn);
      alert('No interview id provided for this item');
      return;
    }

    // If loadAttempts is a module-scoped function, ensure it's exposed:
    if (typeof window.loadAttempts === 'function') {
      window.loadAttempts(interviewId);
    } else if (typeof loadAttempts === 'function') {
      // direct call if available in same scope
      loadAttempts(interviewId);
    } else {
      // fallback: navigate to same page with query param (the review page reads ?interview=)
      window.location.search = new URLSearchParams({ interview: interviewId }).toString();
    }
  });
})();
...
async function showShortlistsForSelectedJob() {
  ...
  window.rdash = {
    ...
    apiFetch, apiFetchSimple, setSavedToken, savedToken,loadAttempts
  };
