// candidate_dashboard.js -- Patched version with safe modal handling & invites fixes
import {
  saveTokens,
  clearTokens,
  getAccessToken,
...
(function () {
  ...
  // ---------- Modal safety helpers (new) ----------
/* ---------------- safe modal helpers (add this block) ---------------- */

/**
 * getCookie(name) - small helper for CSRF fallback
 */
 ...
 * - Uses bootstrap.Modal when available; otherwise uses a safe style-based fallback.
 */
function safeShowModal(modalEl, opts = {}) {
  if (!modalEl) return null;
  // append to body if not present
  if (!document.body.contains(modalEl)) document.body.appendChild(modalEl);

  // ensure ARIA/tab attributes so bootstrap internals don't choke
  try {
    ...
    if (!modalEl.hasAttribute('aria-hidden')) modalEl.setAttribute('aria-hidden', 'true');
  } catch (e) { /* ignore */ }

  // Try Bootstrap native modal first (safe)
  try {
    if (typeof bootstrap !== 'undefined' && typeof bootstrap.Modal === 'function') {
      let inst = null;
      try { inst = bootstrap.Modal.getInstance(modalEl); } catch(e) { inst = null; }
      if (!inst) inst = new bootstrap.Modal(modalEl, Object.assign({ backdrop: 'static' }, opts));
      inst.show();
      return inst;
    }
  } catch (e) {
    console.warn('bootstrap.Modal.show failed, falling back to manual display', e);
  }

  // Fallback: simple visible modal (non-Bootstrap)
  try {
    ...
    if (!modalEl._outsideHandler) {
      modalEl._outsideHandler = function(e) { if (e.target === modalEl) safeHideModal(modalEl); };
      modalEl.addEventListener('click', modalEl._outsideHandler);
    }
  } catch (e) { console.warn('fallback show failed', e); }
  return null;
}

/**
 * safeHideModal(modalEl)
 * - Hides modal using bootstrap API if present, otherwise hides by style.
 */
function safeHideModal(modalEl) {
  ...
  try {
    ...
    if (modalEl._outsideHandler) { modalEl.removeEventListener('click', modalEl._outsideHandler); modalEl._outsideHandler = null; }
  } catch (e) { /* ignore */ }
}

/**
 * attachGlobalModalCloseHandlers()
 * - Delegates clicks on [data-bs-dismiss="modal"] and .btn-close to close the closest .modal
 * - Adds Escape key handler to close any open modals
 */
function attachGlobalModalCloseHandlers() {
  // avoid re-binding
  if (attachGlobalModalCloseHandlers._bound) return;
  attachGlobalModalCloseHandlers._bound = true;

  document.addEventListener('click', (ev) => {
    const btn = ev.target.closest && ev.target.closest('[data-bs-dismiss="modal"], .btn-close');
    if (!btn) return;
    const modal = btn.closest && btn.closest('.modal');
    if (modal) {
      safeHideModal(modal);
    } else {
      // if button outside modal (rare), try to find a modal by data-target or data-bs-target
      const target = btn.getAttribute && (btn.getAttribute('data-bs-target') || btn.getAttribute('data-target'));
      if (target) {
        const sel = target.startsWith('#') ? target : '#' + target.replace(/^#/, '');
        const m = document.querySelector(sel);
        if (m) safeHideModal(m);
      }
    }
  });
  ...
  window.addEventListener('keydown', (ev) => {
    if (ev.key === 'Escape' || ev.key === 'Esc') {
      ...
      modals.forEach(m => {
        const visible = (m.classList.contains('show') || getComputedStyle(m).display === 'block' || !m.classList.contains('d-none'));
        if (visible) safeHideModal(m);
      });
    }
  });
}
  ...
  function openJobDetailModal(job) {
    ...
    applyBtn.onclick = () => { try { safeHideModal(modal); } catch(e){}; openApplyModal(job.id || job.pk); };
    safeShowModal(modal);
    attachGlobalModalCloseHandlers();
  }
  ...
  function closeQuizModal() { try { stopQuizTimer(); const modal = document.getElementById('quizModal'); if (modal) safeHideModal(modal); } catch (e) {} }
  ...
  async function submitQuizAttempt(auto = false) {
    ...
    try {
      ...
      const res = await fetchWithAuthFallback(API.QUIZ_ATTEMPT_SUBMIT, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      let body = null;
  ...
  async function loadInvites() {
    const wrap = document.getElementById('invitesList'); if (!wrap) return;
    ...
    arr.forEach(inv => {
      ...
      div.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center">
        ...
        <div style="text-align:right">
          ...
          <div style="margin-top:6px">
            ...
            ${status==='accepted' && (interview.id||inv.interview_id) ? `<button class="btn btn-sm btn-primary start-invite" data-interview="${interview.id||inv.interview_id}" data-invite="${id}">Start</button>` : ''}
      ...
      div.querySelectorAll('.accept-invite').forEach(b => b.addEventListener('click', async () => {
        ...
        try {
          const token = localStorage.getItem('token');
          const headers = token ? { 'Content-Type':'application/json', 'Authorization': 'Bearer ' + token } : { 'Content-Type':'application/json', 'X-CSRFToken': getCookie('csrftoken') };
          const r = await fetchWithAuthFallback(API.INVITE_RESPOND(id), {
            method: 'POST',
            headers,
            body: JSON.stringify({ response: 'accept' })
          });
          const parsed = await (async()=>{ try { return await (r.ok ? r.json() : (await r.json().catch(()=>null))); } catch(e){ return null } })().catch(()=>null);
          if (r.ok) { showToast('Accepted', 'success'); loadInvites(); }
          else { showToast('Accept failed','error'); b.disabled=false; }
        } catch (e) {
          console.error('accept error', e); showToast('Accept failed','error'); b.disabled=false;
        }
      }));
      ...
      div.querySelectorAll('.decline-invite').forEach(b => b.addEventListener('click', async () => {
        ...
        try {
          ...
          if (r.ok) { showToast('Declined', 'success'); loadInvites(); }
          else { showToast('Decline failed','error'); b.disabled=false; }
        } catch (e) {
          console.error('decline error', e); showToast('Decline failed','error'); b.disabled=false;
        }
      }));
      ...
      div.querySelectorAll('.start-invite').forEach(b => b.addEventListener('click', async () => {
        ...
        try {
          const token = localStorage.getItem('token');
          const headers = token ? { 'Content-Type':'application/json', 'Authorization': 'Bearer ' + token } : { 'Content-Type':'application/json', 'X-CSRFToken': getCookie('csrftoken') };
          const r = await fetchWithAuthFallback(API.START_INTERVIEW(iid), { method: 'POST', headers, body: JSON.stringify({ invite: inviteId }) });
          if (r.ok) {
            const data = await (async ()=>{ try { return await r.json(); } catch(e){ return {} } })();
            if (data.redirect_url || data.join_url || data.url) window.location.href = data.redirect_url || data.join_url || data.url;
            else window.location.href = `${INTERVIEWS_BASE}/page/candidate/${iid}/?invite=${inviteId}`;
            return;
          } else {
            // server rejected -> display message
            let body = r.data || {};
            const detail = body?.detail || body?.message || `Status ${r.status}`;
            showToast('Cannot start: ' + detail, 'error', 6000);
      ...
      div.querySelectorAll('.view-invite').forEach(b => b.addEventListener('click', () => viewInvite(id)));
    });

    // if invites modal exists in page, populate the modal body too (optional)
    ...
    if (invitesModal) {
      ...
      if (body) body.innerHTML = wrap.innerHTML;
      safeShowModal(invitesModal);
      attachGlobalModalCloseHandlers();
    }
  }
  ...
  async function viewInvite(inviteId) {
    ...
    if (!modal) {
      ...
      modal.className = 'modal fade';
      modal.innerHTML = `<div class="modal-dialog modal-lg"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Invite details</h5><button class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><div id="inviteDetailContent">Loading...</div></div><div class="modal-footer"><button class="btn btn-secondary" data-bs-dismiss="modal">Close</button></div></div></div>`;
      document.body.appendChild(modal);
    }
    ...
    try {
      ...
      if (!listResp.ok) {
        content.innerHTML = `<div class="text-danger">Failed to load invite (status ${listResp.status})</div>`;
        safeShowModal(modal);
        attachGlobalModalCloseHandlers();
        return;
      }
      ...
      if (!inv) { content.innerHTML = `<div class="text-muted">Invite not found</div>`; safeShowModal(modal); attachGlobalModalCloseHandlers(); return; }
      ...
      content.innerHTML = `
        ...
        <div style="margin-top:6px"><strong>Status:</strong> ${escapeHtml(inv.status || 'pending')}</div>
        <div style="margin-top:10px;text-align:right">
          ${inv.status === 'pending' ? `<button id="inviteAcceptBtn" class="btn btn-success btn-sm">Accept</button>
            <button id="inviteDeclineBtn" class="btn btn-outline-danger btn-sm">Decline</button>` : ''}
          ${ (inv.status === 'accepted' || inv.status === 'pending') && (interview.id || inv.interview_id) ? `<button id="inviteStartBtn" class="btn btn-primary btn-sm">Start</button>` : ''}
        </div>`;
      // attach handlers
      modal.querySelector('#inviteAcceptBtn')?.addEventListener('click', () => { respondInvite(inviteId, 'accept'); safeHideModal(modal); });
      modal.querySelector('#inviteDeclineBtn')?.addEventListener('click', () => { respondInvite(inviteId, 'decline'); safeHideModal(modal); });
  ...
  async function respondInvite(inviteId, action) {
    ...
    try {
      const token = localStorage.getItem('token');
      const headers = token ? { 'Content-Type':'application/json', 'Authorization': 'Bearer ' + token } : { 'Content-Type':'application/json', 'X-CSRFToken': getCookie('csrftoken') };
      const r = await fetchWithAuthFallback(API.INVITE_RESPOND(inviteId), {
        method: 'POST',
        headers,
      ...
      if (typeof loadInvites === 'function') loadInvites();
    } catch (e) {
  ...
  async function startInterview(interviewId, inviteId = null) {
    if (!interviewId) return showToast('Interview id missing', 'error');
    try {
      const token = localStorage.getItem('token');
      const headers = token ? { 'Content-Type':'application/json', 'Authorization': 'Bearer ' + token } : { 'Content-Type':'application/json', 'X-CSRFToken': getCookie('csrftoken') };
      const r = await fetchWithAuthFallback(API.START_INTERVIEW(interviewId), {
        method: 'POST',
        headers,
        body: JSON.stringify({ invite: inviteId })
      });
      if (!r.ok) {
        let msg = r.data?.detail || r.data?.message || `Status ${r.status}`;
        if (r.data && r.data.scheduled_start) msg += ` — Starts at ${new Date(r.data.scheduled_start).toLocaleString()}`;
        showToast('Cannot start: ' + msg, 'error', 7000);
        return;
      }
      const data = r.data || {};
      if (data.redirect_url || data.join_url || data.url) {
        window.location.href = data.redirect_url || data.join_url || data.url;
        return;
      }
      const attemptParam = data.attempt_id ? `?attempt=${data.attempt_id}` : (inviteId ? `?invite=${inviteId}` : '');
      window.location.href = `${INTERVIEWS_BASE}/page/candidate/${interviewId}/${attemptParam}`;
    } catch (err) {
      console.error('startInterview error', err);
      window.location.href = `${INTERVIEWS_BASE}/page/candidate/${interviewId}/` + (inviteId ? `?invite=${inviteId}` : '');
    }
  }
  ...
  window.attachGlobalModalCloseHandlers = attachGlobalModalCloseHandlers;

  // ---------- Auto-auth init & wiring ----------
  ...
  function init() {
    ...
    // apply modal wiring (ensure element exists)
    ...
    // invites modal auto-create if missing (ensure consistent structure)
    if (!document.getElementById('invitesModal')) {
      const m = document.createElement('div'); m.id='invitesModal'; m.className='modal fade';
      m.innerHTML = `<div class="modal-dialog modal-lg modal-dialog-scrollable"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Invites</h5><button class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body p-3"><div id="invitesListModal"><div class="text-muted small">Loading invites...</div></div></div><div class="modal-footer"><button id="invitesModalRefresh" type="button" class="btn btn-primary">Refresh</button><button id="invitesModalClose" type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button></div></div></div>`;
    ...
    document.getElementById('viewInvitesBtn')?.addEventListener('click', () => {
      const invitesModal = document.getElementById('invitesModal');
      if (invitesModal) {
        loadInvites().then(()=>{ safeShowModal(invitesModal); attachGlobalModalCloseHandlers(); });
      } else {
        // fallback: open inline invites section
        const section = document.getElementById('invitesSection');
        if (section) { section.style.display = 'block'; loadInvites(); }
      }
    });
    ...
    document.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          try { const m = document.getElementById('applyModal'); if (m && m.classList.contains('modal')) safeHideModal(m); else m.style.display='none'; } catch(e){}
          loadMyApplications();
        } else {
    ...
    // hide apply modal click outside (fallback)
    document.addEventListener('click', (e) => {
      const am = document.getElementById('applyModal');
      if (!am) return;
      if (am.style.display === 'block' && !am.contains(e.target)) am.style.display = 'none';
    });
  ...
  window.openApplyModal = function (jobId) {
    ...
    try { if (modal && modal.classList.contains('modal')) safeShowModal(modal); else modal.style.display='block'; } catch (e) { modal.style.display = 'block'; }
    attachGlobalModalCloseHandlers();
  };
