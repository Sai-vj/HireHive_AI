...
// Drop this file in place of your old recruiter_dashboard.js
// Requires the DOM elements used by your template (jobsList, jobDetails, invite modal elements, token input).

(function(){
  const API_BASE = '/api'; // base prefix used by your project
  const interviewsBase = `${API_BASE}/interviews`;
  const resumesBase = `${API_BASE}/resumes`;
  ...
  function qsEscape(s){ return String(s||''); }
  ...
  const inviteScheduledInput = document.getElementById('invite_scheduled_at');
  ...
  const interviewsContainer = document.getElementById('interviews-container') || document.createElement('div');

  console.log('recruiter dashboard initialized');

  // token save button (if your template has save button)
  ...
  if(saveTokenBtn && tokenInput){
    saveTokenBtn.addEventListener('click', () => {
      ...
      tokenStatus && (tokenStatus.innerText = 'Token saved');
      console.log('saved token');
      loadJobs(); // reload with new token
    });
  }

  // invite modal show/hide
  window.showInviteModal = function(interviewId, jobId, candidateId, jobTitle){
    window.__invite_interview_id = interviewId;
    window.__invite_job_id = jobId;
    inviteModalTitle.innerText = `Invite candidate for: ${jobTitle || interviewId}`;
    inviteCandidateIdInput.value = candidateId || '';
    inviteScheduledInput.value = '';
    inviteMessageInput.value = '';
    inviteModal.classList.remove('d-none');
  };
  function hideInviteModal(){ inviteModal.classList.add('d-none'); }

  inviteCancelBtn && inviteCancelBtn.addEventListener('click', hideInviteModal);

  // Send invite POST
  inviteSendBtn && inviteSendBtn.addEventListener('click', async () => {
    const interviewId = window.__invite_interview_id;
    if(!interviewId){ alert('No interview selected'); return; }
    const candidate_id = inviteCandidateIdInput.value.trim();
    const scheduled_at = inviteScheduledInput.value || null;
    const message = inviteMessageInput.value || '';
    if(!candidate_id){ alert('Enter candidate id'); return; }

    const url = `${interviewsBase}/recruiter/${interviewId}/invite/`;
    try {
  ...
  // render job card
  function renderJobCard(job, interviewsForJob=[]) {
    // assume job.id and job.title exist
    const scheduled = interviewsForJob.length ? interviewsForJob[0].scheduled_at || 'Not set' : 'No interview';
    // Show "Show Applications" button will use job.id
    return `
      <div class="job-card" data-job-id="${job.id}">
        <div class="d-flex justify-content-between">
          <div>
            <h6 class="job-title mb-0">${escapeHtml(job.title||'Untitled')}</h6>
            <div class="small-muted">Mode: ${escapeHtml(job.mode || 'N/A')} • Status: ${job.is_active? 'Active':'Inactive'}</div>
            <div class="small-muted">Scheduled: ${scheduled ? new Date(scheduled).toLocaleString() : 'Not set'}</div>
          </div>
          <div class="text-end job-actions">
            ...
            <button class="btn btn-sm btn-outline-info show-apps-btn" data-job-id="${job.id}">Show Applications</button>
          </div>
        </div>
      </div>
    `;
  ...
  // load jobs (from resumes app) and interviews for them (from interviews app)
  async function loadJobs(){
    if(!jobsListEl) return;
    jobsListEl.innerHTML = '<div class="small-muted">Loading jobs…</div>';
    try {
      const token = localStorage.getItem(tokenKey) || '';
      const res = await fetch(`${resumesBase}/recruiter/jobs/`, { headers: token ? { 'Authorization': `Bearer ${token}` } : {} });
      if(!res.ok){
        console.error('loadJobs failed', res.status);
        jobsListEl.innerHTML = `<div class="text-danger">Failed to load jobs (${res.status})</div>`;
        return;
      }
      const jobs = await res.json();
      if(!jobs || !jobs.length){ jobsListEl.innerHTML = '<div class="small-muted">No jobs found</div>'; return; }

      // optionally fetch interviews for each job to display scheduled date. We'll fetch interviews list and map by job id.
      const ivRes = await fetch(`${interviewsBase}/recruiter/`, { headers: headersWithToken() });
      let interviews = [];
      if(ivRes.ok){ interviews = await ivRes.json(); } // may be empty or 401 if token expired
      const byJob = {};
      interviews.forEach(iv => {
        if(!iv.job) return;
        const jid = iv.job.id || iv.job;
        if(!byJob[jid]) byJob[jid] = [];
        byJob[jid].push(iv);
      });

      // render
      jobsListEl.innerHTML = jobs.map(j => renderJobCard(j, byJob[j.id] || [])).join('');
      attachJobHandlers();
    } catch(err){
      console.error(err);
      jobsListEl.innerHTML = '<div class="text-danger">Network error loading jobs</div>';
    }
  }

  // attach handlers for dynamic buttons inside job cards
  function attachJobHandlers(){
    // View button (if you want to navigate)
    document.querySelectorAll('.view-job-btn').forEach(btn => {
      btn.onclick = (e) => {
        const jobId = btn.dataset.jobId;
        // navigate to job detail page or open modal
        // For now show job details area with job data fetched
        showJobDetails(jobId);
      };
    });

    // Show Applications
    document.querySelectorAll('.show-apps-btn').forEach(btn => {
      btn.onclick = (e) => {
        const jobId = btn.dataset.jobId;
    ...
    // Generate quiz (leave as stub)
    document.querySelectorAll('.generate-quiz-btn').forEach(btn => {
      btn.onclick = (e) => {
        const jobId = btn.dataset.jobId;
        alert('Generate quiz for job ' + jobId + ' (not implemented here)');
      };
    });
  }

  // Show job details (simple)
  async function showJobDetails(jobId){
    jobDetailsEl.style.display = '';
    noJobEl.style.display = 'none';
    jobDetailsEl.innerHTML = '<div class="small-muted">Loading job details…</div>';
    try {
      const res = await fetch(`${resumesBase}/jobs/${jobId}/`, { headers: headersWithToken() });
      if(!res.ok){ jobDetailsEl.innerHTML = `<div class="text-danger">Failed to load job (${res.status})</div>`; return; }
      const job = await res.json();

      jobDetailsEl.innerHTML = `
        <div class="d-flex justify-content-between mb-2">
          <div>
            <h4>${escapeHtml(job.title || 'Job')}</h4>
            <div class="small-muted">${escapeHtml(job.company || '')} • ${escapeHtml(job.skills || '')}</div>
          </div>
          <div>
            <button class="btn btn-sm btn-outline-primary" id="jobShowAppsBtn">Show Applications</button>
          </div>
        </div>
        <hr />
        <div id="applicationsArea"><div class="small-muted">Applications will appear here</div></div>
      `;
      document.getElementById('jobShowAppsBtn').addEventListener('click', () => showApplicationsForJob(jobId));
    } catch(err){
      console.error(err);
      jobDetailsEl.innerHTML = '<div class="text-danger">Network error</div>';
    }
  }

  // Show applications for a job
  async function showApplicationsForJob(jobId){
    const appsArea = document.getElementById('applicationsArea') || (function(){
      const d = document.createElement('div'); d.id='applicationsArea'; jobDetailsEl.appendChild(d); return d;
    })();
    appsArea.innerHTML = '<div class="small-muted">Loading applications…</div>';

    // Use the endpoint you already used: /api/resumes/recruiter/job/<job_id>/applications/
    const url = `${resumesBase}/recruiter/job/${jobId}/applications/`;
    try {
      const res = await fetch(url, { headers: headersWithToken() });
      if(!res.ok){
        console.error('showApplications failed', res.status);
        let text = await res.text().catch(()=>'');
        appsArea.innerHTML = `<div class="text-danger">Failed to load applications (${res.status})</div>`;
        return;
      }
      const apps = await res.json();
      if(!apps || !apps.length){ appsArea.innerHTML = '<div class="small-muted">No applications yet</div>'; return; }

      // apps items keys: candidate_id, candidate_name, applied_at, resume_id, status, message, id
      appsArea.innerHTML = apps.map(a => {
        const candName = a.candidate_name || a.candidate || `user ${a.candidate_id||a.id}`;
        return `
          <div class="card mb-2 p-2">
            <div class="d-flex justify-content-between">
              <div>
                <div><strong>${escapeHtml(candName)}</strong></div>
                <div class="small-muted">Applied: ${a.applied_at ? new Date(a.applied_at).toLocaleString() : ''}</div>
                <div class="small-muted">Status: ${escapeHtml(a.status||'')}</div>
              </div>
              <div class="text-end">
                <button class="btn btn-sm btn-outline-primary invite-app-btn" data-job-id="${jobId}" data-candidate-id="${a.candidate_id || a.candidate || ''}" data-app-id="${a.id || ''}" data-candidate-name="${escapeHtml(candName)}">Send Invite</button>
              </div>
            </div>
          </div>
        `;
      ...
      // inside showApplicationsForJob(), after appsArea.innerHTML is set and buttons rendered:
appsArea.querySelectorAll('.invite-app-btn').forEach(btn => {
  btn.onclick = async () => {
    const jobId = btn.dataset.jobId;
    const candidateId = btn.dataset.candidateId;
    const candidateName = btn.dataset.candidateName || '';

    // ensure interviews cache exists
    if(_cachedInterviews === null) {
      _cachedInterviews = await fetchInterviewsList();
    }

    // find interviews for this job (possibly job stored as object or id)
    const interviewsForJob = (_cachedInterviews || []).filter(iv => {
      if(!iv.job) return false;
      // iv.job might be object {id:...} or numeric
      const jid = (typeof iv.job === 'object' && iv.job !== null) ? iv.job.id : iv.job;
      return String(jid) === String(jobId);
    });

    if(interviewsForJob.length === 1){
      // single interview -> open modal directly with that interview id
      const interviewId = interviewsForJob[0].id;
      window.showInviteModal(interviewId, jobId, candidateId, candidateName);
      return;
    }

    if(interviewsForJob.length > 1){
      // many interviews -> show a small chooser modal first
      // create chooser UI in the invite modal before showing
      inviteModalTitle.innerText = `Pick interview for: ${candidateName || jobId}`;
      // build select options
      const selHtml = `
        <div class="mb-2">
          <label>Select Interview</label>
          <select id="__invite_interview_select" class="form-control">
            ${interviewsForJob.map(iv => `<option value="${iv.id}">${iv.title || ('Interview ' + iv.id)} - ${iv.scheduled_at ? new Date(iv.scheduled_at).toLocaleString() : 'unscheduled'}</option>`).join('')}
          </select>
        </div>
        <div class="mb-2">
          <label>Candidate ID</label><input id="invite_candidate_id" class="form-control" value="${candidateId}">
        </div>
        <div class="mb-2">
          <label>Scheduled at</label><input id="invite_scheduled_at" type="datetime-local" class="form-control">
        </div>
        <div class="mb-2">
          <label>Message</label><textarea id="invite_message" class="form-control"></textarea>
        </div>
      `;
      // insert into modal body (we assume inviteModal has card layout); safe replace:
      const card = inviteModal.querySelector('.card') || inviteModal;
      // put chooser content then show modal
      card.querySelector('#inviteModalTitle')?.remove?.(); // optional cleanup if title duplicate
      card.innerHTML = `<h5 id="inviteModalTitle"></h5>${selHtml}<div class="text-end"><button id="inviteCancelBtn" class="btn btn-outline-secondary btn-sm">Cancel</button> <button id="inviteSendBtn" class="btn btn-primary btn-sm">Send Invite</button></div>`;
      document.getElementById('inviteModalTitle').innerText = `Invite ${candidateName || ''}`;
      ...
      document.getElementById('inviteSendBtn').addEventListener('click', async () => {
        const sel = document.getElementById('__invite_interview_select');
        const chosenInterviewId = sel.value;
        const c_id = document.getElementById('invite_candidate_id').value.trim();
        ...
        // POST invite
        try {
          const res = await fetch(`${interviewsBase}/recruiter/${chosenInterviewId}/invite/`, {
            method: 'POST',
            headers: headersWithToken(),
            body: JSON.stringify({ candidate_id: c_id, scheduled_at, message })
          });
          const data = await res.json();
          if(res.ok){ hideInviteModal(); alert('Invite sent'); }
          else { alert('Invite failed: ' + (data.detail || JSON.stringify(data))); }
    ...
    // no interviews exist for job -> tell recruiter to create one
    // show a modal with a helpful message and a link to create
    inviteModalTitle.innerText = `No interview for job ${jobId}`;
    const card = inviteModal.querySelector('.card') || inviteModal;
    card.innerHTML = `<h5 id="inviteModalTitle">No interview</h5>
      <div class="mb-2">No interview exists for this job yet. Create one first, or go to Interviews → Create Interview.</div>
      <div class="text-end">
        <a class="btn btn-outline-primary btn-sm" href="/api/interviews/recruiter/" target="_blank">Open Interviews</a>
        <button id="inviteCancelBtn" class="btn btn-outline-secondary btn-sm">Close</button>
      </div>`;
    document.getElementById('inviteCancelBtn').addEventListener('click', hideInviteModal);
    inviteModal.classList.remove('d-none');
  };
      ...
      appsArea.querySelectorAll('.invite-app-btn').forEach(btn => {
        btn.onclick = () => {
          const interviewForJob = findInterviewForJob(jobId);
          if(!interviewForJob){
            alert('No interview created for this job yet. Create an interview first or pick an interview.');
            return;
          }
          const interviewId = interviewForJob.id;
          const candidateId = btn.dataset.candidateId;
          const candidateName = btn.dataset.candidateName;
          // open invite modal pre-filled
          window.showInviteModal(interviewId, jobId, candidateId, candidateName);
        };
      });
      ...
      console.error(err);
      appsArea.innerHTML = '<div class="text-danger">Network error</div>';
    }
  }

  // find interview for a job by fetching interviews list cache or hitting API
  async function fetchInterviewsList(){
    try {
      const res = await fetch(`${interviewsBase}/recruiter/`, { headers: headersWithToken() });
      if(!res.ok) return [];
      return await res.json();
    } catch { return []; }
  }
  let _cachedInterviews = null;
  async function ensureInterviewsCache(){
    if(_cachedInterviews === null) _cachedInterviews = await fetchInterviewsList();
    return _cachedInterviews;
  }
  function findInterviewForJob(jobId){
    if(!_cachedInterviews) return null;
    // choose first interview for job
    return _cachedInterviews.find(iv => (iv.job && (iv.job.id==jobId || iv.job==jobId)));
  }

  // initial load
  (async ()=>{
    await loadJobs();
    // priming interviews cache
    _cachedInterviews = await fetchInterviewsList();
    setInterval(async ()=>{ _cachedInterviews = await fetchInterviewsList(); }, 60000);
  })();
