...
// Robust Invite Modal Controller (use instead of the old one)
(function(){
  // DOM refs
  ...
  if (!inviteModal) {
    console.warn('inviteModal element not found in DOM. Add <div id="inviteModal">...</div> to your template.');
    return;
  }

  // state
  let inviteContext = { jobId: null, candidateId: null, candidateName: null };

  // utility: ensure inputs exist before using
  function setInputSafe(inputEl, value){
    try { if(inputEl) inputEl.value = value ?? ''; } catch(e){ console.warn('setInputSafe failed', e); }
  }

  // show/hide
  function showInviteModal({ jobId = null, candidateId = '', candidateName = '' } = {}){
    inviteContext.jobId = jobId;
    inviteContext.candidateId = candidateId;
    inviteContext.candidateName = candidateName;

    setInputSafe(inviteCandidateIdInput, candidateId ? String(candidateId) : '');
    setInputSafe(inviteScheduledAtInput, '');
    setInputSafe(inviteMessageInput, `Hi ${candidateName || ''}, you are invited for interview.`);

    // make visible
    inviteModal.classList.remove('d-none');
    inviteModal.style.display = 'flex';
    // ensure modal children can be focused / typed into
    inviteModal.style.alignItems = 'center';
    document.body.style.overflow = 'hidden';

    // focus candidate id input if present
    try { inviteCandidateIdInput && inviteCandidateIdInput.focus(); } catch(e){/* ignore */ }
  }
  ...
  function hideInviteModal(){
    ...
    // clear context if desired
    // inviteContext = { jobId: null, candidateId: null, candidateName: null };
  }

  // send invite (uses inviteContext.jobId by default)
  async function sendInvite(passedJobId){
    // determine jobId: prefer explicit arg, fallback to modal context
    const jobId = passedJobId || inviteContext.jobId;
    if(!jobId){
      alert('No interview/job selected for this invite. Please open the interviews page or create/select an interview for this job first.');
      return;
    }

    const candidate_id = (inviteCandidateIdInput && inviteCandidateIdInput.value || '').trim();
    const scheduled_at = (inviteScheduledAtInput && inviteScheduledAtInput.value) || '';
    const message = (inviteMessageInput && inviteMessageInput.value || '').trim();

    if (!candidate_id) { alert('Enter candidate id'); return; }

    const token = localStorage.getItem('token') || '';
    const url = `/api/interviews/recruiter/${jobId}/invite/`;
    ...
    try {
      ...
      if (res.ok) {
        // success
        alert('Invite sent ✅');
        // optional: update UI badge for this candidate row if present
        // e.g. find .application-row[data-candidate-id="..."] and update text
        try {
          const appRow = document.querySelector(`.application-row[data-candidate-id="${candidate_id}"]`);
          if(appRow){
            const badge = appRow.querySelector('.invite-badge');
            if(badge) badge.textContent = (data.status || 'pending');
          }
        } catch(e){ /* ignore */ }
        ...
        const details = data.detail || data.error || JSON.stringify(data);
        alert('Invite failed ❌: ' + details);
      }
    } catch (err) {
  ...
  // click outside to close (only when clicking backdrop)
  ...
  // wire buttons safely: don't pass event object to sendInvite
  if (inviteCancelBtn) inviteCancelBtn.addEventListener('click', hideInviteModal);
  if (inviteSendBtn) inviteSendBtn.addEventListener('click', function(e){ e.preventDefault(); sendInvite(); });

  // expose global helper so other code can call:
  // window.showInviteModal(jobId, candidateId, candidateName)
  window.showInviteModal = function(jobId, candidateId = '', candidateName = ''){
    // support both object and arg-style call
  ...
  console.log('Invite modal controller initialized.');
})();

// small create-interview controller (drop in same file or separate)
(function(){
  const modal = document.getElementById('createInterviewModal');
  const titleIn = document.getElementById('ci_title');
  const schedIn = document.getElementById('ci_scheduled_at');
  const durIn = document.getElementById('ci_duration');
  const modeIn = document.getElementById('ci_mode');
  const descIn = document.getElementById('ci_description');
  const btnCreate = document.getElementById('ci_create');
  const btnCancel = document.getElementById('ci_cancel');

  let currentJobId = null;

  function showCreateInterview(jobId, defaults = {}){
    currentJobId = jobId;
    if(defaults.title) titleIn.value = defaults.title;
    else titleIn.value = `Interview for job ${jobId}`;
    schedIn.value = defaults.scheduled_at || '';
    durIn.value = defaults.duration_minutes || 45;
    modeIn.value = defaults.mode || 'online';
    descIn.value = defaults.description || '';
    modal.classList.remove('d-none'); modal.style.display = 'flex'; document.body.style.overflow='hidden';
    titleIn.focus();
  }
  function hide(){ modal.classList.add('d-none'); modal.style.display='none'; document.body.style.overflow=''; }

  async function createInterview(){
    if(!currentJobId) { alert('No job selected'); return; }
    const payload = {
      title: titleIn.value.trim(),
      scheduled_at: schedIn.value || null,
      duration_minutes: parseInt(durIn.value || 45, 10),
      mode: modeIn.value,
      description: descIn.value.trim(),
    };
    const token = localStorage.getItem('token') || '';
    const url = `/api/interviews/recruiter/job/${currentJobId}/create/`;
    try{
      const res = await fetch(url, {
        method:'POST',
      ...
      if(res.ok){
        ...
        hide();
        // optionally call a function to reload interviews for this job / update UI
        if(window.loadInterviewsForJob) window.loadInterviewsForJob(currentJobId);
      } else {
        alert('Create failed: ' + (data.detail || JSON.stringify(data)));
      }
    }catch(e){
  ...
  if(btnCreate) btnCreate.addEventListener('click', createInterview);
  if(btnCancel) btnCancel.addEventListener('click', hide);
  modal && modal.addEventListener('click', (e)=>{ if(e.target===modal) hide(); });

  window.showCreateInterview = showCreateInterview; // call from job UI: showCreateInterview(jobId)
})();
