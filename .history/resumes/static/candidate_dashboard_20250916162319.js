...
function openQuizModal(jobId, questionPayload) {
  ...
  (async () => {
    try {
      ...
      // create/update summary element
      let attemptSummaryEl = modal.querySelector('#attempt-summary');
      if (!attemptSummaryEl) {
        attemptSummaryEl = document.createElement('div');
      ...
      if (attempts >= QUIZ_MAX_ATTEMPTS) {
        // hide/disable questions and submit
        renderQuestionsInModal([]); // clears questions area
...
async function fetchAttemptCount(jobId) {
  ...
  for (const u of tries) {
    try {
      ...
      // auth problems
      if (status === 401) return { error:'unauthorized', status:401, raw: data || text };
      if (status === 403) {
        let detail = data && typeof data === 'object' ? (data.detail || data.message) : text;
        return { error:'forbidden', status:403, detail: detail || 'Forbidden' };
      }
      ...
      console.warn('fetchAttemptCount try error', e);
      continue;
    }
  }
  return 0;
...
async function onQuizSubmit(jobId, answers) {
  try {
    ...
    // handle max attempts returned by backend
    if (r.status === 403) {
      let msg = "Max attempts reached — you cannot take this quiz again.";
      try { const parsed = JSON.parse(text); if (parsed.detail) msg = parsed.detail; } catch(e) {}
    ...
    // success: update UI via handler if provided
    if (typeof handleQuizSubmitResponse === 'function') handleQuizSubmitResponse(jobId, data);

    // stop timer
    if (window.__quizTimerHandle) { clearInterval(window.__quizTimerHandle); window.__quizTimerHandle = null; }
    // update attempts summary immediately
    const attemptsSummaryEl = document.querySelector('#attempt-summary');
    if (attemptsSummaryEl) {
      try {
        const count = await fetchAttemptCount(jobId);
        if (typeof count === 'number') attemptsSummaryEl.textContent = `Attempts: ${count} / 3`;
        else if (count && count.status === 403) attemptsSummaryEl.textContent = `Attempts: 3 / 3`;
      } catch (e) { console.warn("Couldn't refresh attempts", e); }
    }
