  ...
  arr.forEach(inv => {
    ...
    card.querySelectorAll('.decline-invite').forEach(b => b.addEventListener('click', async () => {
      ...
      if (r && r.ok) { showToast('Declined', 'success'); await loadInvites(); } else { showToast('Decline failed', 'error'); b.disabled = false; }
        ...
        // --- START: START-BUTTON HTML & wiring (replace existing startBtnHtml + handler) ---
    // decide start button appearance: green but visually "muted" when disabled
    const startBtnHtml = (interview.id || inv.interview_id)
      ? `<button class="btn btn-sm btn-success start-invite" data-interview="${escapeHtml(interview.id||inv.interview_id)}" data-invite="${escapeHtml(id)}" ${canStart ? '' : 'disabled'} style="${canStart ? '' : 'opacity:0.65'}">Start</button>`
      : '';

    // append card (already done above) then wire handlers for start-invite buttons on this card:
