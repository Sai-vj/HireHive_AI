...
document.addEventListener('click', function(e){
  ...
  if (!btn) return;
  // optional: prevent default if it's inside form/link
  e.preventDefault && e.preventDefault();

  const jid = Number(btn.dataset.jobId || btn.getAttribute('data-job-id'));
  console.log('DEBUG view-attempts clicked jobId=', jid);
 ...
 // in handleQuizSubmitResponse or after render
const jobCard = document.querySelector(`.job-card[data-job-id="${jobId}"]`);
if (jobCard) {
  const applyBtn = jobCard.querySelector('.apply-btn') || jobCard.querySelector(`#apply-btn-${jobId}`);
  if (applyBtn) {
    if (data.passed) { applyBtn.disabled = false; applyBtn.classList.remove('disabled'); }
    else { /* keep disabled or leave as-is */ }
  }
}
}

window.handleQuizSubmitResponse = handleQuizSubmitResponse;
// after successful submit response and parsing `data`
...
// Open attempt history modal for a job
...
(function(){
  ...
  // initial attach on DOM ready + expose function
  if (document.readyState === 'loading') {
...
async function loadInvites() {
  ...
  invites.forEach(inv => {
    ...
    div.innerHTML = `
      ...
      <div class="invite-details mt-2" style="display:${status==='accepted'?'block':'none'}">
        ...
        <div class="small-muted">Mode: ${escapeHtml(interview.mode || '—')} • Duration: ${escapeHtml(String(interview.duration_minutes || '—'))} mins</div>
      </div>
    `;
  ...
  listEl.querySelectorAll('.start-interview-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      // go to dedicated interview page
...
async function startInterview(interviewId) {
  ...
  if (!res.ok) return showToast('Failed to start interview', 'error');
  const data = res.data || {};
  // Show in modal
  const bodyEl = document.getElementById('interviewModalBody');
  bodyEl.innerHTML = `<pre>${JSON.stringify(data, null, 2)}</pre>`;
  const modalEl = new bootstrap.Modal(document.getElementById('interviewModal'));
  modalEl.show();
}
