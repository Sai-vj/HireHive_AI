...
// auto-auth.js — paste into candidate_dashboard.js or load after it
(async function() {
  // helper to check server auth using cookies
  async function checkAuth() {
    try {
      // this call will include cookies by default for same-origin
      const r = await fetch('/api/resumes/my-resumes/', { credentials: 'same-origin' });
      return r.ok;
    } catch (e) {
      return false;
    }
  ...
  // update UI: hide token input if cookie-auth works
  async function updateTokenUI() {
    const ok = await checkAuth();
    const tokenInputRow = document.getElementById('tokenInput')?.closest('.card') || null;
    const tokenStatus = document.getElementById('tokenStatus');
    if (ok) {
      // hide the token input area or show status
      if (tokenInputRow) tokenInputRow.style.display = 'none';
      if (tokenStatus) tokenStatus.innerText = 'Authenticated via cookies (auto)';
    } else {
      if (tokenInputRow) tokenInputRow.style.display = ''; // show
      if (tokenStatus) tokenStatus.innerText = 'No cookie token — paste token or login';
    }
  }

  // wire the Save button to allow optional manual fallback (keeps existing behaviour)
  document.getElementById('saveTokenBtn')?.addEventListener('click', () => {
    const t = document.getElementById('tokenInput')?.value.trim();
    if (!t) { alert('Paste token or login'); return; }
    // store to localStorage only as a fallback (not recommended long-term)
    localStorage.setItem('access_token_fallback', t);
    document.getElementById('tokenStatus').innerText = 'Saved fallback token (localStorage).';
  });

  // wire upload / other buttons to use cookie auth automatically (example)
  document.getElementById('uploadBtn')?.addEventListener('click', async () => {
    const fileEl = document.getElementById('resumeFile');
    if (!fileEl || !fileEl.files?.length) { alert('Choose file'); return; }
    const fd = new FormData();
    fd.append('file', fileEl.files[0]);

    // try cookie-auth fetch first
    let res = await fetch('/api/resumes/upload/', {
      method: 'POST',
      body: fd,
      credentials: 'same-origin'   // critical so cookies are sent
    });

    // fallback: if cookie auth 401 and fallback token present, try using Authorization header
    if (res.status === 401) {
      const fallback = localStorage.getItem('access_token_fallback');
      if (fallback) {
    ...
    alert('Upload ok');
    // refresh resumes list - call your refresh handler
    document.getElementById('refreshResumesBtn')?.click();
  });

  // run initial UI update
  updateTokenUI();

  // optionally poll auth status every X seconds (or call after certain actions)
  setInterval(updateTokenUI, 60 * 1000); // every minute
})();
