...
async function startInterview(interviewId, inviteId) {
  ...
  try {
    ...
    if (res && res.ok) {
      // If backend sends join URL, use that
      const joinUrl = res.data?.url || res.data?.join_url || null;
      if (joinUrl) {
        window.open(joinUrl, '_blank', 'noopener,noreferrer');
        return { ok: true, url: joinUrl };
      }
      // ✅ fallback: open your @login_required frontend route
      const frontendUrl = `/interviews/frontend/start/${encodeURIComponent(interviewId)}/?invite=${encodeURIComponent(inviteId || '')}`;
