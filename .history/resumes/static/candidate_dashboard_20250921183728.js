...
(function () {
  ...
  // ---------- detect utils availability (use imported functions if present) ----------
  // Note: imported names (fetchWithAuth, apiFetch) exist in module scope (not window)
  const utilsAvailable = (typeof fetchWithAuth === 'function' && typeof apiFetch === 'function');

  // ---------- safe fetchWithAuth fallback (won't overwrite imported fetchWithAuth) ----------
  async function fetchWithAuthFallback(url, options = {}) {
    // if real helper available, use it
    if (typeof fetchWithAuth === 'function') return fetchWithAuth(url, options);

    // fallback: include Authorization header from localStorage token and session cookies
  ...
  // ---------- safe apiFetch fallback (returns normalized { ok, status, data }) ----------
  async function apiFetchFallback(url, opts = {}) {
    // if real helper available, use it
    if (typeof apiFetch === 'function' && !apiFetchFallback._usingFallback) {
      // if apiFetch is the imported one, call it
  ...
  // Make a single canonical apiFetch function reference to use throughout file:
  const _apiFetch = (typeof apiFetch === 'function' && utilsAvailable) ? apiFetch : apiFetchFallback;
  // Make canonical fetchWithAuth reference:
  const _fetchWithAuth = (typeof fetchWithAuth === 'function') ? fetchWithAuth : fetchWithAuthFallback;

  // Use _apiFetch/_fetchWithAuth in the rest of the file (replace usages of apiFetch and fetchWithAuthFallback)
...
// Lightweight aliases so other parts of code that call `safeShowModal` / `safeHideModal` still work:
function safeShowModal(el, opts = {}) {
  // prefer bootstrap modal helper, fallback to simple show
  try {
    return safeShowBootstrapModal(el, opts);
  } catch (e) {
    try { el.style.display = 'flex'; return null; } catch (err) { return null; }
  }
}
function safeHideModal(el) {
  try {
    // prefer bootstrap hide
    return safeHideBootstrapModal(el);
  } catch (e) {
    try { el.style.display = 'none'; document.body.style.overflow = ''; } catch(err){} 
  }
}

// Add a small helper that attaches click/outside/Escape close behavior to modals created by us.
function attachGlobalModalCloseHandlers() {
  // close when clicking the backdrop (for our fallback modals)
  document.querySelectorAll('.modal').forEach(modalEl => {
    ...
    modalEl.addEventListener('click', (ev) => {
      if (ev.target === modalEl) {
        try { safeHideModal(modalEl); } catch(e){ modalEl.style.display='none'; }
      }
    });

    // escape key closes fallback (only for non-bootstrap fallback; bootstrap handles its own)
...
// Expose to window so inline handlers (or other modules) can call them
...
// --- PATCH: early fallback so startInterview is always defined ---
if (!window.startInterview) {
  window.startInterview = async function (interviewId, inviteId = null) {
    console.warn('Fallback startInterview called (real impl not ready yet).', { interviewId, inviteId });
    try {
