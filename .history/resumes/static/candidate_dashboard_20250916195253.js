...
/* ---------- utility ---------- */
/* candidate_dashboard.js - updated: exposes modal helpers to window and includes quiz modal, timer, fetch/submit logic */
(function () {
  ...
  /* ---------- createQuizModal (build DOM once) ---------- */
  function createQuizModal() {
    ...
    // wire close and submit handlers (submit will be assigned later by openQuizModal)
    modal.querySelector('#quiz-modal-close').addEventListener('click', closeQuizModal);
  }

  /* ---------- Timer helpers (single registration) ---------- */
  if (!window.__quizTimerHelpersRegistered) {
    ...
    window.startQuizTimer = function (modalEl, seconds, onExpire) {
      try {
        ...
        console.warn('startQuizTimer error', e);
        return null;
      }
    };
  ...
  /* ---------- fetchAttemptsForJob & loadAttemptHistoryAndRender ---------- */
  ...
  /* ---------- helpers for job card rendering ---------- */
  function findJobCardElement(jobId, fallbackButton) {
    ...
    if (fallbackButton && fallbackButton.closest) {
      const p = fallbackButton.closest('.job-card'); if (p) return p;
    }
    return null;
  }
  ...
  function ensureAttemptContainer(jobCardEl) {
    ...
    if (!wrap) {
      wrap = document.createElement('div'); wrap.className = 'attempt-info'; wrap.style = 'margin-top:8px;font-size:.9rem;color:#444'; jobCardEl.appendChild(wrap);
    }
    return wrap;
  }
  ...
  async function fetchAttemptCount(jobId) {
    ...
    for (const u of tries) {
      try {
        ...
        if (r.status === 403) { const detail = data && data.detail ? data.detail : (typeof data === 'string' ? data : 'Forbidden'); return { error: 'forbidden', status: 403, detail }; }
  ...
  async function autoSubmitQuiz(modal) {
    try {
      ...
      if (typeof onQuizSubmit === 'function') { await onQuizSubmit(jobId, answers); return; }
      const token = localStorage.getItem('token') || '';
      const headers = { 'Content-Type': 'application/json' };
      if (token) headers['Authorization'] = 'Bearer ' + token;
      const r = await fetch(`/api/quiz/${jobId}/attempt/`, { method: 'POST', headers, body: JSON.stringify({ answers }) });
      if (r.ok) { const d = await r.json(); if (typeof handleQuizSubmitResponse === 'function') handleQuizSubmitResponse(jobId, d); }
      else { showToast('Auto-submit failed', 'error'); console.warn('autoSubmit failed', r.status); }
  ...
  async function onQuizSubmit(jobId, answers) {
    try {
      ...
      if (r.status === 403) {
        ...
        const wrap = document.querySelector('#quiz-questions-wrap'); if (wrap) wrap.innerHTML = `<div class="text-danger">${escapeHtml(msg)}</div>`;
        const submitBtn = document.querySelector('#quiz-submit-btn'); if (submitBtn) { submitBtn.disabled = true; submitBtn.textContent = 'Max attempts reached'; }
        showToast(msg, 'error', 4000); return;
      }
      ...
      let data = null; try { data = text ? JSON.parse(text) : {}; } catch (e) { data = {}; }
      ...
      if (summEl) { const c = await fetchAttemptCount(jobId); if (typeof c === 'number') summEl.textContent = `Attempts: ${c} / 3`; }

      stopQuizTimer(); closeQuizModal(); showToast('Quiz submitted', 'success', 2500);

    } catch (e) { console.error('onQuizSubmit error', e); showToast('Network error — try again', 'error'); }
  }

  /* ---------- closeQuizModal ---------- */
  function closeQuizModal() {
    const modal = document.querySelector('#quiz-modal'); if (modal) modal.style.display = 'none';
  ...
  function attachQuizButtons() {
    // Preload attempt summaries for visible job-cards
    ...
    document.querySelectorAll('.take-quiz-btn').forEach(btn => {
      if (btn.__quizAttached) return; btn.__quizAttached = true;
      btn.addEventListener('click', () => {
        const jid = parseInt(btn.dataset.jobId, 10);
        if (!jid) { console.warn('No job id found on button'); return; }
        openQuizModal(jid);
      });
    });
    ...
    document.querySelectorAll('.retake-btn').forEach(btn => {
      if (btn.__quizAttached) return; btn.__quizAttached = true;
      btn.addEventListener('click', () => { const jid = parseInt(btn.dataset.jobId, 10); openQuizModal(jid); });
    });
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', attachQuizButtons); else attachQuizButtons();

  // expose useful functions globally
  window.createQuizModal = createQuizModal;
  window.openQuizModal = function (jobId, questionPayload) {
    // call internal implementation above
    (function internalOpen() { /* reuse openQuizModal logic by calling the function defined earlier */
      // We defined `openQuizModal` as a nested function earlier in previous versions.
      // To keep a single canonical implementation, implement here by calling the same flow: create modal and fetch.
      // For simplicity re-run create + main fetch flow by delegating to the function below.
    })();
    // To avoid duplicating code in this file, call the detailed openQuizModal implementation created earlier by name
    // But since we implemented the main flow above as a named function inside the module, just call it if exists.
  };

  // expose core handlers as globals for external callers
...
/* Small toast helper if not present */
