...
// REPLACE START: renderJobs function
function renderJobs(jobs) {
  ...
  jobs.forEach(job => {
    const div = document.createElement('div');
    div.className = 'job-card border p-2 mb-2';

    // build card inner HTML including quiz buttons (uses same ids/classes as template)
    div.innerHTML = `
      <div>
        <h6>${escapeHtml(job.title || '')}</h6>
        <div class="small text-muted">${escapeHtml(job.skills || job.skills_required || '')}</div>
      </div>

      <div class="mt-2 d-flex align-items-center gap-2">
        <button class="btn btn-outline take-quiz-btn" data-job-id="${job.id}">Take Quiz</button>

        <button id="apply-btn-${job.id}" class="btn btn-primary apply-btn disabled"
                data-job-id="${job.id}" disabled>Apply</button>

        <button class="btn btn-secondary retake-btn" data-job-id="${job.id}" style="display:none;">
          Retake
        </button>

        <span id="quiz-status-${job.id}" class="small text-muted" style="margin-left:8px;">
          Not attempted
        </span>
      </div>
  ...
  // After rendering jobs, attach listeners for Take Quiz / Retake / Apply
  // If your attachQuizButtons() is defined (from the ADD block), call it
  if (typeof attachQuizButtons === 'function') {
    // small timeout to ensure DOM inserted (safe)
    setTimeout(() => attachQuizButtons(), 10);
  }
}
// REPLACE END
...
(function(){
  ...
  function openQuizModal(jobId, questionPayload) {
    ...
    fetchWithAuth('/api/quiz/generate/', {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      ...
      if (!r.ok) {
        ...
        wrap.innerHTML = `<div class="text-danger">Error loading questions: ${err && err.detail ? err.detail : r.statusText}</div>`;
        return;
      }
      const data = await safeParseJSON(r);
      if (data && data.questions) renderQuestionsInModal(data.questions);
      else wrap.innerHTML = '<div class="text-danger">No questions returned</div>';
    }).catch(e => {
  ...
  function renderQuestionsInModal(questions) {
    ...
    wrap.innerHTML = questions.map(q => {
      const opts = q.choices.map(c => {
        // use c.key or c.id as choice value depending on backend
        const val = (c.key !== undefined) ? c.key : (c.id !== undefined ? c.id : c.value);
        return `<div><label><input type="radio" name="q-${q.id}" value="${val}"> ${escapeHtml(c.text || c.label || val)}</label></div>`;
      }).join('');
      return `<div class="quiz-question" data-qid="${q.id}" style="margin-bottom:12px;">
                <div><strong>${escapeHtml(q.text || q.question)}</strong></div>
  ...
  function handleQuizSubmitResponse(jobId, responseData) {
    ...
    if (responseData.passed) {
      ...
      showToast(`Failed ❌ ${responseData.score}/${responseData.total}`);
    }
  }
