...
// CSRF helper (if you are not using JWT)
...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.start-interview-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      await startInterview(interviewId, inviteId);
    });
  });
});

// Actual function: call API, then open candidate page
  ...
  try {
    const resp = await fetch(`/api/interviews/candidate/${interviewId}/start/`, {
      ...
      headers: {
        ...
        'X-CSRFToken': getCookie('csrftoken'), // remove if JWT auth
      },
    ...
    if (!resp.ok) {
      showToast(`Failed: ${data.detail || resp.status}`, 'error', 6000);
      return;
    }

    // ✅ Always redirect to your candidate_interview page
    const pageUrl = `/interviews/page/candidate/${interviewId}/`;
