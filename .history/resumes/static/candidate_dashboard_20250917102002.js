...
(function () {
  ...
  document.querySelectorAll('.view-attempts-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      console.log("DEBUG: View Attempts clicked for jobId=", jid);
      if (typeof openAttemptHistoryModal === 'function') {
        openAttemptHistoryModal(jid);
      } else {
        alert("openAttemptHistoryModal not available");
      }
    });
  });
});
...
// Put this near top of candidate_dashboard.js (after DOM ready)
document.addEventListener('click', (e) => {
  ...
  if (!jid) {
    console.warn('No jobId for view-attempts button', btn);
    return;
  }
  // call your modal opener (implement below)
  if (typeof openAttemptHistoryModal === 'function') {
    openAttemptHistoryModal(jid, btn);
  } else {
    // fallback: fetch attempts and show simple modal
    fetchAndShowAttemptsModal(jid, btn);
  }
});
...
async function fetchAttemptsForJobSimple(jobId) {
  const token = localStorage.getItem('token') || '';
  const headers = {'Content-Type':'application/json'};
  if (token) headers['Authorization'] = 'Bearer ' + token;
  try {
    const r = await fetch(`/api/quiz/${jobId}/attempts/`, { method:'GET', headers });
    if (!r.ok) return { error:true, status:r.status, text: await r.text().catch(()=>null) };
    return await r.json();
  } catch(e) { return { error:true, exception:String(e)}; }
}

async function fetchAndShowAttemptsModal(jobId, btn){
  const wrapId = 'attempts-modal';
  let modal = document.getElementById(wrapId);
  if (!modal) {
    ...
    modal.innerHTML = `<div style="background:#fff;padding:16px;border-radius:8px;max-width:720px;width:92%;max-height:80vh;overflow:auto">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <h5 style="margin:0">Attempt history</h5>
        <button id="${wrapId}-close" class="btn btn-sm btn-outline-secondary">Close</button>
      </div>
      <div id="${wrapId}-body" style="margin-top:12px">Loading…</div>
    </div>`;
    document.body.appendChild(modal);
    modal.querySelector(`#${wrapId}-close`).addEventListener('click', ()=> modal.remove());
  } else {
    modal.querySelector(`#${wrapId}-body`).innerHTML = 'Loading…';
    modal.style.display = 'flex';
  }
  const body = modal.querySelector(`#${wrapId}-body`);
  const data = await fetchAttemptsForJobSimple(jobId);
  if (!data || data.error) {
    body.innerHTML = `<div class="text-danger">Error loading attempts.</div>`;
    console.warn('attempts fetch error', data);
    return;
  }
  const arr = Array.isArray(data) ? data : (data.results || data.attempts || []);
  if (!arr || arr.length === 0) {
    body.innerHTML = '<div>No attempts yet.</div>';
    return;
  }
  // render table
  const rows = arr.map(a => {
    const id = a.id || a.attempt_id || '';
    const score = a.score ?? 0;
    const total = a.total ?? a.total_questions ?? 0;
    const passed = a.passed ? 'Yes' : 'No';
    const t = a.finished_at || a.started_at || '';
    return `<tr><td>${id}</td><td>${score}/${total}</td><td>${passed}</td><td>${t}</td></tr>`;
  }).join('');
  body.innerHTML = `<table style="width:100%;border-collapse:collapse">
    <thead><tr><th>#</th><th>Score</th><th>Passed</th><th>Finished</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>`;
}
