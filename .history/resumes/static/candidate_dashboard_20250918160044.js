// candidate_dashboard.patched.js - patched & consolidated
// Key fixes:
// - single escapeHtml
// - single getAuthToken
// - single canonical handleQuizSubmitResponse
// - normalized data-job-id attributes
// - completed loadMyApplications and wiring
// Use as a module (<script type="module">) if you keep the import at top.
...
function getAuthToken() {
  ...
  return v || (localStorage.getItem('token') || '').trim();
}
...
function uploadWithXHR(file) {
  return new Promise(resolve => {
    ...
    const token = getAuthToken(); if (token) xhr.setRequestHeader('Authorization', 'Bearer ' + token);
...
async function loadJobs() {
  ...
  jobs.forEach(j => {
    ...
    card._job = j;
    card.setAttribute('data-job-id', String(j.id));
...
/* Fetch & render attempt history (minimal) */
...
/* Quiz submit handler & UI updates (canonical) */
async function handleQuizSubmitResponse(jobId, responseData) {
  try {
    // update job card attempts area
    // minimal: call loadAttemptHistory to refresh UI
    try { await loadAttemptHistory(jobId); } catch (e) { /* ignore */ }
    ...
    if (jobCard) {
      ...
      if (applyBtn) {
        if (responseData.passed) { applyBtn.disabled = false; applyBtn.classList.remove('disabled'); }
      }
      const statusEl = jobCard.querySelector(`#quiz-status-${jobId}`);
      if (statusEl) { statusEl.textContent = responseData.passed ? 'Passed' : 'Failed'; statusEl.style.color = responseData.passed ? 'green' : 'crimson'; }
    }
  ...
  } catch (e) { console.error('handleQuizSubmitResponse error', e); }
}
...
function initDashboard() {
  ...
  // --- Delegated handlers for dynamic buttons (fix for "Take Quiz" and "Refresh Jobs") ---
document.addEventListener('click', function (e) {
  try {
    // Take Quiz (delegated)
    const takeBtn = e.target.closest && e.target.closest('.take-quiz-btn');
    if (takeBtn) {
      e.preventDefault();
      const jid = Number(takeBtn.dataset.jobId || takeBtn.getAttribute('data-job-id') || 0);
      if (!jid) {
        console.warn('take-quiz-btn missing job id', takeBtn);
        showToast('Job id missing on button', 'error');
      } else {
        if (typeof openQuizModal === 'function') {
          openQuizModal(jid);
        } else {
          // fallback: try to open via window.openQuizModal if it exists
          if (typeof window.openQuizModal === 'function') window.openQuizModal(jid);
          else {
            console.warn('openQuizModal not defined');
            showToast('Quiz UI not available', 'error');
          }
        }
      }
      return;
    ...
    // Refresh Jobs button (delegated) — works even if #refreshJobs added dynamically
    const refreshBtn = e.target.closest && e.target.closest('#refreshJobs');
    if (refreshBtn) {
      e.preventDefault();
      try { loadJobs(); } catch (err) { console.error('loadJobs error', err); showToast('Failed to refresh jobs', 'error'); }
      return;
    }
  } catch (err) {
...
console.log('candidate_dashboard.patched.js - init complete');
