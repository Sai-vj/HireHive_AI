...
function savedToken(){ return localStorage.getItem('token') || ''; }
...
async function apiFetch(path, opts = {}) {
  ...
  try {
    ...
    if (r.status === 401 || r.status === 403) {
      ...
      alert('Not authorized — paste valid token.');
      return { ok:false, status:r.status, data:null };
    }
    ...
    console.error('apiFetch error', e);
    return { ok:false, error:true, exception:String(e) };
  }
}
...
// "Create Interview & Invite" — convenient: creates interview then sends invite
if (inviteCreateThenSendBtn) inviteCreateThenSendBtn.addEventListener('click', async (e) => {
  ...
  if (!candidateId) { alert('Enter candidate id'); return; }
  if (!jobId) {
    alert('No job selected. Open the job first then create interview.');
    return;
  }

  inviteCreateThenSendBtn.disabled = true;
  try {
    // create interview with basic defaults (title from job + scheduled)
    const payload = {
    ...
    // created may include id
    const interviewId = (created && (created.id || created.pk)) || null;
    // now send invite using job-based invite endpoint (server will use interview if needed)
...
function injectInviteButtonsIntoAppRows(selector = '.application-row') {
  ...
  rows.forEach(row => {
    ...
    // create button area if not exists
    ...
    btn.dataset.candidateName = candidateName;
    if (jobId) btn.dataset.jobId = jobId;
...
// run injection at start and later
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    const mo = new MutationObserver((muts) => { muts.forEach(m => { m.addedNodes && m.addedNodes.forEach(n => { if (n.nodeType === 1) { if (n.matches && n.matches('.application-row')) injectInviteButtonsIntoAppRows(); n.querySelectorAll && n.querySelectorAll('.application-row').forEach(r=>injectInviteButtonsIntoAppRows()); } }); }); });
