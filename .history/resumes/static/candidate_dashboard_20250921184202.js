        ...
        invites.forEach(inv => {
          ...
          div.querySelectorAll('.start-invite').forEach(b => b.addEventListener('click', async () => {
            ...
            try {
              ...
              // If API returns redirect info, use it; otherwise fallback to candidate page
              if (r && r.ok && (r.data?.redirect_url || r.data?.join_url || r.data?.url)) {
                ...
                window.location.href = `${INTERVIEWS_BASE}/page/candidate/${iid}?invite=${encodeURIComponent(inviteId)}`;
              }
            } catch (e) {
