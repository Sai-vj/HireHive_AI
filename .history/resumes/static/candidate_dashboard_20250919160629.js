...
async function startInterview(interviewId) {
  ...
  // some backends expect POST to /api/interviews/candidate/<id>/start/
  const tries = [
  ...
  for (const u of tries) {
    try {
      ...
      bodyEl.innerHTML = `<pre>${escapeHtml(JSON.stringify(data, null, 2))}</pre>`;
...
// Put this after your other event attachment code in candidate_dashboard.js

// If multiple handlers exist, guard to avoid double-binding
if (!window.__startInterviewHandlerRegistered) {
  ...
  document.addEventListener('click', function (e) {
    ...
    try {
      // If you want to call API to "start" attempt server-side before opening,
      // uncomment the fetch below (optional). If not needed, we simply open page.
      /*
      (async function(){
        try {
          const r = await fetch(`/api/interviews/candidate/${interviewId}/start/`, { method: 'POST' });
          if (!r.ok) {
            const txt = await r.text().catch(()=>null);
            showToast('Failed to start interview: ' + (txt || r.status), 'error');
            btn.disabled = false;
            btn.classList.remove('disabled');
            return;
          }
          // Optional: read returned data if needed
      ...
      */
      ...
      // default: directly open the interview page
      openInterviewPage(interviewId, inviteId);
    } finally {
      // re-enable after a short delay, to allow the window open to happen
      setTimeout(() => { try { btn.disabled = false; btn.classList.remove('disabled'); } catch(e){} }, 800);
    }
  }, false);
