...
/* ---------- Invites + Interview helpers ---------- */

// Correct frontend page route from interviews/urls.py
...
async function openInvitesModal() {
  ...
  if (!mEl || !listEl) {
    alert('Invites modal DOM missing');
    return;
  }
  ...
  for (const u of tries) {
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        listEl.innerHTML =
          '<div class="text-danger">Auth required. Paste token and Save.</div>';
        return;
      }
      if (!res.ok) continue;
      data = Array.isArray(res.data)
        ? res.data
        : res.data?.results || res.data?.invitations || [];
      break;
    } catch (e) {
      continue;
    }
  ...
  data.forEach((inv) => {
    ...
    const interviewId =
      interview.id || interview.pk || inv.interview_id || inv.interview || '';
    const title = interview.title || inv.title || 'Interview';
    const recruiter =
      inv.recruiter_name || inv.recruiter || interview.recruiter || '';
    ...
    row.innerHTML = `
      <div class="d-flex justify-content-between">
        ...
        <div style="min-width:160px;text-align:right">
          <div class="mb-2">Status: <span class="badge ${
            status === 'accepted'
              ? 'bg-success'
              : status === 'declined'
              ? 'bg-danger'
          ...
          <div>
            ${
              status === 'pending'
                ? `<button class="btn btn-sm btn-success modal-accept" data-id="${inv.id}">Accept</button>
                   <button class="btn btn-sm btn-outline-danger modal-decline" data-id="${inv.id}">Decline</button>`
                : ''
            }
            ${
              status === 'accepted' && interviewId
                ? `<button class="btn btn-sm btn-primary modal-start" data-interview-id="${interviewId}" data-invite-id="${inv.id}">Start Interview</button>`
                : ''
            }
          </div>
        </div>
  ...
  listEl.querySelectorAll('.modal-accept').forEach((b) => {
    b.addEventListener('click', async () => {
      ...
      if (r && r.ok) showToast('Accepted', 'success');
  ...
  listEl.querySelectorAll('.modal-decline').forEach((b) => {
    b.addEventListener('click', async () => {
      ...
      if (r && r.ok) showToast('Declined', 'success');
  ...
  listEl.querySelectorAll('.modal-start').forEach((b) => {
    b.addEventListener('click', async () => {
      ...
      if (r && r.ok && r.url) {
        showToast('Interview opened', 'success');
        try {
          bootstrap.Modal.getInstance(
            document.getElementById('invitesModal')
          ).hide();
        } catch (e) {}
        return;
      }
      // fallback open frontend page
    ...
    div.innerHTML = `
      <div class="d-flex justify-content-between">
        ...
        <div style="text-align:right;min-width:180px">
          <div class="mb-1">Status: <span class="badge ${
            status === 'accepted'
              ? 'bg-success'
              : status === 'declined'
              ? 'bg-danger'
          ...
          <div>
            ${
              status === 'pending'
                ? `<button class="btn btn-sm btn-success accept-invite-btn me-1" data-id="${inv.id}">Accept</button><button class="btn btn-sm btn-outline-danger decline-invite-btn" data-id="${inv.id}">Decline</button>`
                : ''
            }
            ${
              status === 'accepted' && interviewId
                ? `<button class="btn btn-sm btn-primary start-interview-btn" data-interview-id="${interviewId}" data-invite-id="${inv.id}">Start Interview</button>`
                : ''
            }
          </div>
        </div>
...
// after calling /api/interviews/candidate/<id>/start/
if (res && res.ok) {
  ...
  // fallback: open the frontend helper route that exists in urls.py
  const helper = `/interviews/frontend/start/${encodeURIComponent(interviewId)}/?invite=${encodeURIComponent(inviteId || '')}`;
  window.open(helper, '_blank', 'noopener,noreferrer');
  return { ok: true };
}
...
async function respondInvite(inviteId, action) {
  ...
  try {
    // --- Send form-urlencoded instead of raw JSON ---
    const form = new URLSearchParams();
    form.append("action", action);  // or "response" if your view expects that
