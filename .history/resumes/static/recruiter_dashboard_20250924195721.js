...
async function openReviewAttempts(jobId) {
  try {
    const r = await apiFetch(`/api/recruiter/jobs/${jobId}/attempts/`, { method: 'GET' });
    if (!r || !r.ok) return showToast('Unable to load attempts', 'error');
    
    console.log('Attempts:', r.data);
    // TODO: render into UI (make a new section or modal)
    // Example: show in a modal
    const modal = new bootstrap.Modal(document.getElementById('reviewAttemptsModal'));
    modal.show();
    document.getElementById('attemptsList').innerHTML = JSON.stringify(r.data, null, 2);
  } catch (e) {
    console.error('Review attempts failed', e);
