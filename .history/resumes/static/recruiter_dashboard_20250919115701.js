...
async function loadApplicationsForSelectedJob() {
  ...
  listEl.innerHTML = '';
   // inside loadApplicationsForSelectedJob(), replace the card creation / append section with:
  apps.forEach(a => {
    ...
    const card = document.createElement('div');
    card.className = 'card mb-2 p-2 application-row';   // <-- add application-row class
  ...
  // after rendering all apps, call injector so Invite buttons are appended
...
// static/recruiter_dashboard.js
// recruiter_invites.js
// Minimal merged invite/create-interview module for recruiter dashboard.
// Assumptions: bootstrap optional. Expects modal HTML with following IDs:
// inviteModal, invite_candidate_id, invite_candidate_name, invite_job_id, invite_interview_id,
// invite_scheduled_at, invite_message, inviteSendBtn, inviteCancelBtn, inviteCreateThenSendBtn
// createInterviewModal, createInterviewForm, ci_job_id, ci_title, ci_description, ci_scheduled_at, ci_duration, ci_mode, ci_create
...
/* ---------------- light apiFetch wrapper ---------------- */

/* ---------------- DOM refs (safe getters) ---------------- */
const el = id => document.getElementById(id);
const inviteModal = el('inviteModal');
const inviteCandidateIdInput = el('invite_candidate_id');
const inviteCandidateNameInput = el('invite_candidate_name');
const inviteJobIdInput = el('invite_job_id');
const inviteInterviewIdInput = el('invite_interview_id');
const inviteScheduledAtInput = el('invite_scheduled_at');
const inviteMessageInput = el('invite_message');
const inviteSendBtn = el('inviteSendBtn');
const inviteCancelBtn = el('inviteCancelBtn');
const inviteCreateThenSendBtn = el('inviteCreateThenSendBtn');

const createInterviewModal = el('createInterviewModal');
const createInterviewForm = el('createInterviewForm');
const ci_job_id = el('ci_job_id');
const ci_title = el('ci_title');
const ci_description = el('ci_description');
const ci_scheduled_at = el('ci_scheduled_at');
const ci_duration = el('ci_duration');
const ci_mode = el('ci_mode');
const ci_create_btn = el('ci_create');
...
function showInviteModal({ jobId=null, interviewId=null, candidateId='', candidateName='' } = {}) {
  ...
  inviteModal.classList.remove('d-none'); inviteModal.style.display = 'flex'; document.body.style.overflow = 'hidden';
}
function hideInviteModal() {
  if (!inviteModal) return;
  inviteModal.classList.add('d-none'); inviteModal.style.display = 'none'; document.body.style.overflow = '';
  inviteContext = { jobId:null, interviewId:null, candidateId:null, candidateName:null };
}

/* ---------------- create interview + invite functions ---------------- */
...
/* ---------------- bindings: invite modal controls ---------------- */
...
if (inviteSendBtn) inviteSendBtn.addEventListener('click', async (e) => {
  ...
  try {
    ...
    if (res.ok) {
      alert('Invite sent');
      hideInviteModal();
      // optional refresh hook if present
      if (typeof loadApplicationsForSelectedJob === 'function') {
        try { loadApplicationsForSelectedJob(); } catch(e){ /* ignore */ }
      }
    } else {
...
if (inviteCreateThenSendBtn) inviteCreateThenSendBtn.addEventListener('click', async (e) => {
  ...
  } finally { inviteCreateThenSendBtn.disabled = false; }
});

/* ---------------- utility: inject invite buttons into application rows ---------------- */
...
/* ---------------- run injection and observers on DOM ready ---------------- */
document.addEventListener('DOMContentLoaded', () => {
...
/* ---------------- optional: export helpers for console/debug ---------------- */
window.showInviteModal = showInviteModal;
window.hideInviteModal = hideInviteModal;
window.injectInviteButtonsIntoAppRows = injectInviteButtonsIntoAppRows;
window.setRecruiterToken = setSavedToken;
window.getRecruiterToken = savedToken;
