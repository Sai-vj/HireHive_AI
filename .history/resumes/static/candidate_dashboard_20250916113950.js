...
(function(){
  ...
  function createQuizModal() {
    ...
    document.querySelector('#quiz-submit-btn').addEventListener('click', async () => {
      ...
      if (btn) { btn.disabled = true; btn.dataset.orig = btn.innerText; btn.innerText = 'Submitting...'; }
      try {
        await submitQuizAttempt(jobId, answers);
      } finally {
        if (btn) { btn.disabled = false; btn.innerText = btn.dataset.orig || 'Submit'; }
      }
    });
  }

  // open quiz modal and fetch questions (generate endpoint)
  function openQuizModal(jobId, questionPayload) {
    ...
    // Use your backend route: POST /api/quiz/generate/<job_id>/
    // inside openQuizModal, replace current fetch(...) with:
const token = localStorage.getItem('token') || document.getElementById('tokenInput')?.value || '';
console.log('>>> calling generate, jobId=', jobId, ' token present=', !!token);
const resp = await fetch(`/api/quiz/generate/${jobId}/`, {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': token ? ('Bearer ' + token) : ''
  },
  body: JSON.stringify({}) // backend may or may not need body
});
console.log('generate status', resp.status);
console.log('generate text', await resp.text());



    fetch(`/api/quiz/generate/${jobId}/`, {
      method: 'POST',
      headers,
    ...
    .then(async r => {
      if (!r.ok) {
        // show helpful server message (could be HTML when not logged in)
        const body = await safeParseJSON(r);
        const msg = (body && (body.detail || body.message)) || (typeof body === 'string' ? body : `Status ${r.status}`);
        wrap.innerHTML = `<div class="text-danger">Error loading questions: ${msg}</div>`;
        console.warn('quiz generate error', r.status, body);
        return;
      }
      const data = await safeParseJSON(r);
      if (!data || !Array.isArray(data.questions)) {
        wrap.innerHTML = `<div class="text-danger">No questions returned</div>`;
        console.warn('quiz generate: unexpected data', data);
        return;
      }
    ...
    .catch(err => {
      ...
      console.error('quiz generate network error', err);
    });
  }
  ...
  function renderQuestionsInModal(questions) {
    ...
    wrap.innerHTML = questions.map(q => {
      const qid = q.id ?? q.question_id ?? Math.floor(Math.random()*100000);
      const opts = (q.choices || []).map((c, idx) => {
        const val = (c.key !== undefined) ? c.key : (c.id !== undefined ? c.id : (c.value !== undefined ? c.value : ('opt' + idx)));
        return `<div><label><input type="radio" name="q-${qid}" value="${escapeHtml(String(val))}"> ${escapeHtml(c.text || c.label || String(val))}</label></div>`;
      }).join('');
      return `<div class="quiz-question" data-qid="${escapeHtml(String(qid))}" style="margin-bottom:12px;">
                <div><strong>${escapeHtml(q.text || q.question || ('Question ' + qid))}</strong></div>
                <div style="margin-left:8px;">${opts}</div>
              </div>`;
    }).join('');
  }

  // Submit attempt to backend (use POST /api/quiz/<jobId>/attempt/)
  async function submitQuizAttempt(jobId, answers) {
    try {
      const token = localStorage.getItem('token') || document.getElementById('tokenInput')?.value || '';
      const headers = { 'Content-Type': 'application/json' };
      if (token) headers['Authorization'] = 'Bearer ' + token;

      // If your backend expects resume_id, you can add it here; currently pass answers + job_id
      const payload = { job_id: jobId, answers };

      const resp = await fetch(`/api/quiz/${jobId}/attempt/`, {
        method: 'POST',
        headers,
        body: JSON.stringify(payload)
      });
      ...
      if (!resp.ok) {
        const msg = (body && (body.detail || body.message)) || `Status ${resp.status}`;
        showToast(msg, 'error');
        console.warn('quiz submit failed', resp.status, body);
        return;
      }

      // expected { attempt_id, score, total, passed, ... }
      const data = body || {};
  ...
  function handleQuizSubmitResponse(jobId, responseData) {
    // update attempt history and UI
    updateAttemptHistoryUI(jobId, responseData);
    const statusEl = document.querySelector(`#quiz-status-${jobId}`);
    const retakeBtn = document.querySelector(`#retake-btn-${jobId}`) || document.querySelector(`.retake-btn[data-job-id="${jobId}"]`);
    const applyBtn = document.querySelector(`#apply-btn-${jobId}`) || document.querySelector(`.apply-btn[data-job-id="${jobId}"]`);

    const passed = !!responseData.passed;
    const scoreStr = (responseData.score !== undefined && responseData.total !== undefined) ? `${responseData.score}/${responseData.total}` : '';

    if (passed) {
      if (statusEl) statusEl.textContent = 'Passed';
      if (applyBtn) { applyBtn.disabled = false; applyBtn.classList.remove('disabled'); }
      if (retakeBtn) { retakeBtn.style.display = 'none'; }
      showToast(`Passed ✅ ${scoreStr}`, 'success');
    } else {
      if (statusEl) statusEl.textContent = 'Failed';
      if (applyBtn) { applyBtn.disabled = true; applyBtn.classList.add('disabled'); }
      if (retakeBtn) { retakeBtn.style.display = 'inline-block'; }
      showToast(`Failed ❌ ${scoreStr}`, 'info');
    }
  }
  ...
  function updateAttemptHistoryUI(jobId, attempt) {
    ...
    node.innerHTML = `Attempt ${id}: ${score}/${total} — ${passed} <small>(${new Date().toLocaleString()})</small>`;
    wrap.prepend(node);
  }

  // optional: load attempt history helper (calls existing endpoint if available)
  async function loadAttemptHistory(jobId) {
    try {
      const token = localStorage.getItem('token') || document.getElementById('tokenInput')?.value || '';
      const headers = {};
      if (token) headers['Authorization'] = 'Bearer ' + token;
      const r = await fetch(`/api/quiz/attempts/?job_id=${jobId}`, { headers });
      if (!r.ok) return;
  ...
  // attach click listeners to existing buttons rendered in DOM
  ...
  // expose for debug & integration with other code
