  ...
  async function loadInvites() {
    ...
    arr.forEach(inv => {
      ...
      div.querySelectorAll('.accept-invite').forEach(b => b.addEventListener('click', async () => {
        b.disabled = true; const r = await apiFetch(API.INVITE_RESPOND(id), { method: 'POST', headers: { 'Content-Type':'application/x-www-form-urlencoded' }, body: `response=accept` });
      ...
      div.querySelectorAll('.decline-invite').forEach(b => b.addEventListener('click', async () => {
        b.disabled = true; const r = await apiFetch(API.INVITE_RESPOND(id), { method: 'POST', headers: { 'Content-Type':'application/x-www-form-urlencoded' }, body: `response=decline` });
      ...
      div.querySelectorAll('.start-invite').forEach(b => b.addEventListener('click', async () => {
        ...
        if (r.ok && (r.data?.redirect_url || r.data?.join_url || r.data?.url)) {
          ...
          // fallback: navigate to candidate interview page
          window.location.href = `/interviews/page/candidate/${iid}/?invite=${inviteId}`;
        }
        b.disabled = false;
      }));
    });
