...
async function submitAddJob(e) {
  ...
  try {
  let res;   // define only once

  if (editingId) {
    res = await apiFetch(`${JOBS_ENDPOINT}${encodeURIComponent(editingId)}/`, {
      method: 'PATCH',
      body: JSON.stringify(payload)
    });
    if (!res || !res.ok) {
      res = await apiFetch(`/api/recruiter/job/${encodeURIComponent(editingId)}/`, {
        method: 'PATCH',
        body: JSON.stringify(payload)
      });
    }
 ...
 // after the POST/CREATED success
const createdJob = res.data || null;
console.log("Job API response (res):", res);          // whole response object
console.log("Created Job object (res.data):", createdJob);
...
;
  ...
  // optional: auto-generate quiz for new job
if (newId) {
  // call with proper id (this uses the correct URL /api/quiz/<id>/generate/)
  const quizData = await generateQuizForJob(newId, 5);
  console.log('Quiz generate response:', quizData);
}
