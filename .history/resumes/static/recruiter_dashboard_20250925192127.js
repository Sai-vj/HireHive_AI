...
/* ---------- Separate: Interview vs Quiz generation ---------- */

/* helper: get csrftoken (already present) */
async function getCsrfTokenFromCookie() {
  try { return document.cookie.split('; ').find(c=>c.startsWith('csrftoken='))?.split('=')[1] || null; } catch(e){ return null; }
}

/* helper: resolve token (same as yours) */
function resolveAuthToken() {
  return (typeof window.savedToken === 'function') ? window.savedToken() : (localStorage.getItem('recruiter_token_v1') || '');
}

/* ---------------- Interview generation (existing) ---------------- */
async function generateInterviewQuestions(interviewPk, nQuestions = 25, autoPublish = false) {
  ...
  try {
    const body = { n_questions: Number(nQuestions)||25, auto_publish: !!autoPublish, params: {} };
...
/* ---------------- Quiz generation (NEW) ----------------
   - Hitting a different API endpoint (you must implement backend if missing)
   - Example endpoint: POST /api/quiz/recruiter/<job_pk>/generate/ or /api/quiz/generate/
*/
async function generateQuizQuestionsForJob(jobPk, nQuestions = 10, autoPublish = false) {
  ...
  // Change URL to your quiz endpoint. Example below:
  const url = `/api/quiz/recruiter/${encodeURIComponent(jobPk)}/generate/`;
  try {
    const body = { n_questions: Number(nQuestions)||10, auto_publish: !!autoPublish, params: {} };
    const r = await fetch(url, { method:'POST', credentials:'include', headers, body: JSON.stringify(body) });
    const text = await r.text().catch(()=>null);
    let data = null;
    try { data = text ? JSON.parse(text) : null; } catch(e) { data = text; }
    return { ok: r.ok, status: r.status, data, text };
  } catch(e){ console.error('generateQuizQuestionsForJob', e); return { ok:false, error:String(e) }; }
}

/* utility: create interview for job (unchanged) */
...
/* convenience: create interview then generate interview questions */
...
/* convenience: create quiz directly for job (if your backend requires quiz creation first) */
async function createQuizForJobThenGenerate(jobPk, questionsCount = 10) {
  // Optional: implement create-quiz endpoint if you need a quiz resource
  // Example: POST /api/quiz/recruiter/job/<jobPk>/create/ then call generateQuizQuestionsForJob with returned quiz id
  return { ok:false, error: 'Not implemented: createQuizForJobThenGenerate' };
}
...
function attachGenerateInterviewButtons() {
  document.querySelectorAll('.generate-questions-btn').forEach(btn => {
    ...
    btn.addEventListener('click', async (ev) => {
      ...
      if (res && res.ok) showToast('Interview generation started/complete', 'success'); else showToast('Interview generation failed', 'error');
      try { if (typeof loadInterviewDetail === 'function') await loadInterviewDetail(pk); } catch(e){}
    });
  });
}

function attachGenerateQuizButtons() {
  document.querySelectorAll('.generate-quiz-btn').forEach(btn => {
    if (btn._quizBound) return; btn._quizBound = true;
    btn.addEventListener('click', async (ev) => {
      ev.stopPropagation(); ev.preventDefault();
      ...
      const n = parseInt(btn.dataset.questionsCount || btn.getAttribute('data-questions-count') || 10, 10) || 10;
      // call quiz endpoint (no interview creation)
      const res = await generateQuizQuestionsForJob(jobId, n, (btn.dataset.autoPublish === 'true'));
      if (res && res.ok) showToast('Quiz generation queued/complete', 'success'); else showToast('Quiz generation failed', 'error');
...
window.attachGenerateQuizButtons = attachGenerateQuizButtons;

/* auto-run attach on DOM ready */
document.addEventListener('DOMContentLoaded', () => {
  attachGenerateInterviewButtons();
  attachGenerateQuizButtons();
});
