...
(function(){ 
  ...
  window.openQuizModal = async function(jobId, questionPayload) {
    ...
    function applyAttemptLimitUI(jobId, attemptsArr) {
  const count = Array.isArray(attemptsArr) ? attemptsArr.length : (attemptsArr || 0);
  const retakeBtn = document.querySelector(`.retake-btn[data-job-id="${jobId}"]`);
  if (!retakeBtn) return;
  if (count >= 3) {
    retakeBtn.disabled = true;
    retakeBtn.textContent = 'Max Attempts';
...
// --- Timer helpers (copy into same file where openQuizModal is) ---
let __quizTimerHandle = null;     // global for clearing between opens
...
function startQuizTimer(seconds, onExpire) {
  ...
  // first immediate update and then interval
  tick();
...
renderQuestionsInModal(questions);

// start timer: set seconds as needed, e.g. 120 seconds
startQuizTimer(120, async () => {
  // onExpire: auto-submit or disable UI
  // auto submit: trigger submit button click
  const submitBtn = document.querySelector('#quiz-submit-btn');
  if (submitBtn) {
    submitBtn.disabled = true;
    // optionally show toast
    showToast('Time up — submitting...', 'info');
    submitBtn.click(); // make sure your submit handler reads values and sends
    // if your submit handler uses async and disables button, adjust accordingly
  } else {
