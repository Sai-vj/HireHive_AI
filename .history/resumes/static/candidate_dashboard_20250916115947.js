...
(function(){
 ...
 fetchWithAuth(`/api/quiz/generate/${jobId}/`, {
  method: 'POST',
  headers: {'Content-Type':'application/json'},
  body: JSON.stringify({ job_id: jobId })
}).then(async r => {
  if (!r.ok) {
    // show helpful message for auth errors too
    const err = await safeParseJSON(r);
    const detail = err && (err.detail || err.message) ? (err.detail || err.message) : (r.statusText || `Status ${r.status}`);
    wrap.innerHTML = `<div class="text-danger">Error loading questions: ${detail}</div>`;
    console.warn('quiz generate failed', r.status, err);
    return;
  }
  const data = await safeParseJSON(r);
  if (data && data.questions && Array.isArray(data.questions)) renderQuestionsInModal(data.questions);
  else {
    // sometimes backend returns array directly
    if (Array.isArray(data)) renderQuestionsInModal(data);
