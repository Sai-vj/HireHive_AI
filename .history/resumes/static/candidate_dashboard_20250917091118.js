...
window.openAttemptHistoryModal = async function(jobId) {
  ...
  try {
    const r = await fetch(`/api/quiz/${jobId}/attempts/`, { headers });
    if (!r.ok) {
      const txt = await r.text();
      listEl.innerHTML = `<div class="text-danger">Error: ${txt}</div>`;
      return;
    }
    ...
    if (!arr || arr.length === 0) {
      listEl.innerHTML = "<div class='small-muted'>No attempts yet.</div>";
      return;
    }
    ...
    listEl.innerHTML = `
      <table class="table table-sm table-bordered">
        <thead>
          <tr><th>ID</th><th>Score</th><th>Status</th><th>Finished</th></tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>`;
  } catch (e) {
    listEl.innerHTML = `<div class="text-danger">Error loading attempts</div>`;
    console.error("openAttemptHistoryModal error", e);
  }
