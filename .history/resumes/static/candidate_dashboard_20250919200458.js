...
// correct frontend route: use Django's interviews/candidate/<id>/
const FRONTEND_INTERVIEW_PAGE = (id, inviteId) => `/interviews/candidate/${encodeURIComponent(id)}/?invite=${encodeURIComponent(inviteId)}`;
// robust fallback: try server start -> probe likely frontend paths (HEAD) -> final open
async function openCandidateInterviewFallback(iid, inviteId) {
  // 1) try server-side start first (startInterview may return url)
  try {
    const r = await startInterview(iid);
    if (r && r.ok && r.url) {
      // startInterview already opened the url or returned it
      return;
    }
  } catch (e) {
    console.warn('startInterview attempt failed', e);
  }

  // 2) try likely frontend routes with HEAD check (avoids opening 404 right away)
  const tries = [
    ...
    `/candidate/interview/${encodeURIComponent(iid)}/`,
    `/interviews/templates/candidate_interview.html?interview=${encodeURIComponent(iid)}&invite=${encodeURIComponent(inviteId)}`,
    `/candidate/interview_page/${encodeURIComponent(iid)}/?invite=${encodeURIComponent(inviteId)}`
  ];

  for (const url of tries) {
    try {
      // HEAD is quicker and avoids rendering 404 page; credentials same-origin
      ...
      // HEAD might be blocked by CORS if different origin; ignore and try next
      console.warn('HEAD check failed for', url, err);
    }
  }

  // 3) final fallback: open the canonical guess so dev can see Django 404 & patterns
...
// open invites modal and render invites (light reuse of your endpoints)
async function openInvitesModal() {
  const rbtn = document.getElementById('invitesModalRefresh');
if (rbtn) rbtn.addEventListener('click', openInvitesModal);

  const mEl = document.getElementById('invitesModal');
  const listEl = document.getElementById('invitesListModal');
  if (!mEl || !listEl) { alert('Invites modal DOM missing'); return; }
  ...
  // call your existing loadInvites endpoints sequence
  const tries = [
  ...
  listEl.querySelectorAll('.modal-accept').forEach(b=>{
    b.addEventListener('click', async ()=>{ b.disabled=true; const r=await respondInvite(b.dataset.id,'accept'); if(r && r.ok) showToast('Accepted','success'); else showToast('Accept failed','error'); setTimeout(openInvitesModal,400); });
  });
  listEl.querySelectorAll('.modal-decline').forEach(b=>{
    b.addEventListener('click', async ()=>{ b.disabled=true; const r=await respondInvite(b.dataset.id,'decline'); if(r && r.ok) showToast('Declined','success'); else showToast('Decline failed','error'); setTimeout(openInvitesModal,400); });
  });
  listEl.querySelectorAll('.modal-start').forEach(b=>{
    b.addEventListener('click', async ()=>{
      b.disabled=true;
      const iid = b.dataset.interviewId, inviteId = b.dataset.inviteId;
      // try server-side start first
      const r = await startInterview(iid);
      if (r && r.ok && r.url) { showToast('Interview opened','success'); try{ bootstrap.Modal.getInstance(document.getElementById('invitesModal')).hide(); }catch(e){}; return; }
      // fallback open frontend page (not API)
      const page = FRONTEND_INTERVIEW_PAGE(iid, inviteId);
...
// attach open to any button with attribute data-open-invites-modal
document.addEventListener('click', function(e){
...
// modal refresh
document.getElementById?.('invitesModalRefresh')?.addEventListener?.('click', openInvitesModal);

/* ---------- Quick loadInvites for page (renders in #invitesSection / #invitesList) ---------- */
async function loadInvites() {
  const container = document.getElementById('invitesSection');
  const listEl = document.getElementById('invitesList');
  if (!listEl || !container) {
    console.warn('Invites UI missing (#invitesList or #invitesSection)');
    return;
  }
  ...
  for (const u of tries) {
    try {
      ...
      console.warn('loadInvites fetch error', u, e);
      continue;
    }
  }
  ...
  if (!data || data.length === 0) {
    ...
    // optionally hide container: container.style.display = 'none';
    return;
  }
  ...
  data.forEach(inv => {
    ...
    div.innerHTML = `
      <div class="d-flex justify-content-between">
        <div style="min-width:0">
          <strong style="display:block">${escapeHtml(title)}</strong>
          <div class="small-muted" style="margin-top:4px">${inv.scheduled_at ? new Date(inv.scheduled_at).toLocaleString() : (interview.scheduled_at ? new Date(interview.scheduled_at).toLocaleString() : '—')}</div>
  ...
  // wire handlers (delegation ok but keep local)
  ...
  listEl.querySelectorAll('.start-interview-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      const r = await startInterview(iid);
      if (r && r.ok && r.url) { showToast('Interview opened', 'success'); return; }
      // fallback open frontend page (not raw API)
      const page = FRONTEND_INTERVIEW_PAGE(iid, inviteId);
      window.open(page, '_blank', 'noopener,noreferrer');
      btn.disabled = false;
    });
...
async function startInterview(interviewId) {
  ...
  try {
    ...
    const res = await apiFetch(`/api/interviews/${interviewId}/start/`, { method: 'POST' });
    showSpinner(false);
    if (res && res.ok) {
      // if backend sends a join url
      const joinUrl = res.data?.url || res.data?.join_url || null;
      if (joinUrl) {
        window.open(joinUrl, '_blank', 'noopener,noreferrer');
        return { ok: true, url: joinUrl };
      }
      return { ok: true };
    } else {
      showToast(`Failed to start interview (${res.status})`, 'error');
      return { ok: false };
    }
  } catch (err) {
    showSpinner(false);
...
async function respondInvite(inviteId, action) {
  ...
  try {
    const res = await apiFetch(`/api/interviews/invites/respond/`, {
      method: 'POST',
...
window.loadInvites = openInvitesModal; // link to our modal loader

// Expose functions to global scope for legacy inline handlers / templates
