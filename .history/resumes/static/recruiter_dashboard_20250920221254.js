...
function openAddJobModal() {
  ...
  if (modalEl) {
    // only call bootstrap if the element exists (and is bootstrap modal)
    try {
      const bsModal = new bootstrap.Modal(modalEl, { backdrop: 'static' });
      bsModal.show();
    } catch (e) {
      console.warn('Bootstrap modal failed, falling back to show element:', e);
      modalEl.style.display = 'block';
    }
  } else {
    // fallback UI: show a lightweight custom form modal or open a new page
    // (you already have custom invite modal code — implement similarly)
    alert('Add Job form/modal not present in HTML. Add #addJobModal or implement fallback UI.');
  }
}
...
function addReviewButton(jobId, interviewId) {
  const btn = document.createElement("button");
  btn.className = "btn btn-sm btn-outline-info";
  btn.textContent = "Review Attempts";
  btn.addEventListener("click", () => {
    window.open(`/recruiter/review/?interview=${interviewId}`, "_blank");
  });
  document.querySelector("#jobDetails .d-flex")?.appendChild(btn);
}

qs('#reviewAttemptsBtn')?.addEventListener('click', async () => {
  if (!selectedJob) {
    showToast('Select a job first', 'error');
    return;
  }

  // fetch last created interview for this job
  const res = await apiFetchSimple(`/api/interviews/recruiter/`);
  if (!res.ok || !Array.isArray(res.data)) {
    showToast('No interviews found for this job', 'error');
    return;
  }

  // filter interview by job id
  const interview = res.data.find(i => String(i.job) === String(selectedJob.id));
  if (!interview) {
    showToast('No interview found for this job', 'error');
    return;
  }

  // redirect to review page
  window.open(`/interviews/recruiter/review/?interview=${interview.id}`, '_blank');
});
function showInviteModal({ jobId, candidateId, candidateName }) {
