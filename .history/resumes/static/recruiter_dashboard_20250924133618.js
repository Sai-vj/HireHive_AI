...
function ensureInviteBindings() {
  ...
  // SANITIZE candidateId: accept only pure digits
...
// ensure jobId always stored if present
if (jobId) {
  modal.dataset.jobId = String(jobId);
  if (jobDisplay) jobDisplay.value = `Job ID: ${jobId}`;
} else {
  // if no jobId provided, preserve existing modal.dataset.jobId (do not delete here)
  // so the send handler can still use a previously selected job if applicable
}
...
function injectInviteButtonsIntoAppRows(selector = '.application-row') {
  ...
  // robust job id resolution (try several fallbacks)
  const el = e.currentTarget;
  let jd = el.dataset.jobId || jobId || (window.state && window.state.selectedJobId) || null;

  // try nearest ancestor attribute [data-job-id]
  if (!jd) {
