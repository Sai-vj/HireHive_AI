...
// delegate invite open
  // ============== INVITE MODAL ==============
// ======= Robust Invite Modal Controller (replace existing invite modal patch) =======
(function(){
  // DOM refs
  ...
  async function sendInvite(){
    const jobId = inviteContext.jobId;
    const candidate_id = (inviteCandidateIdInput && inviteCandidateIdInput.value.trim()) || inviteContext.candidateId || '';
    const scheduled_at = inviteScheduledAtInput ? inviteScheduledAtInput.value : '';
    const message = (inviteMessageInput && inviteMessageInput.value.trim()) || 'You are invited for an interview';

    if (!candidate_id) { alert('Enter candidate id'); return; }
    if (!jobId) {
      if (!confirm('No job selected. Open Interviews page to create/select interview?')) return;
  ...
  if (inviteSendBtn) inviteSendBtn.addEventListener('click', sendInvite);

  // expose global helper so other code can call window.showInviteModal(jobId, candidateId, candidateName)
