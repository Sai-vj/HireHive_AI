...
async function loadJobs() {
  ...
  jobs.forEach(j => {
    ...
    // Left content (title + meta + bottom actions)
    const content = document.createElement('div');
    ...
    titleHtml.innerHTML = `
      <h4 style="margin:0 0 6px 0; font-size:1rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
        ${escapeHtml(j.title || '')}
      </h4>
    ...
    // Buttons container placed at bottom of card
    ...
    // clicking title area opens detail
    titleHtml.addEventListener('click', () => openJobDetail(j.id));
...
function attachJobCardEvents() {
  ...
  // Make entire card clickable to open job (but avoid when clicking buttons)
  qsa('.job-card').forEach(card => {
    if (card._cardBound) return; card._cardBound = true;
    card.addEventListener('click', (ev) => {
      const btn = ev.target.closest('button');
      if (btn) return; // clicked an action button; ignore
