...
async function fetchAttemptCount(jobId) {
  ...
  for (const u of tries) {
    try {
      ...
      // continue trying if 404/405
      if (r.status && (r.status === 404 || r.status === 405)) continue;
      // otherwise if 200 but unexpected, try to infer
      if (r.ok) {
        console.warn('fetchAttemptCount: unexpected data', u, data);
  ...
  async function autoSubmitQuiz(modal) {
    try {
      ...
      // after a successful submit (inside try/catch where you got `data`)
if (r.ok) {
  const data = await safeParseJSON(r); // or await r.json()
  // existing handling (update UI, show result, etc.)
  handleQuizSubmitResponse(jobId, data);

  // stop timer if any
  const modal = document.querySelector('#quiz-modal');
  if (modal && modal._quizTimerInterval) {
  ...
  // optional: show a small toast confirming
  showToast(`Submitted — score: ${data.score}/${data.total}`, 'success', 3000);
  return;
}
  }
  async function onQuizSubmit(jobId, answers) {
  try {
    ...
    const text = await r.text().catch(()=>null);
    // If server returned HTML (starts with "<"), print it for debugging
    if (!r.ok) {
