...
// Features:
// - load jobs, view job details, take quiz (timed), apply if passed
// - upload/delete resumes, list resumes
// - view quiz attempts, attempt history modal
// - my applications (list, remove), export CSV
// - invites modal (list, accept/decline/start) and inline invites
// - robust API path fallbacks and defensive parsing
//
// Required DOM IDs (create in your HTML):
// #jobsList, #jobDetailModal (optional will be created), #resumeList, #resumeFile, #uploadBtn,
// #myApplicationsList, #applyModal, #applyForm, #applyResumeSelect, #applyMessage, #applySubmitBtn,
// #invitesModal, #invitesList, #attempts-modal (optional will be created),
// #refreshJobs, #refreshMyAppsBtn, #exportMyAppsBtn, #saveTokenBtn, #tokenInput
//
// Requires a utils.js providing fetchWithAuth(url, options) and apiFetchAsJson(url, options) OR
// you can replace those calls with your own fetch wrapper.
import {
  saveTokens,
  clearTokens,
  getAccessToken,
...
(function () {
  ...
  async function fetchWithAuthFallback(url, options = {}) {
    ...
    const opts = Object.assign({}, options, { headers, credentials: 'same-origin' }); // <- added credentials
  ...
  async function apiFetch(url, opts = {}) {
    ...
    try {
      if (hasUtils) {
        // we imported apiFetch above as apiFetch, but to be defensive:
  ...
  function showToast(msg, type = 'info', timeout = 3500) {
    ...
    // minimal DOM toast (non-blocking)
  ...
  function renderQuizQuestions(questions) {
    ...
    qWrap.innerHTML = questions.slice(0, 50).map(q => {
      // q.choices could be array or object
  ...
  async function submitQuizAttempt(auto = false) {
    ...
    try {
      ...
      if (!res.ok) {
        // try alternative endpoint: /api/quiz/{jobId}/attempt/
      ...
      // update UI based on result (body)
      ...
      // refresh attempts & applications UI
  ...
  async function loadMyApplications() {
    ...
    // filter by current user if possible (JWT)
    const token = localStorage.getItem('token') || '';
    ...
    apps.forEach(a => {
      ...
      div.querySelector('.remove-app-btn').addEventListener('click', async () => {
        ...
        // API might accept DELETE /api/applications/{id}/ or POST to withdraw
        const tryUrls = [
        ...
        if (!ok) {
          // fallback: try POST to a withdraw endpoint
      ...
      div.querySelectorAll('.start-invite').forEach(b => b.addEventListener('click', async () => {
        ...
        try {
          ...
          if (r.ok && (r.data?.redirect_url || r.data?.join_url || r.data?.url)) {
            ...
            // fallback: navigate to candidate interview page (use INTERVIEWS_BASE constant)
  ...
  // ---------- Invite helpers: view, respond, start ----------
/**
 * viewInvite(inviteId)
 * - Shows a modal with invite details.
 * - Tries HTMX fragment endpoint first, then falls back to JSON endpoint.
 */
async function viewInvite(inviteId) {
  ...
  if (!modal) {
    ...
    modal.style = 'position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.45);z-index:99999';
    modal.innerHTML = `<div style="background:#fff;padding:14px;border-radius:8px;max-width:720px;width:94%;"><div id="inviteDetailContent">Loading...</div><div style="text-align:right;margin-top:10px"><button id="inviteDetailClose" class="btn btn-secondary">Close</button></div></div>`;
    document.body.appendChild(modal);
    modal.querySelector('#inviteDetailClose').addEventListener('click', () => modal.style.display = 'none');
  }
  modal.style.display = 'flex';
  ...
  try {
    // try fragment endpoint (human-friendly HTML)
    ...
    // ignore fragment errors and fallback to JSON
    console.debug('Fragment fetch failed, falling back to JSON', e);
  }

  try {
    // fallback: JSON API
    const apiUrl = API.INVITES.replace(/\/candidate\/?$/, '') + `/${inviteId}/`; // try to build /api/interviews/candidate/<id>/ or adjust below
    // but safest: call list and find invite
    ...
    modal.querySelector('#inviteAcceptBtn')?.addEventListener('click', () => { respondInvite(inviteId, 'accept'); modal.style.display='none'; });
    modal.querySelector('#inviteDeclineBtn')?.addEventListener('click', () => { respondInvite(inviteId, 'decline'); modal.style.display='none'; });
 ...
 * - sends JSON { response: 'accept' } and reloads invites
 */
...
/**
 * startInterview(interviewId, inviteId)
 * - Calls start API and redirects to candidate interview page
 * - inviteId optional (server uses it to validate scheduled time)
 */
async function startInterview(interviewId, inviteId = null) {
  if (!interviewId) return showToast('Interview id missing', 'error');

  try {
    const r = await apiFetch(API.START_INTERVIEW(interviewId), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!r.ok) {
      ...
      // if server provided scheduled_start (they often do on forbidden), show it
    ...
    // server may return attempt_id or a redirect/join URL
    const data = r.data || {};
    ...
    // otherwise redirect to candidate interview page where attempt will be loaded
    // uses INTERVIEWS_BASE constant from your file
    ...
    // fallback: open candidate page anyway
    window.location.href = `${INTERVIEWS_BASE}/page/candidate/${interviewId}/` + (inviteId ? `?invite=${inviteId}` : '');
  }
}

// Export to window so event handlers can call them (and console debugging)
...
(function autoSaveTokenFromInput() {
  try {
    ...
    // auto-hide if already token present
    if (getAccessToken()) {
      if (tokenInputEl) tokenInputEl.style.display = 'none';
      if (saveBtn) saveBtn.style.display = 'none';
    }
...
if (saveBtn) {
  saveBtn.addEventListener('click', async () => {
    ...
    // try parse JSON { access, refresh } first
    try {
      const parsed = JSON.parse(val);
      ...
      // if parsed but no access, fallthrough to treat as raw token below
    } catch (e) {
      // not JSON: treat as raw access token
    }

    // treat as raw access token (no refresh)
    saveTokens({ access: val, refresh: null });
    ...
    // hide apply modal click outside
...
(function autoAuthInit() {
  try {
    // hide token input if tokens present
    const tokenInputEl = document.getElementById('tokenInput');
    const saveBtn = document.getElementById('saveTokenBtn');
    ...
    // attempt to refresh access token in background (non-blocking)
    refreshAccessToken().catch(()=>{ /* ignore */ });
  } catch (e) {
  ...
  loadInvites(); // actually fetch + render
});
