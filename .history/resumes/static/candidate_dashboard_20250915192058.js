...
document.addEventListener('submit', async function (e) {
 ...
 try {
  ...
  let res = await apiFetch(APPLY_URL, {
    ...
    body: { job_id: jobId, resume_id: resumeId, message }
  });

  // If server rejected JSON (415), retry with FormData
  if (res && res.status === 415) {
    console.warn('Server rejected JSON — retrying with FormData');
  ...
  if (res.ok) {
    ...
    // hide modal + refresh UI (keep your old code here)
  } else {
    // your existing error/409 handling here
  }
