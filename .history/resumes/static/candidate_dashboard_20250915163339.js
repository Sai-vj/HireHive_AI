...
import { apiFetchAsJson, getAccessToken, refreshAccessToken } from '/static/js/utils.js'; // path you used
...
async function initDashboard() {
  ...
  // call dashboard API (apiFetchAsJson will use saved tokens if you implemented)
  const res = await apiFetchAsJson('/accounts/api/dashboard/');
  if (!res.ok) {
    console.error('dashboard fetch failed', res);
    document.getElementById('dashboardArea').innerText = 'Unable to load dashboard';
    return;
  }
  const data = res.data;
...
function renderDashboard(data) {
  // simple example: show username and progress
  const root = document.getElementById('dashboardArea');
  root.innerHTML = `
    <h2>Welcome, ${data.username}</h2>
    <p>Role: ${data.role}</p>
    <div>Profile completion: ${data.progress.profile_complete}%</div>
    <div>Applications: ${data.progress.applications_submitted}</div>
  `;
  // you can build nice cards, charts etc later
}
