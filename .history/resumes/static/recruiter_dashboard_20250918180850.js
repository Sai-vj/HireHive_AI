...
// recruiter_dashboard.js (PATCHED) - drop-in replacement
// Backup original before replacing: recruiter_dashboard.old.js
...
(function () {
  ...
  function headersWithToken(contentType = 'application/json') {
    const t = getToken();
    const h = {};
  ...
  function showToast(msg, type='info'){ // simple toast: type = success/info/danger
  ...
  // ---------- DOM nodes (safe) ----------
  const jobsList = el('jobsList') || el('jobsListContainer') || el('interviews-container') || el('interviewsContainer');
  const jobDetailsPane = el('jobDetails') || el('applicationsSection') || el('rightPane') || null;
  const inviteModal = el('inviteModal');
  const inviteCandidateInput = el('invite_candidate_id') || el('modalCandidateId');
  const inviteScheduledInput = el('invite_scheduled_at') || el('modalScheduledAt');
  const inviteMessageInput = el('invite_message') || el('modalMessage');
  const inviteSendBtn = el('inviteSendBtn') || el('modalSendBtn');
  const inviteCancelBtn = el('inviteCancelBtn') || el('inviteCancelBtn');

  if(!jobsList){
    console.error('recruiter_dashboard.js: jobs container not found. Add <div id="jobsList"></div> to template.');
    // create a fallback container at top
    const fallback = document.createElement('div');
    fallback.id = 'jobsList';
    document.body.prepend(fallback);
  }

  // ---------- Render helpers ----------
  function renderInterviewCard(i){
    const scheduled = i.scheduled_at ? new Date(i.scheduled_at).toLocaleString() : 'Not set';
    const status = i.is_active ? 'Active' : 'Inactive';
    const inviteStatus = i.invite_status || '-';
    const titleEsc = escapeHtml(i.title || `Interview ${i.id}`);
    return `
      <div class="card mb-2 p-2" data-interview-id="${i.id}">
        <div class="d-flex justify-content-between align-items-start">
          <div>
            <h6 class="mb-0">${titleEsc}</h6>
            <div class="small-muted">Scheduled: ${escapeHtml(scheduled)}</div>
            <div class="small-muted">Mode: ${escapeHtml(i.mode||'')} • Status: ${escapeHtml(status)}</div>
            <div class="small-muted">Invite: <span class="invite-status">${escapeHtml(inviteStatus)}</span></div>
          </div>
          <div>
            <button class="btn btn-sm btn-outline-primary me-1" data-action="view" data-id="${i.id}">View</button>
            <button class="btn btn-sm btn-outline-success me-1" data-action="generate" data-id="${i.id}">Generate Quiz</button>
            <button class="btn btn-sm btn-outline-secondary" data-action="open" data-id="${i.id}">Open</button>
          </div>
        </div>
      </div>
    `;
  ...
  // safe place to render HTML
  function setJobsHtml(html){
    const container = document.getElementById('jobsList') || document.getElementById('interviews-container');
    if(!container){
      console.warn('jobs container missing at render time, creating one.');
      const c = document.createElement('div'); c.id = 'jobsList'; document.body.prepend(c);
  ...
  async function loadInterviews(){
    setJobsHtml('<div class="small-muted">Loading interviews…</div>');
    const url = '/api/interviews/recruiter/'; // recruiter list endpoint
    try {
      const res = await fetch(url, { headers: headersWithToken() });
      if(!res.ok){
        const text = await res.text().catch(()=>null);
        setJobsHtml(`<div class="text-danger">Failed to load interviews (${res.status})</div>`);
        console.error('loadInterviews failed', res.status, text);
        return;
      }
      const data = await res.json();
      if(!Array.isArray(data) || data.length === 0){
        setJobsHtml('<div class="small-muted">No interviews yet.</div>');
        return;
      }
      setJobsHtml(data.map(renderInterviewCard).join(''));
      attachJobsListeners(); // wire buttons
    } catch(err){
      console.error('loadInterviews err', err);
      setJobsHtml('<div class="text-danger">Network error while loading interviews</div>');
    }
  }

  // ---------- Attach event listeners to the job cards (delegation) ----------
  function attachJobsListeners(){
    const container = document.getElementById('jobsList') || document.getElementById('interviews-container');
    if(!container) return;
    container.querySelectorAll('[data-action]').forEach(btn=>{
      btn.removeEventListener('click', jobActionHandler);
      btn.addEventListener('click', jobActionHandler);
    });
  }

  function jobActionHandler(e){
    ...
    if(action === 'view') {
      ...
      // placeholder: call generate endpoint
      generateQuizForInterview(id);
    } else if(action === 'open'){
      window.location.href = `/api/interviews/recruiter/${id}/`;
    }
  }

  // ---------- Generate quiz (placeholder) ----------
  async function generateQuizForInterview(interviewId){
    try {
      const url = `/api/interviews/recruiter/${interviewId}/generate_questions/`;
      const res = await fetch(url, { method:'POST', headers: headersWithToken() });
      if(!res.ok) { showToast('Generate failed', 'danger'); return; }
      showToast('Questions generation started', 'success');
    } catch(e){ console.error(e); showToast('Network error', 'danger'); }
  }

  // ---------- Show applications for interview/job ----------
  async function showApplications(interviewId){
    // show details pane
    const details = jobDetailsPane || document.getElementById('jobDetails') || (() => {
      const wrapper = document.createElement('div');
      wrapper.id = 'jobDetails';
      document.querySelector('.col-md-8')?.appendChild(wrapper);
      return wrapper;
    })();

    details.style.display = 'block';
    details.innerHTML = `<h5>Applications for interview ${escapeHtml(interviewId)}</h5><div id="appsList">Loading applications…</div>`;

    // Try multiple possible endpoints (adjust if your API uses different path)
    const tokenHdr = headersWithToken();
    const candidateAppsUrl1 = `/api/resumes/jobs/${interviewId}/applications/`;
    const candidateAppsUrl2 = `/api/interviews/${interviewId}/applications/`;
    const candidateAppsUrl3 = `/api/resumes/dashboard/recruiter/${interviewId}/applications/`;
    let res = await fetch(candidateAppsUrl1, { headers: tokenHdr });
    if(!res.ok) res = await fetch(candidateAppsUrl2, { headers: tokenHdr });
    if(!res.ok) res = await fetch(candidateAppsUrl3, { headers: tokenHdr });

    if(!res.ok){
      const txt = await res.text().catch(()=>null);
      document.getElementById('appsList').innerHTML = `<div class="text-danger">Failed to load applications (${res.status})</div>`;
      console.error('showApplications failed', res.status, txt);
      return;
    }

    const apps = await res.json();
    if(!apps || apps.length === 0){
      document.getElementById('appsList').innerHTML = '<div class="small-muted">No applications</div>';
      return;
    }

    // Render applications - map common field names
    const html = apps.map(a => {
      // derive candidate id and name from common shapes:
      let candId = a.candidate_id || (a.candidate && (a.candidate.id || a.candidate.user_id)) || a.resume_id || a.id || '';
      let candName = (a.candidate && (a.candidate.name || a.candidate.full_name)) || a.candidate_name || a.resume_name || '';
      const appliedAt = a.applied_at || a.created_at || '';
      return `
        <div class="card mb-2 p-2 d-flex justify-content-between" id="app-${escapeHtml(String(a.id||candId))}">
          <div>
            <div><strong>${escapeHtml(candName || ('Candidate ' + candId))}</strong>
              <small class="small-muted">(${escapeHtml(candId)})</small></div>
            <div class="small-muted">Applied: ${escapeHtml(appliedAt)}</div>
            <div class="small-muted">Message: ${escapeHtml(a.message || '')}</div>
          </div>
          <div class="text-end">
            <button class="btn btn-sm btn-outline-primary me-1" onclick="window.open('/resumes/${escapeHtml(candId)}','_blank')">View Resume</button>
            <button class="btn btn-sm btn-secondary" onclick="openInviteForApplication(${escapeHtml(interviewId)}, '${escapeHtml(candId)}', '${escapeHtml(candName||'')}')">Send Invite</button>
          </div>
        </div>
      `;
    }).join('');
  ...
  // Make available globally for inline onclick usage
  window.showApplications = showApplications;

  // ---------- Invite modal handling ----------
  // openInviteForApplication: prefill and show
  window.openInviteForApplication = function(interviewId, candidateId, candidateName){
    if(!inviteModal || !inviteCandidateInput || !inviteScheduledInput || !inviteMessageInput || !inviteSendBtn){
      // if modal elements missing, fallback to prompt
      const scheduled = prompt('Scheduled at (YYYY-MM-DDTHH:MM):', new Date(Date.now()+24*3600*1000).toISOString().slice(0,16));
      const message = prompt('Message:', `Hi ${candidateName || ''}, you are invited for interview ${interviewId}`);
      if(scheduled !== null){
        sendInvite(interviewId, candidateId, scheduled, message).then(r=>{
          if(r.ok){ showToast('Invite sent', 'success'); } else showToast('Invite failed', 'danger');
        });
      }
      return;
    }
    // save interview id on modal root for send handler
    inviteModal.dataset.interviewId = interviewId;
    inviteCandidateInput.value = candidateId || '';
    // default scheduled time = next day 10am (local)
    const d = new Date(Date.now() + 24*3600*1000); d.setHours(10,0,0,0);
    inviteScheduledInput.value = inviteScheduledInput.value || d.toISOString().slice(0,16);
    inviteMessageInput.value = inviteMessageInput.value || `Hi ${candidateName || ''}, recruiter invited you for interview on ${inviteScheduledInput.value}`;
    inviteModal.classList.remove('d-none');
    // scroll into view
    inviteModal.scrollIntoView({behavior:'smooth'});
  };
  ...
  if(inviteCancelBtn){
    inviteCancelBtn.addEventListener('click', (e)=>{ hideInviteModal(); });
  }

  // sendInvite function returns {ok, status, data}
  async function sendInvite(interviewId, candidateId, scheduled_at, message){
    const url = `/api/interviews/recruiter/${interviewId}/invite/`;
    try {
      const res = await fetch(url, {
        method: 'POST',
        headers: headersWithToken(),
        body: JSON.stringify({ candidate_id: candidateId, scheduled_at, message })
      });
      const data = await res.json().catch(()=>null);
      if(res.ok){
        // update invite-status in UI if present
        const elCard = document.querySelector(`[data-interview-id="${interviewId}"]`);
        if(elCard){
          const statusEl = elCard.querySelector('.invite-status');
          if(statusEl) statusEl.innerText = 'Pending';
        }
        hideInviteModal();
        showToast('Invite sent', 'success');
      } else {
        const detail = (data && (data.detail || JSON.stringify(data))) || `Status ${res.status}`;
        showToast(`Invite failed: ${detail}`, 'danger');
      }
      return { ok: res.ok, status: res.status, data };
    } catch(e){
      console.error('sendInvite error', e);
      showToast('Network error while sending invite', 'danger');
  ...
  // wire send button
  if(inviteSendBtn){
    inviteSendBtn.addEventListener('click', async (e)=>{
      const interviewId = inviteModal?.dataset?.interviewId || (window.__inviteInterviewId) || null;
      const candidateId = (inviteCandidateInput && inviteCandidateInput.value) || null;
      const scheduled_at = (inviteScheduledInput && inviteScheduledInput.value) || null;
      const message = (inviteMessageInput && inviteMessageInput.value) || '';

      if(!interviewId || !candidateId){
        showToast('Interview id and candidate id required', 'danger');
        return;
      }
      await sendInvite(interviewId, candidateId, scheduled_at, message);
    });
  }

  // ---------- Init ----------
  function init(){
    // load interviews on start
    loadInterviews();
    ...
    // save token UI (if present)
    const saveTokenBtn = el('saveTokenBtn') || el('saveToken');
    if(saveTokenBtn){
      const tokenInput = el('tokenInput');
      saveTokenBtn.addEventListener('click', ()=> {
        const t = tokenInput?.value?.trim();
        if(!t){ showToast('Paste access token first', 'danger'); return; }
        localStorage.setItem('token', t);
        showToast('Saved token', 'success');
        loadInterviews();
      });
    }
  }
  ...
  window._rd_debug = { loadInterviews, showApplications, sendInvite };
})();
