...
const res = await apiFetchAsJson(apiStart, {
  method: 'POST',
  body: { invite: inviteId || null } // utils will stringify
});

if (!res.ok) {
  const body = res.data;
  const detail = (body && (body.detail || body.error)) || `Status ${res.status}`;
  const scheduled = body && body.scheduled_start ? `Starts at: ${new Date(body.scheduled_start).toLocaleString()}` : '';
  // show toast or message div
  showToast?.(`Cannot start: ${detail}${scheduled ? ' — ' + scheduled : ''}`, 'error', 6000);
  // or fallback:
  const msgEl = document.getElementById('message');
  if (msgEl) msgEl.innerHTML = `<div class="alert alert-danger">Cannot start: ${escapeHtml(detail)}${scheduled ? '<br/>'+escapeHtml(scheduled) : ''}</div>`;
  return { ok: false, res };
}
