...
def job_(request, pk):
    ...
    # PUT/PATCH -> update job
        ...
        if not request.user.is_recruiter:  # adjust if you use is_recruiter helper
            return Response({"error": "Only recruiters can update jobs"}, status=403)
    ...
    # DELETE -> remove job
        ...
        if not request.user.is_recruiter:
            return Response({"error": "Only recruiters can delete jobs"}, status=403)
        job.delete()
