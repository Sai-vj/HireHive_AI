...
def recruiter_invite_candidate_by_job(request, job_pk):
    ...
    resume_id_raw = request.data.get('resume_id')
    ...
    # If not found, fallback: maybe it's actually a resume id
    if not candidate and (resume_id_raw or candidate_id_raw):
        Resume = apps.get_model('resumes', 'Resume')
        resume_pk = resume_id_raw or candidate_id_raw
    ...
    return Response(InterviewInviteSerializer(invite, context={'request': request}).data,
