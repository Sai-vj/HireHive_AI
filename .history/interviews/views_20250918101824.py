...
@api_view(['GET', 'POST'])
@permission_classes([IsAuthenticated])
def recruiter_addquestions(request, pk):
    if not is_recruiter(request.user):
        return Response({"detail": "Forbidden"}, status=status.HTTP_403_FORBIDDEN)

    # fetch Interview instance (important: pass instance, not pk)
    ...
    serializer = InterviewQuestionSerializer(data=request.data, many=many, context={'request': request})
    serializer.is_valid(raise_exception=True)

    # PASS the Interview instance (NOT the pk)
