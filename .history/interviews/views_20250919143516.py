...
def candidate_invite_respond(request, invite_id):
    """
    POST /api/interviews/candidate/invites/<invite_id>/respond/
    body: { "action": "accept" } or { "action": "decline" }
    accepts 'response' too for compatibility.
    """
    invite = get_object_or_404(InterviewInvite, pk=invite_id, candidate=request.user)
    action = (request.data.get('action') or request.data.get('response') or '').strip().lower()
    if action not in ('accept', 'decline'):
        return Response({"detail": "Invalid action"}, status=status.HTTP_400_BAD_REQUEST)

    invite.status = 'accepted' if action == 'accept' else 'declined'
    invite.save(update_fields=['status', 'updated_at'])
    return Response({"detail": invite.status}, status=status.HTTP_200_OK)
...
def candidate_interview_page(request, pk):
    ...
    return render(request, 'candidate_interview.html', {})
