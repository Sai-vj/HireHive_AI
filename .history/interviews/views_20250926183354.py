...
def generate_interview_questions_backend(interview, count, auto_generate=True):
    """
    Real save: create InterviewQuestion rows for this interview.
    Returns: {"ok": True, "questions": [<created_ids>]}
    """
    topics = ["Python", "Django", "REST", "SQL", "Algorithms", "Data Structures"]
    ...
    for i in range(int(count)):
        ...
        # sample MCQ
        q_text = f"{topic}: Sample question {i+1}"
        choices = [
            "Option A", "Option B", "Option C", "Option D"
        ]
        answer = "Option A"  # demo

        q = InterviewQuestion.objects.create(
            interview=interview,
            question_text=q_text,
            question_type="mcq",
            choices=choices,             # JSONField: list is fine
            answer=answer,
            difficulty=diff,
            topic=topic,
            generated_by="ai",
    ...
    return {"ok": True, "questions": created_ids}
...
@api_view(['GET'])
@permission_classes([AllowAny])  # change to IsAuthenticated if you want only logged-in candidates
def candidate_interview_questions(request, pk):
    """
    Candidate-facing list of questions for interview `<pk>`.
    Returns a JSON object: { "questions": [ ... ] }
    Filters by common visibility flags if they exist on the model.
    """
    interview = get_object_or_404(Interview, pk=pk)

    qs = InterviewQuestion.objects.filter(interview=interview)

    # If your model uses a 'status' field, only expose published/active ones:
    if hasattr(InterviewQuestion, 'status'):
        qs = qs.filter(status__in=['published', 'active', 'visible', 'approved'])

    # If there are boolean flags:
    if hasattr(InterviewQuestion, 'is_published'):
        qs = qs.filter(is_published=True)
    if hasattr(InterviewQuestion, 'private'):
