...
@api_view(['GET'])
@permission_classes([AllowAny])  # or change to IsAuthenticated if you require login
def candidate_interview_questions(request, pk):
    """
    Candidate-facing list of questions for interview `<pk>`.
    Returns only visible/published questions where possible.
    """
    interview = get_object_or_404(Interview, pk=pk)
    ...
    # - if model uses 'status' field, return only 'published' or 'active'
    if hasattr(InterviewQuestion, 'status'):
        qs = qs.filter(status__in=['published', 'active', 'visible', 'approved'])

    # If model uses boolean flags:
    if hasattr(InterviewQuestion, 'is_published'):
        qs = qs.filter(is_published=True)
    if hasattr(InterviewQuestion, 'private'):
        qs = qs.filter(private=False)

    qs = qs.order_by('id')  # optional: stable order

    ser = InterviewQuestionSerializer(qs, many=True, context={'request': request})
