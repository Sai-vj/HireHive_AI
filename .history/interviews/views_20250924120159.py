...
def recruiter_invite_candidate_by_job(request, job_pk):
    ...
    # Debug: show what backend receives
    try:
        print("DEBUG invite payload:", request.data)
    except Exception:
        pass

    # Accept either candidate_id (user pk) OR resume_id
    candidate_id_raw = request.data.get('candidate_id') or request.data.get('candidate') or None
    resume_id_raw = request.data.get('resume_id') or request.data.get('resume') or None

    # Normalize ints where possible
    candidate = None
    try:
        if candidate_id_raw not in (None, '', []):
            candidate_pk = int(candidate_id_raw)
            candidate = User.objects.filter(pk=candidate_pk).first()
    except Exception:
        candidate = None

    # Fallback: if resume_id provided or candidate not found, try resolving Resume -> user
    ...
    # Final: if still not found, return debug response
    if not candidate:
        # helpful debug info for frontend mismatch
        return Response({
            "error": "Candidate not found",
            "received": {
                "candidate_id_raw": candidate_id_raw,
                "resume_id_raw": resume_id_raw,
                "user_exists_for_candidate_id": bool(candidate_id_raw and User.objects.filter(pk=candidate_id_raw).exists())
            }
        }, status=status.HTTP_400_BAD_REQUEST)

    # proceed to create invite (same as before)
