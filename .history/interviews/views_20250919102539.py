...
from resumes.models import Job  # wherever your Job model lives
from .models import Interview
from .permissions import IsRecruiter  # your helper used before

@api_view(['POST'])
@permission_classes([IsAuthenticated])
def recruiter_create_interview_for_job(request, job_pk):
    if not is_recruiterrecruiter(request.user):
        return Response({"detail":"Forbidden"}, status=status.HTTP_403_FORBIDDEN)

    job = get_object_or_404(Job, pk=job_pk)  # optionally require created_by

    data = request.data.copy()
    data['job'] = job.pk
    # optionally set created_by server-side instead of client
    serializer = InterviewCreateSerializer(data=data)
    if serializer.is_valid():
        interview = serializer.save(created_by=request.user)  # set owner
        return Response(InterviewCreateSerializer(interview).data, status=status.HTTP_201_CREATED)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
