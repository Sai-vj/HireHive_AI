...
def generate_questions_view(request, pk):
    ...
    try:
        ...
        return Response({"detail": "Generation started", "task_id": getattr(task, 'id', None)}, status=status.HTTP_202_ACCEPTED)
    except Exception:
        ...
        try:
            ...
            logger.exception("generate_questions fallback failed: %s", e2)
...
def recruiter_review_attempts(request, job_pk=None):
    """
    GET /api/interviews/recruiter/review/?job_id=<job_id>
    Recruiter can see all attempts for a job (all interviews under that job).
    """
    if not is_recruiter(request.user):
        return Response({"detail": "Forbidden"}, status=status.HTTP_403_FORBIDDEN)
    ...
    # fetch all interviews under this job
    ...
    return render(request{"count": len(data), "attempts": data})
