...
def start_interview_attempt(request, pk):
    ...
    Enforces scheduled start/end window. If an invite id is provided in request.data['invite'],
    the invite's scheduled_at will be used (and validated for this candidate).
    """
    interview = get_object_or_404(Interview, pk=pk)
    ...
    # Resolve invite (if provided) and prefer invite scheduled time if valid
    invite_id = request.data.get('invite') or None
    invite_obj = None
    if invite_id:
        try:
            invite_obj = InterviewInvite.objects.get(pk=invite_id)
            # ensure invite is for this interview and this candidate
            if invite_obj.interview and invite_obj.interview.id != interview.id:
                invite_obj = None
            elif invite_obj.candidate and invite_obj.candidate != request.user:
                # not the candidate's invite -> ignore invite (do not allow)
                invite_obj = None
        except InterviewInvite.DoesNotExist:
            invite_obj = None
            
    if request.user.is_staff and request.GET.get('force') == '1':
        scheduled_start = None  # allow staff to force start


    # Determine scheduled start
    scheduled_start = None
    # check invite scheduled_at first
    if invite_obj and getattr(invite_obj, 'scheduled_at', None):
        scheduled_start = invite_obj.scheduled_at
    # then interview-level scheduled fields (common names)
    ...
    # Determine scheduled end: prefer explicit end_time field else compute using duration
    scheduled_end = None
    ...
    # Enforce start window if scheduled_start exists
    if scheduled_start:
        # If now is before scheduled start -> block
        if now < scheduled_start:
            return Response({'detail': 'Cannot start before scheduled time', 'scheduled_start': scheduled_start}, status=status.HTTP_403_FORBIDDEN)
    # Enforce end window if scheduled_end exists
    if scheduled_end:
    ...
    # ----- select questions (existing logic kept) -----
    TOTAL_Q = 25
    ...
    mcq_pool = list(q_qs.filter(question_type__in=['mcq', 'objective', 'single_choice', 'multiple_choice'])) if q_qs.exists() else []
    coding_pool = list(q_qs.filter(question_type__in=['coding', 'code', 'programming'])) if q_qs.exists() else []
    ...
    if (len(mcq_pool) + len(coding_pool)) < TOTAL_Q:
        ...
        global_mcq = list(all_qs.filter(question_type__in=['mcq', 'objective', 'single_choice', 'multiple_choice']))
        global_coding = list(all_qs.filter(question_type__in=['coding', 'code', 'programming']))
    ...
    for q in selected:
        ...
        questions_out.append({
            ...
            'type': getattr(q, 'question_type', None) or 'mcq',
    ...
    # persist selected question ids and duration on attempt where possible
    try:
...
now = timezone.now()
print("DEBUG start_attempt: now=", now, "scheduled_start=", scheduled_start)
