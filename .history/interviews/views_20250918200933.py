...
# Recruiter: invite candidate (API)
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def recruiter_invite_candidate(request, pk):
    if not is_recruiter(request.user):
        return Response({"detail":"Forbidden - not a recruiter"}, status=status.HTTP_403_FORBIDDEN)

    # 1) Try to get Interview with this pk (common case)
    interview = None
    try:
        interview = Interview.objects.get(pk=pk, created_by=request.user)
    except Interview.DoesNotExist:
        # 2) Maybe frontend passed a JOB id instead of interview id.
        # Try to find the Job and resolve an interview for it.
        try:
            job = Job.objects.get(pk=pk, created_by=request.user)
        except Job.DoesNotExist:
            return Response(
                {"detail": "Interview not found for this recruiter (tried as interview id and job id)"},
                status=status.HTTP_404_NOT_FOUND
            )

        # Prefer job.default_interview_id if you store one, otherwise pick the first interview
        # Adjust to your model: if job has a field job.default_interview_id, use that. Otherwise pick first.
        interview = None
        # if job has a saved default id (example)
        default_id = getattr(job, "default_interview_id", None)
        if default_id:
            interview = Interview.objects.filter(pk=default_id, created_by=request.user).first()

        if not interview:
            # fallback: pick any interview linked to job and created by this recruiter
            interview = job.interviews.filter(created_by=request.user).first()
            if not interview:
                # last attempt: any interview linked to job (even if created by someone else)
                interview = job.interviews.first()

        if not interview:
            return Response({"detail": "No interview is linked to this job. Create one first."},
                            status=status.HTTP_400_BAD_REQUEST)

    # At this point we have a valid interview object
    ...
    # Optional: validate scheduled_at format here (ISO 8601)
    # If your InterviewInvite model expects datetime field, Django will parse ISO string on save if passed
    ...
    try:
        ...
        # fallback: ignore or send sync (do not crash)
        pass
