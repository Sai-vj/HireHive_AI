...
def start_interview_attempt(request, pk):
    """
    Start or return existing attempt for the authenticated candidate.
    Picks up to `n_questions` published questions (question_type used, not 'kind').
    Stores a snapshot of question ids on the attempt so the same questions are served.
    """
    interview = get_object_or_404(Interview, pk=pk)

    # create or get existing attempt (do not create duplicates if candidate can retry - adjust logic if needed)
    ...
    # If attempt already had a question snapshot, use that; otherwise select questions now
    n_questions = getattr(interview, 'n_questions', 25)  # fallback or set default
    # get published questions (use question_type field name)
    qs = InterviewQuestion.objects.filter(interview=interview, status='published')

    # optional: filter by type mix, difficulty etc. here
    q_ids = list(qs.values_list('id', flat=True))
    if not q_ids:
        # no saved/published questions - return informative error
        return Response({"detail": "No questions available for this interview"}, status=status.HTTP_404_NOT_FOUND)

    # If attempt has stored snapshot of questions, use it; else sample and save snapshot
    snapshot = getattr(attempt, 'question_snapshot', None)  # adjust to your model field name
    if not snapshot:
        # shuffle and pick
        random.shuffle(q_ids)
        picked = q_ids[:n_questions]
        # Save snapshot onto attempt. Use whichever field you have - below uses `question_ids` JSON field.
        # If your model uses a different field name update accordingly.
        attempt.question_ids = picked  # ensure your InterviewAttempt model has JSONField `question_ids` or similar
        attempt.started_at = attempt.started_at or timezone.now()
        attempt.save(update_fields=['question_ids', 'started_at'])
    else:
        picked = snapshot if isinstance(snapshot, list) else snapshot

    # fetch question objects preserving order
    questions = list(InterviewQuestion.objects.filter(pk__in=picked))
    # maintain same order as picked list
    qmap = {q.id: q for q in questions}
    ordered_questions = [qmap[qid] for qid in picked if qid in qmap]

    # build payload for frontend (don't include answers if you don't want to leak)
    payload_questions = []
    for q in ordered_questions:
        payload_questions.append({
            "id": q.id,
            "question_text": q.question_text,
            "question_type": q.question_type,   # use this field
            "choices": q.choices,
            "difficulty": q.difficulty,
            "topic": q.topic,
            # DO NOT include q.answer here for MCQ if you want to prevent cheating on client-side
        })
    ...
    resp = {
        ...
        "duration_minutes": getattr(interview, 'duration_minutes', 60),
        "interview": {
            "id": interview.id,
            "title": getattr(interview, 'title', ''),
            "description": getattr(interview, 'description', ''),
        }
    }
