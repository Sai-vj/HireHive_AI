...
def is_recruiter(user):
    """
    Change this to match your project's recruiter identification logic.
    Example: user.profile.role == 'recruiter' or user.is_staff etc.
    """
    try:
        # adjust as necessary
...
def recruiter_create_list_interviews(request):
    ...
    # Determine FK field to set owner if model uses created_by or recruiter
    fk_field = None
    ...
    if 'created_by' in model_field_names:
        ...
        fk_field = 'recruiter'
    # fallback: find any FK to user model
    if not fk_field:
...
def recruiter_retrieve_update_delete_interview(request, pk):
    ...
    # ensure ownership if model has created_by
    lookup = {'pk': pk}
...
def recruiter_add_questions(request, pk):
    ...
    serializer.is_valid(raise_exception=True)
    saved = serializer.save()  # serializer.create should use context['interview']
...
def start_interview_attempt(request, pk):
    ...
    # optional: check schedule or invite/eligibility (if required)
...
def submit_interview_attempt(request, attempt_id):
    ...
    # scoring: compare submitted answers to InterviewQuestion.answer when available
...
def recruiter_question_detail(request, interview_pk, q_pk):
    ...
    serializer.is_valid(raise_exception=True)
    serializer.save()
    # optionally set created_by if published and no creator present
...
def recruiter_review_bulk(request, pk):
    """
    Body:
    { "approve": [1,2], "reject": [3], "publish": true }
    """
    if not is_recruiter(request.user):
        return Response({"detail": "Forbidden"}, status=status.HTTP_403_FORBIDDEN)
...
def recruiter_invite_candidate_by_job(request, job_pk):
    """
    POST /api/interviews/recruiter/<job_pk>/invite/
    body: { "candidate_id": <id>, "scheduled_at": "...", "message": "..." }
    """
    if not is_recruiter(request.user):
        return Response({"detail": "Forbidden"}, status=status.HTTP_403_FORBIDDEN)
    ...
    # find or create interview for job
...
def candidate_invites(request):
    """
    GET /api/interviews/candidate/invites/
    """
...
def candidate_invite_respond(request, invite_id):
    """
    POST /api/interviews/candidate/<invite_id>/respond/
    Accepts JSON body with either {"response":"accept"} or {"action":"accept"}.
    Accepts values: "accept", "accepted", "decline", "declined" (case-insensitive).
    """
    try:
    ...
    if not val and isinstance(request.data, str):
        # last-resort: sometimes body arrives as raw string
        val = request.data
...
# ----------------- Frontend redirect: start interview attempt page -----------------
@login_required
def start_interview_attempt_frontend(request, pk):
    interview = get_object_or_404(Interview, pk=pk)

    # optional: check invite exists and accepted
    invite = interview.invites.filter(candidate=request.user).first()
    if invite and invite.status != 'accepted':
        return redirect('/error/not-authorized/')  # adjust to your error page
    ...
    # redirect to your frontend attempt UI; replace with the real path in your app
    return redirect(f"/attempts/{attempt.id}/")


# ----------------- Recruiter: create interview for job (helper) -----------------
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def recruiter_create_interview_for_job(request, job_pk):
    ...
    instance = serializer.save(created_by=request.user) if 'created_by' in [f.name for f in Interview._meta.fields] else serializer.save()
...
def candidate_interview_page(request, pk):
    """
    Render the candidate-facing interview page.
    The template can read the invite query param (?invite=ID) if needed.
    """
    interview = get_object_or_404(Interview, pk=pk)
    # supply interview minimal info (or let client fetch full details via API)
    context = {
        "interview": interview,
    }
    return render(request, "candidate_interview.html", context)
