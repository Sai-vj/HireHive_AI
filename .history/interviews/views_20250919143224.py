...
def recruiter_invite_candidate_by_job(request, job_pk):
    ...
    job = get_object_or_404(Job, pk=job_pk)   # <-- during testing remove created_by check
    ...
    if interview is None:
        interview = Interview.objects.create(
            ...
            duration_minutes=45,
            # created_by=request.user,  <-- comment out if causing trouble
            is_active=True,
        )
...
from .permissions import IsRecruiter  # make sure this class exists and returns True for recruiters
...
def recruiter_create_interview_for_job(request, job_pk):
    ...
    # print("DEBUG recruiter_create_interview_for_job: user:", request.user, "data:", request.data)

    # ensure job exists
    job = get_object_or_404(Job, pk=job_pk)

    # Optional ownership check (uncomment if you require recruiter to own the job)
    # if getattr(job, 'created_by_id', None) is not None and job.created_by_id != request.user.id:
    #     return Response({"detail": "You do not own this job."}, status=status.HTTP_403_FORBIDDEN)

    data = request.data.copy()
    data['job'] = job.pk

    # Use serializer and set created_by server-side to avoid letting client spoof owner
    serializer = InterviewCreateSerializer(data=data)
    if not serializer.is_valid():
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    # Some serializers have default create signature; pass created_by if model requires it.
    interview = serializer.save(created_by=request.user) if 'created_by' in serializer.validated_data or hasattr(Interview, 'created_by') else serializer.save()
    out = InterviewCreateSerializer(interview)
...
def candidate_invites(request):
    ...
    returns invites for current user (candidate)
    """
...
def candidate_invite_respond(request, invite_id):
    ...
    resp = (request.data.get("response") or request.data.get("answer") or "").lower()
    if resp not in ("accept", "decline"):
        return Response({"detail": "Invalid response"}, status=status.HTTP_400_BAD_REQUEST)

    invite.status = "accepted" if resp == "accept" else "declined"
    invite.save()
    return Response({"detail": f"Invite {invite.status}"}, status=status.HTTP_200_OK)
