  ...
  function renderUser(container, norm) {
    ...
    if (b) {
      b.addEventListener('click', async () => {
        // clear local tokens first (so frontend won't treat user as logged in)
        clearClientTokens();
        ...
        if (!ok) {
          try {
            const resp2 = await fetch(LOGOUT_URL, { method: 'GET', credentials: 'include' });
            ok = resp2.ok || resp2.status === 302;
          } catch (e) {
