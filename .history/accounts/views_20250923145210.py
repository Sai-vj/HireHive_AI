...
# accounts/views.py (replace existing logout_view)
...
def logout_view(request):
    """
    Logout user (session) and also clear JWT cookie tokens if present.
    Accepts GET or POST so frontends without CSRF token can call it.
    """
    try:
        # server-side logout (session)
        django_logout(request)
    except Exception:
        pass

    # Build redirect response (to home)
    resp = redirect("home")

    # remove Django session cookie
    resp.delete_cookie("sessionid", path="/")

    # remove JWT cookie names if used by token_cookie_obtain
    resp.delete_cookie("access", path="/")
    resp.delete_cookie("refresh", path="/")

    # extra: also remove common names if any (optional)
