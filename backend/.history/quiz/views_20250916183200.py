...
def list_quiz_attempts(request, job_id):
    """
    Return all attempts for this job and current candidate
    """
    job = get_object_or_404(Job, pk=job_id)
    ...
    attempts = QuizAttempt.objects.filter(quiz=quiz, candidate=request.user).order_by("-created_at")
    data = []
    for a in attempts:
        data.append({
            "id": a.id,
            "score": a.score,
            "passed": a.passed,
            "correct": a.answers and len([k for k,v in a.answers.items() if v]) or 0,
            "total": quiz.questions_count if hasattr(quiz, "questions_count") else None,
