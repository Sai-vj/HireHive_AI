...
from .models import Resume, Quiz, QuizAttempt
...
def generate_quiz_for_job(request, job_id):
    """Recruiter generates quiz for a job using LLM. (debuggable)"""
    ...
    # fetch job (keep 404 behavior)
    try:
        job = get_object_or_404(Job, pk=job_id)
    except Exception as e:
        # get_object_or_404 will normally return 404; extra debug printed
        print(f"[QUIZ] Job not found for id={job_id}: {e}")
        return Response({"detail": f"No job with id {job_id}"}, status=404)
    ...
    # generate via LLM — wrap in try so we can log errors
    try:
        questions = generate_quiz_questions(job.title, skills, count=count)
    except Exception as e:
        print(f"[QUIZ] generate_quiz_questions raised exception for job_id={job_id}: {e}")
        return Response({"detail": "Quiz generation error"}, status=500)

    if not questions:
        print(f"[QUIZ] generate_quiz_questions returned empty for job_id={job_id}. job.title={job.title} skills={skills}")
