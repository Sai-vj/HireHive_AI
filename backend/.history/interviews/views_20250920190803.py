...
def start_interview_attempt(request, pk):
    ...
    # DEBUG / log (safe JSON friendly)
    try:
        logger.info("start_interview_attempt: user=%s interview=%s now=%s scheduled_start=%s scheduled_end=%s",
                    request.user.pk,
                    getattr(interview, 'pk', None),
                    now.isoformat(),
                    scheduled_start.isoformat() if scheduled_start else None,
                    scheduled_end.isoformat() if scheduled_end else None)
    except Exception:
        # fail silently on logging problems
        pass

    # dev bypass for staff (only if they append ?force=1)
    ...
    if scheduled_start:
        ...
        if now < scheduled_start:
            # return ISO string so JSON is safe
            return Response({
                'detail': 'Cannot start before scheduled time',
    ...
    # (keep the rest of your existing logic for selecting questions, saving to attempt, etc.)
    # ...
    # at the end return as before, ensure response fields are JSON-serializable
