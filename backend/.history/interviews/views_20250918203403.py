...
def recruiter_invite_candidate_by_job(request, job_pk):
    ...
    # validate job exists and recruiter owns it (adjust field names)
    job = get_object_or_404(Job, pk=job_pk, created_by=request.user)

    # extract body
    candidate_id = request.data.get('candidate_id')
    scheduled_at = request.data.get('scheduled_at')  # expect ISO string
    ...
    # find existing interview for this job created_by this recruiter, or create one
    interview = Interview.objects.filter(job=job, created_by=request.user).order_by('-id').first()
    if interview is None:
        # create a minimal interview (set defaults as you like)
