...
def start_interview_attempt(request, pk):
    ...
    for q in questions:
        qitem = {
            ...
            "prompt": getattr(q, 'prompt', getattr(q, 'text', getattr(q, 'question_text', ''))),
    ...
    if not questions.exists():
        ...
        for qid, qobj in qmap.items():
            ...
            if submitted is None:
                continue
            if getattr(qobj, 'answer', None) is not None and str(submitted).strip().lower() == str(qobj.answer).strip().lower():
                correct += 1
...
def generate_questions_view(request, pk):
    ...
    try:
        ...
        try:
            ...
            logger.exception("generate questions failed", e2)
...
def interview_invite_modal(request, pk):
    interview = get_object_or_404(Interview, pk=pk, created_by=request.user) if any(f.name == 'created_by' for f in Interview._meta.get_fields()) else get_object_or_404(Interview, pk=pk)
