...
def start_interview_attempt(request, pk):
    """
    POST /api/interviews/candidate/<pk>/start/
    Creates or returns existing attempt for candidate+interview.
    Returns serialized attempt and `redirect_url` pointing to frontend attempts page.
    """
    interview = get_object_or_404(Interview, pk=pk)
    ...
    data['redirect_url'] = f"/attempts/{attempt.id}/"
