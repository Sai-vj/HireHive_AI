...
def recruiter_invite_candidate(request, pk):
    ...
    try:
        ...
        # if Celery not configured just send synchronously or ignore
        pass
...
def start_interview_attempt_frontend(request, pk):
    # optional: check invite exists and user is invited / accepted
    # Call your start_interview_attempt view logic (or call its function) to create attempt
    attempt = start_interview_attempt_logic(request.user, pk)  # implement a helper that returns attempt
    return redirect(f'/attempts/{attempt.id}/')  # adjust to your attempt page URL
