...
class InterviewInviteSerializer(serializers.ModelSerializer):
    ...
    interview = InterviewSerializer(read_only=True)   # nested interview with job (if InterviewSerializer exists)
    interview_title = serializers.CharField(source="interview.title", read_only=True)

    # NEW: safe job title field (traverse interview -> job -> title)
    ...
    def get_job_title(self, obj):
        """
        Safe traversal: InterviewInvite -> interview -> job -> title
        If any step is missing return empty string or None (your choice).
        """
        try:
