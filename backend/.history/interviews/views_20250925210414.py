...
@api_view(['GET'])
@permission_classes([AllowAny])  # or custom permission (if candidate must be authenticated)
def candidate_interview_questions(request, pk):
    """
    Candidate-facing list of questions for interview `<pk>`.
    Filters out unpublished/private questions (only exposes visible ones).
    """
    try:
        interview = Interview.objects.get(pk=pk)
    except Interview.DoesNotExist:
    ...
    qs = InterviewQuestion.objects.filter(interview=interview)
    # apply common visibility flags if your model uses them:
    if hasattr(InterviewQuestion, 'is_published'):
        qs = qs.filter(is_published=True)
    if hasattr(InterviewQuestion, 'private'):
        qs = qs.filter(private=False)

    ser = InterviewQuestionSerializer(qs, many=True)
