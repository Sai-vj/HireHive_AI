...
from .serializers import InterviewSerializer,InterviewCreateUpdateSerializer,InterviewQuestionSerializer,InterviewAttemptSerializer
...
def recruiter_invite_candidate(request, pk):
    if not is_recruiter(request.user):
        return Response(...,403)
    interview = get_object_or_404(Interview, pk=pk, created_by=request.user)
    cid = request.data.get('candidate_id')
    sched = request.data.get('scheduled_at')
    msg = request.data.get('message','You are invited')
    candidate = get_object_or_404(User, pk=cid)
