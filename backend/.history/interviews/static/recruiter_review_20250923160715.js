...
async function resolveInterviewIdFromJob(jobId) {
  if (!jobId) return null;
  // Try endpoint that returns latest interview for a job — create on server if missing
  const tries = [
    `/api/interviews/job/${encodeURIComponent(jobId)}/latest/`,
    `/api/interviews/recruiter/?job_id=${encodeURIComponent(jobId)}`
  ];
  for (const u of tries) {
    try {
      ...
      // endpoint may return interview object or list
      if (!data) continue;
      if (Array.isArray(data) && data.length) return data[0].id || null;
      if (data.id) return data.id;
      if (data.results && data.results.length) return data.results[0].id || null;
    } catch(e){}
  }
  return null;
}
...
document.addEventListener("DOMContentLoaded", async () => {
  const params = new URLSearchParams(window.location.search);
  let interviewId = params.get("interview");
  const jobId = params.get("job");
  if (!interviewId && jobId) {
    interviewId = await resolveInterviewIdFromJob(jobId);
  }
  if (interviewId) {
    loadAttempts(interviewId);
  }
