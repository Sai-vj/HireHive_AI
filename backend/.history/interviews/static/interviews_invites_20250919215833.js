  ...
  try {
    const resp = await fetch(`/interviews/candidate/${interviewId}/start/`, {
      ...
      headers: {
        ...
        // if you have token in input/localStorage, add it:
        // 'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}`,
      },
      body: JSON.stringify({ invite_id: inviteId || null })
    });

    console.debug('start API ->', resp);
    ...
    try {
      ...
      console.warn('Start response not JSON', e);
    }
    ...
    if (resp.ok) {
      // Prefer explicit redirect/join URL from backend
      const redirect = data?.redirect_url || data?.url || data?.join_url || null;
      if (redirect) {
        window.open(redirect, '_blank', 'noopener,noreferrer');
        return { ok: true, url: redirect };
      }

      // If backend returned an attempt id, try to open it (but your urls don't have /attempts/*)
      const attemptId = data?.id || data?.attempt_id || data?.pk || null;
      if (attemptId) {
        // If you DO want to open attempts URL (ensure route exists), uncomment:
        ...
        // Recommended: open candidate interview page you already have
        const candidatePage = `/interviews/page/candidate/${interviewId}/`;
        window.open(candidatePage, '_blank', 'noopener,noreferrer');
        return { ok: true, url: candidatePage };
      }

      // Fallback: open candidate page
      const fallback = `/interviews/page/candidate/${interviewId}/`;
      ...
      if (resp.status === 401 || resp.status === 403) {
        ...
        const detail = (data && (data.detail || data.error)) || `Status ${resp.status}`;
        showToast('Failed to start interview: ' + detail, 'error', 6000);
      }
      return { ok: false, res: resp, data };
    }
  } catch (err) {
