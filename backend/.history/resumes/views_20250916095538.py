...
def apply_for_job(request):
    ...
    # REPLACE START: apply view check snippet
# inside apply view before creating Application:
attempt = Attempt.objects.filter(job=job, candidate=request.user).order_by('-created_at').first()
if not attempt or not attempt.passed:
    return Response({'detail':'Quiz not passed'}, status=409)
# REPLACE END
