...
// Requires: utils.js exports fetchWithAuth and apiFetchAsJson (aliased to apiFetch)
...
/* ---------- Invites: debug-enhanced load + respond ---------- */
async function loadInvites() {
  const listEl = document.getElementById('invitesList');
  const container = document.getElementById('invitesSection');
  if (!listEl || !container) {
    console.warn('Invites UI elements missing: invitesList or invitesSection not present in HTML.');
    return;
  }

  listEl.innerHTML = '<div class="small-muted">Loading invites...</div>';
  container.style.display = 'block'; // ensure visible while we diagnose

  // Try several possible endpoints (robust)
  const tries = [
    '/api/interviews/candidate/invites/',
    '/api/interviews/invites/candidate/',
    '/api/interviews/candidate/invites',   // without trailing slash
    '/api/interviews/invites/',
    '/api/interviews/invites',             // fallback shapes
    '/api/interviews/candidate/',          // maybe a list endpoint there
  ];

  let finalRes = null;
  for (const u of tries) {
    try {
      console.debug('Invites: trying', u);
      const res = await apiFetch(u, { method: 'GET' });
      console.debug('Invites: response from', u, res);
      if (!res) { continue; }
      // If auth error, show friendly message and stop trying
      if (res.status === 401 || res.status === 403) {
        listEl.innerHTML = `<div class="text-danger">Authentication required to load invites. Paste token and Save.</div>`;
        return;
      }
      // if success with data
      if (res.ok) { finalRes = { url: u, res }; break; }
      // if 404/405 -> try next
      if (res.status === 404 || res.status === 405) { continue; }
      // otherwise capture and break (we want to show server message)
      finalRes = { url: u, res }; break;
    } catch (err) {
      console.warn('Invites: fetch error for', u, err);
  ...
  if (!finalRes) {
    listEl.innerHTML = `<div class="text-danger">Failed to fetch invites (no response). Check console/network.</div>`;
    return;
  }

  const res = finalRes.res;
  // res.data should contain array or paginated results
  if (!res.ok) {
    const detail = (res.data && (res.data.detail || res.data.message)) ? (res.data.detail || res.data.message) : (`Status ${res.status}`);
    listEl.innerHTML = `<div class="text-danger">Failed to load invites from <strong>${escapeHtml(finalRes.url)}</strong>: ${escapeHtml(detail)}</div>`;
    console.warn('Invites load non-ok response', finalRes.url, res);
    return;
  }

  const invites = Array.isArray(res.data) ? res.data : (res.data?.results || res.data?.invitations || []);
  console.debug('Invites: parsed invites array from', finalRes.url, invites);
  ...
  invites.forEach(inv => {
    ...
    div.innerHTML = `
      <div class="d-flex justify-content-between align-items-start">
        <div style="min-width:0">
          <strong style="display:block">${escapeHtml(interview.title || inv.title || 'Interview')}</strong>
          <div class="small-muted" style="margin-top:4px">Scheduled: ${interview.scheduled_at ? new Date(interview.scheduled_at).toLocaleString() : (inv.scheduled_at ? new Date(inv.scheduled_at).toLocaleString() : '—')}</div>
          <div class="small-muted" style="margin-top:4px">From: ${escapeHtml(inv.recruiter_name || inv.recruiter || interview.recruiter || '')}</div>
        </div>
      ...
      <div class="invite-details mt-2" style="display:${status==='accepted'?'block':'none'}">
        <strong>Details:</strong>
        <div>${escapeHtml(interview.description || interview.instructions || inv.notes || 'No description')}</div>
        <div class="small-muted">Mode: ${escapeHtml(interview.mode || inv.mode || '—')} • Duration: ${escapeHtml(String(interview.duration_minutes || interview.duration || inv.duration || '—'))} mins</div>
      </div>
    `;
  ...
  // Wire Start Interview buttons
  listEl.querySelectorAll('.start-interview-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const iid = btn.dataset.interviewId;
      const inviteId = btn.dataset.inviteId;
      if (!iid) return showToast('Interview id missing', 'error');
      // redirect to interview page; change URL shape if your app uses different route
      window.location.href = `/interviews/candidate/${iid}/?invite=${inviteId}`;
    });
  });
}

// candidate_dashboard.js - robust respondInvite
async function respondInvite(inviteId, answer) {
  if (!inviteId) return showToast('Invite id missing', 'error');

  // normalize answer
  const normalized = (String(answer || '').trim().toLowerCase());
  const payloadValue = (normalized === 'accept' || normalized === 'accepted') ? 'accept' :
                       (normalized === 'decline' || normalized === 'declined') ? 'decline' :
                       normalized;

  // First try: JSON body with "response"
  try {
    const res = await apiFetch(`/api/interviews/candidate/${inviteId}/respond/`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ response: payloadValue })
    });
    if (res && res.ok) {
      showToast(`Invite ${payloadValue}ed`, 'success');
      setTimeout(loadInvites, 300);
      return res;
    }
    // if server replies 400+ try alternate key name
    if (res && (res.status === 400 || res.status === 422)) {
      // fallback: try "action"
      const r2 = await apiFetch(`/api/interviews/candidate/${inviteId}/respond/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: payloadValue })
      });
      if (r2 && r2.ok) {
        showToast(`Invite ${payloadValue}ed`, 'success');
        setTimeout(loadInvites, 300);
      ...
      // final fallback: send form-encoded
      const form = new URLSearchParams();
      form.append('response', payloadValue);
      const r3 = await fetchWithAuth(`/api/interviews/candidate/${inviteId}/respond/`, {
        method: 'POST',
    ...
    // auth problems
    if (res && (res.status === 401 || res.status === 403)) {
      showToast('Auth required. Paste and Save token.', 'error');
      return res;
    }
    // other failure
    showToast('Failed: ' + (res?.data?.detail || res?.status || 'error'), 'error');
    return res;
  } catch (err) {
    console.error('respondInvite error', err);
    showToast('Network error while responding', 'error');
...
/* ---------- Start interview helpers & handler (paste near end of candidate_dashboard.js) ---------- */

// Try starting interview via multiple backend endpoints (uses apiFetch so auth refresh works)
async function startInterview(interviewId) {
  if (!interviewId) return showToast('Missing interview id', 'error');

  const tries = [
    `/api/interviews/candidate/${interviewId}/start/`,
    `/api/interviews/start/${interviewId}/`,
    `/api/interviews/${interviewId}/start/`
  ];

  for (const u of tries) {
    try {
      const res = await apiFetch(u, { method: 'POST' });
      if (!res) continue;
      // unify error handling: skip if not ok
      if (!res.ok) {
        console.warn('startInterview attempt failed', u, res.status, res.data);
        continue;
      }
      ...
      if (data.redirect_url) {
        ...
        return { ok: true, url: data.redirect_url, data };
      }

      // If backend returns an attempt id, open attempt page (common pattern)
      if (data.attempt_id) {
        const attemptUrl = `/attempts/${data.attempt_id}/`;
        window.open(attemptUrl, '_blank', 'noopener,noreferrer');
        return { ok: true, url: attemptUrl, data };
      }

      // otherwise show returned data in a modal (fallback)
      ...
      if (!modal) {
        ...
        modal.innerHTML = `
          <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header"><h5 class="modal-title">Interview</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
              <div class="modal-body" id="interviewModalBody"></div>
      ...
      const bodyEl = document.getElementById('interviewModalBody');
      bodyEl.innerHTML = `<pre style="white-space:pre-wrap">${escapeHtml(JSON.stringify(data, null, 2))}</pre>`;
      try {
      ...
      console.warn('startInterview try failed', u, e);
      continue;
    }
  }

  showToast('Failed to start interview (no matching endpoint)', 'error');
  return { ok: false };
}

// --------- Start interview button handler (open new page) ---------
if (!window.__startInterviewHandlerRegistered) {
  window.__startInterviewHandlerRegistered = true;

  // open interview page (new tab). prefer invite param if provided
function openInterviewPage(interviewId, inviteId) {
  if (!interviewId) { showToast('Missing interview id','error'); return; }
  // Open the frontend page route we added
  let url = `/api/interviews/page/candidate/${interviewId}/`;
  if (inviteId) url += `?invite=${encodeURIComponent(inviteId)}`;
  window.open(url, '_blank');
}



  // Delegated click handler for any button with .start-interview-btn class
  document.addEventListener('click', function (e) {
    const btn = e.target.closest && e.target.closest('.start-interview-btn');
    if (!btn) return;
    e.preventDefault();
    ...
    const interviewId = btn.dataset.interviewId || btn.getAttribute('data-interview-id') || btn.dataset.iid || null;
    const inviteId = btn.dataset.inviteId || btn.getAttribute('data-invite-id') || btn.dataset.invite || null;
    const startBeforeOpen = !!btn.dataset.startBeforeOpen; // optional attr to POST before opening

    (async () => {
      try {
        if (startBeforeOpen) {
          // attempt to start server-side first (uses apiFetch inside startInterview)
          const r = await startInterview(interviewId);
          if (r && r.ok && r.url) {
            // if the start call provided a url (redirect or attempt), open that
            // already opened inside startInterview when appropriate; still safe to check
            // do nothing else
            return;
          }
          // if startInterview showed modal or returned data but no url, also open candidate page
        }

        // default: just open candidate interview page in new tab
        openInterviewPage(interviewId, inviteId);
      } catch (err) {
        console.error('start-interview handler error', err);
        showToast('Network error', 'error');
      } finally {
        // re-enable after a short delay to allow window.open to work and avoid popup blockers
        setTimeout(() => { try { btn.disabled = false; btn.classList.remove('disabled'); } catch (e) {} }, 700);
      }
    })();
  }, false);

  console.log('start interview delegation attached');
}
