...
(function () {
  ...
  // timer
// Full openQuizModal (includes timer, attempts check, auto-submit, rendering, and helpers)
// Paste this into your candidate_dashboard.js (replace any existing openQuizModal / timer helpers)
...
function openQuizModal(jobId, questionPayload) {
  ...
  // These are safe to re-declare (they check/clear intervals).
  ...
  (async () => {
    try {
      ...
      // If GET failed try recruiter-generate POST fallback (some backends expect recruiter to POST)
      if (!resp || !resp.ok) {
        try {
          let r2 = null;
          if (typeof fetchWithAuth === 'function') {
            r2 = await fetchWithAuth(`/api/quiz/generate/${jobId}/`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ job_id: jobId }) });
          } else {
            r2 = await fetch(`/api/quiz/generate/${jobId}/`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': token ? 'Bearer ' + token : '' }, body: JSON.stringify({ job_id: jobId }) });
          }
          if (r2 && r2.ok) resp = r2;
      ...
      if (!resp || !resp.ok) {
        ...
        const msg = err?.detail || err?.message || `${resp ? (resp.status + ' ' + resp.statusText) : 'No response'}`;
        wrap.innerHTML = `<div class="text-danger">Error loading questions: ${escapeHtml(msg)}</div>`;
        console.warn('quiz GET error', resp && resp.status, err);
        return;
      }
      if (!resp.ok) {
  ...
  const detail = parsed?.detail || parsed?.message || text || `${resp.status} ${resp.statusText}`;
  // show in modal and close if needed
  const wrap = document.querySelector('#quiz-questions-wrap');
  if (wrap) wrap.innerHTML = `<div class="text-danger">${escapeHtml(detail)}</div>`;
  if (resp.status === 403) {
    // disable UI
    const submitBtn = document.querySelector('#quiz-submit-btn');
    if (submitBtn) { submitBtn.disabled = true; submitBtn.innerText = 'Max attempts'; }
  }
  return;
}
...
// inside your async flow after you have `questions` and before renderQuestionsInModal(...)
const QUIZ_MAX_ATTEMPTS = 3;
const attemptsResult = await fetchAttemptCount(jobId);

// attemptsResult may be number or {error:...}
if (typeof attemptsResult === 'object' && attemptsResult.error) {
  if (attemptsResult.status === 401) {
    wrap.innerHTML = '<div class="text-danger">Authentication required — please sign in again.</div>';
    return;
  }
  if (attemptsResult.status === 403) {
    // show backend reason inside modal
...
renderQuestionsInModal(questions.slice(0, Math.min(questions.length, 10)));
      ...
      // start timer if provided by backend (body may include time_limit_seconds)
      const timeLimit = body?.time_limit_seconds || body?.time_limit || 120;
      if (timeLimit && Number(timeLimit) > 0) {
        // start timer attached to modal, auto-submit on expire
      ...
      // wire submit button (ensure single handler)
      const submitBtn = modal.querySelector('#quiz-submit-btn');
      if (submitBtn && !submitBtn.__quizHandlerAttached) {
        submitBtn.__quizHandlerAttached = true;
        submitBtn.onclick = async function() {
          // disable to prevent double-click
          submitBtn.disabled = true;
          submitBtn.dataset.origText = submitBtn.innerText || 'Submit';
          ...
          await onQuizSubmit(jobId, answers);
          // restore button state (onQuizSubmit closes modal on success)
          submitBtn.disabled = false;
          submitBtn.innerText = submitBtn.dataset.origText || 'Submit';
        };
      }

      // ensure modal close stops timer
...
/* ---------- helper: renderQuestionsInModal ---------- */
function renderQuestionsInModal(questions) {
  const wrap = document.querySelector('#quiz-questions-wrap');
  if (!wrap) return;

  if (!questions || questions.length === 0) {
    wrap.innerHTML = '<div class="small-muted">No questions to show.</div>';
    return;
  }

  wrap.innerHTML = questions.map(q => {
    // choices may be object {A: "opt", B: "opt", ...}
    const opts = Object.entries(q.choices || {}).map(([key, val]) => {
      return `
        <div style="margin-bottom:6px">
          <label style="cursor:pointer">
            <input type="radio" name="q-${q.id}" value="${escapeHtml(key)}" style="margin-right:8px">
            ${escapeHtml(val)}
          </label>
        </div>
      `;
    ...
    return `
      <div class="quiz-question" data-qid="${q.id}" style="margin-bottom:12px;">
        ...
        <div style="margin-left:6px;">${opts}</div>
      </div>
    `;
  }).join('');
}
// fetch attempts for a job (candidate view)
async function fetchAttemptsForJob(jobId) {
  ...
  try {
    ...
    if (!r.ok) {
      // return error object for caller
      const txt = await r.text().catch(()=>null);
      return { error:true, status:r.status, body: txt };
    }
    const data = await r.json();
    // data is array of attempts
    return data;
  } catch(e) {
...
// render short attempt summary into job card
function renderJobAttemptSummary(jobId, attemptsArray) {
  // locate job card: ensure your job card has data-job-id attribute
  let card = document.querySelector(`.job-card[data-job-id="${jobId}"]`);
  if (!card) card = document.querySelector(`[data-job-id="${jobId}"]`);
  if (!card) return;

  // create container
  let wrap = card.querySelector('.attempt-info');
  if (!wrap) {
  ...
  if (!attemptsArray || attemptsArray.length === 0) {
    wrap.innerHTML = 'Not attempted';
    return;
  }

  // show most recent attempt summary
  const last = attemptsArray[0];
  const score = last.score || 0;
  const total = last.total || (last.total_questions || 0);
  const passed = last.passed ? 'Passed' : 'Failed';
  const dt = new Date(last.finished_at || last.started_at || Date.now()).toLocaleString();

  wrap.innerHTML = `Attempt ${last.id}: ${score}/${total} — <strong style="color:${last.passed ? 'green':'crimson'}">${passed}</strong> <small style="color:#666">(${dt})</small>`;
}

// call during attachQuizButtons or after page load for each job
async function loadAttemptHistory(jobId) {
  const res = await fetchAttemptsForJob(jobId);
  if (res && res.error) {
    console.warn('could not fetch attempts', res);
    return;
  }
  renderJobAttemptSummary(jobId, res);
}

/* ---------- helper: fetchAttemptCount (robust) ---------- */
async function fetchAttemptCount(jobId) {
  ...
  for (const u of tries) {
    try {
      ...
      // auth / forbidden handling -> bubble up
...
async function autoSubmitQuiz(modal) {
  try {
    ...
    if (r.ok) {
      ...
      if (typeof handleQuizSubmitResponse === 'function') handleQuizSubmitResponse(jobId, d);
    } else {
...
/* ---------- helper: onQuizSubmit (submit handler) ---------- */
async function onQuizSubmit(jobId, answers) {
  try {
    const token = localStorage.getItem('token') || '';
    const headers = {'Content-Type':'application/json'};
    ...
    // refresh attempt-count shown in modal
  ...
  return;
  eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiZXhwIjoxNzU4MDI3MzY5LCJpYXQiOjE3NTgwMjcwNjksImp0aSI6ImM1MDhkZjdiYjIyNDQxMDlhMGI2MGY2ZmRjNTA0ZGUwIiwidXNlcl9pZCI6IjkifQ.InMkWBXxXqhfAI40oLpfkGn_GsTHB6ga8LXwTaGJLXI
 
}



    // close modal or show result panel as you want
    closeQuizModal();
...
/* ---------- ATTEMPT UI helpers (paste into candidate_dashboard.js) ---------- */

function findJobCardElement(jobId, fallbackButton) {
  // 1) prefer .job-card[data-job-id="X"]
  let el = document.querySelector(`.job-card[data-job-id="${jobId}"]`);
  if (el) return el;
  // 2) try any element with data-job-id
  el = document.querySelector(`[data-job-id="${jobId}"]`);
  if (el) return el;
  // 3) fallback: if we have a button that triggered modal, find closest .job-card
...
function ensureAttemptContainer(jobCardEl) {
  ...
  if (!wrap) {
    ...
    wrap.className = 'attempt-info';
    wrap.style = 'margin-top:8px;font-size:.9rem;color:#444';
    // append near card footer (adjust if your card layout different)
...
async function loadAttemptHistoryAndRender(jobId, fallbackButton) {
  try {
    ...
    const resp = await fetch(`/api/quiz/${jobId}/attempts/`, { method:'GET', headers });
    if (!resp.ok) {
      console.warn('attempts fetch failed', resp.status);
      // show message in job card if forbidden or other
      ...
      if (wr) {
        try {
          ...
          wr.innerText = parsed?.detail || `Attempts: (error ${resp.status})`;
        } catch(e){
          wr.innerText = `Attempts: (error ${resp.status})`;
        }
      }
      return null;
    }
    ...
    const arr = Array.isArray(data) ? data : (data.results || data.attempts || []);
    const jobCard = findJobCardElement(jobId, fallbackButton);
    ...
    // most recent first? assume API returns recent first; otherwise sort by finished_at desc
...
/* Usage:
 - call loadAttemptHistoryAndRender(jobId) when page loads for each job
 - or call after quiz submit: loadAttemptHistoryAndRender(jobId)
 - attach to attachQuizButtons()
*/


/* ---------- helper: closeQuizModal (stop timer) ---------- */
function closeQuizModal() {
  ...
  const jid = parseInt(card.dataset.jobId, 10);
