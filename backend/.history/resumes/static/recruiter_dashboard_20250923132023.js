...
async function loadJobs() {
  ...
  jobs.forEach(j => {
    ...
    // Company / meta line (below title)
    const metaEl = document.createElement('div');
    metaEl.className = 'small-muted';
    metaEl.style.fontSize = '.9rem';
    metaEl.style.color = '#666';
    metaEl.style.marginBottom = '8px';
    metaEl.innerHTML = `${escapeHtml(j.company || '')} • ${escapeHtml(j.skills_required || j.skills || '')}`;
    titleWrap.appendChild(metaEl);

    // Buttons row that appears BELOW the company/meta
    ...
    // Make clicking title/meta open detail (but not buttons)
    titleEl.addEventListener('click', () => openJobDetail(j.id));
    metaEl.addEventListener('click', () => openJobDetail(j.id));
...
function attachJobCardEvents() {
  ...
  // Optional: make card click open job (if clicked outside buttons)
  qsa('.job-card').forEach(card => {
    if (card._cardBound) return; card._cardBound = true;
    card.addEventListener('click', (ev) => {
      const btn = ev.target.closest('button');
      if (btn) return; // clicked action button
...
async function showShortlistsForSelectedJob() {
  ...
  // helper: try to read candidate name from various possible shapes (prefer candidate/uploader/owner fields)
  function guessCandidateName(s) {
    if (!s) return null;
    const tryList = [
      () => s.candidate_name,                                   // explicit candidate_name
      () => s.candidate || s.candidate_id,                      // candidate id or value
      () => (s.resume && (s.resume.candidate_name || s.resume.name)), // resume embedded name
      () => (s.resume && (s.resume.user_name || s.resume.uploaded_by || s.resume.uploader || s.resume.owner)), // uploader/owner
      () => (s.resume && s.resume.user && (s.resume.user.full_name || s.resume.user.name || s.resume.user.username)), // nested user
      () => s.user || s.username || s.applicant || s.uploaded_by, // other top-level fields
      () => (s.resume && typeof s.resume === 'string' ? s.resume : null), // resume as string
      () => s.shortlisted_by // last resort (this is usually recruiter)
    ];
  ...
  list.forEach(s => {
    // prefer showing candidate name; include resume id if available
    const candidateName = guessCandidateName(s);
    let resumeIdPart = '';
    if (s.resume && (s.resume.id || typeof s.resume === 'number' || (s.resume && s.resume.id === 0))) {
      const rid = s.resume.id || s.resume;
      resumeIdPart = ` (Resume ${escapeHtml(String(rid))})`;
    } else if (s.id) {
      resumeIdPart = ` (Entry ${escapeHtml(String(s.id))})`;
    }

    const displayName = candidateName ? `${escapeHtml(candidateName)}${resumeIdPart}` : (`Resume ${escapeHtml(String(s.resume && (s.resume.id || s.resume) ? (s.resume.id || s.resume) : (s.id || 'N/A')) )}`);
    ...
    div.innerHTML = `
      <div class="d-flex justify-content-between align-items-start">
        ...
        <div>
          <button class="btn btn-sm btn-outline-primary resend-btn" data-job="${escapeHtml(String(jobId))}" data-resume="${escapeHtml(String(s.resume && (s.resume.id || s.resume) ? (s.resume.id || s.resume) : (s.id || '')) )}">Resend</button>
