...
// --- Auto-cookie-auth helper for candidate dashboard ---
// Paste this into candidate_dashboard.js (after your other code)
(async function autoCookieAuth() {
  const tokenCard = document.querySelector('#tokenInput')?.closest('.card') || null; // wrapper card
  const tokenInput = document.getElementById('tokenInput');
  const tokenStatus = document.getElementById('tokenStatus');

  // small UI helpers
  function setStatus(txt, isError=false) {
    if (tokenStatus) tokenStatus.innerText = txt;
    if (isError && tokenStatus) tokenStatus.style.color = 'crimson';
    else if (tokenStatus) tokenStatus.style.color = '';
  }
  ...
  // check cookie-auth by calling a protected endpoint
  async function checkAuth() {
    try {
      const res = await fetch('/api/resumes/my-resumes/', {
  ...
  // attempt to auto-hide token UI and refresh lists when auth present
  async function init() {
    ...
    if (ok) {
      ...
      // refresh lists that rely on auth
      if (typeof refreshJobs === 'function') try { refreshJobs(); } catch(e) {}
      if (typeof refreshResumes === 'function') try { refreshResumes(); } catch(e) {}
      if (typeof refreshMyApplications === 'function') try { refreshMyApplications(); } catch(e) {}
    } else {
      showTokenUI();
    }
  }

  // wire logout (optional): will call token logout endpoint to clear cookies
  async function doLogout() {
    try {
      ...
      if (r.ok) {
        ...
        // refresh lists to reflect unauthenticated state
        if (typeof refreshJobs === 'function') try { refreshJobs(); } catch(e) {}
        if (typeof refreshResumes === 'function') try { refreshResumes(); } catch(e) {}
      } else {
  ...
  // create a small logout button in the token area for convenience
  function addLogoutButton() {
    try {
      ...
      // place near tokenStatus
      if (tokenStatus) tokenStatus.parentNode.insertBefore(btn, tokenStatus.nextSibling);
