...
async function loadAttemptSummary(jobId){
  ...
  async function loadInvites(){
    ...
    // --- render invites with interview-detail lookup (replaces arr.forEach loop) ---
wrap.innerHTML = '';
const interviewCache = {}; // simple in-function cache

// helper to fetch interview detail by id (tries a couple endpoints)
async function fetchInterviewDetailById(iid) {
  if (!iid) return null;
  if (interviewCache[iid]) return interviewCache[iid];
  const tries = [
    `/api/interviews/${encodeURIComponent(iid)}/`,
    `/api/interviews/${encodeURIComponent(iid)}/detail/`,
    `/api/interviews/${encodeURIComponent(iid)}` // fallback without trailing slash
  ];
  for (const u of tries) {
    try {
      const r = await _apiFetch(u);
      if (r && r.ok && r.data) {
        interviewCache[iid] = r.data;
        return r.data;
      }
    } catch (e) { /* ignore and try next */ }
  }
  interviewCache[iid] = null;
  return null;
}
...
for (const inv of arr) {
  ...
  // interview here might be an object or just an id
  let interview = (inv.interview && typeof inv.interview === 'object') ? inv.interview : null;
  const interviewRef = interview ? (interview.id || interview.pk || null) : (inv.interview || inv.interview_id || null);
  if (!interview && interviewRef) {
    // fetch interview detail if we only have an id
    try {
      const detail = await fetchInterviewDetailById(interviewRef);
      if (detail) interview = detail;
    } catch (e) { /* ignore */ }
  }

  const interviewId = interview?.id || interviewRef || '';
  const id = inv.id || inv.invite_id || '';

  // Try to resolve title/job name:
  // priority: inv.interview_title > interview.title > job from interview > job from nested inv > fallback
  let title = inv.interview_title || (interview && (interview.title || interview.name)) || null;

  // If still blank or auto-generated (contains "job <id>"), try to map job id -> job title
  if ((!title || /auto interview for job/i.test(String(title))) ) {
    // try job id from interview or inv
    const jobId = (interview && (interview.job || interview.job_id)) || inv.job_id || inv.job || null;
  ...
  // try nested job object on invite or interview
  if (!title) {
    const nestedJob = (inv.job && typeof inv.job === 'object') ? inv.job : ((interview && interview.job && typeof interview.job === 'object') ? interview.job : null);
    if (nestedJob) title = nestedJob.title || nestedJob.job_title || null;
  }

  if (!title) title = `Interview ${interviewId || id || ''}`;

  const scheduled = inv.scheduled_at || (interview && (interview.scheduled_at || interview.start_time)) || '';
  const recruiter = inv.recruiter_name || inv.recruiter || (interview && (interview.recruiter_name || interview.recruiter)) || '';
