...
const TOKEN_KEY = 'recruiter_token_v1'; // unified token key
...
function showToast(msg) {
  try {
    ...
    const el = document.createElement('div');
    el.style = 'padding:8px 12px;border-radius:6px;background:#fff8d6;border:1px solid #ddd;margin:6px 0';
    el.innerText = msg;
    c.appendChild(el);
...
async function apiFetch(path, opts = {}) {
  ...
  try {
    ...
    if (r.status === 401 || r.status === 403) {
      localStorage.removeItem(TOKEN_KEY);
      showToast('Not authorized — paste a valid token and retry');
      return { ok: false, status: r.status, data: null };
    }
    const txt = await r.text().catch(()=>null);
    let json = null;
