...
document.body.addEventListener('click', e => {
  const b = e.target.closest('.invite-btn');
  if (!b) return;
  window.__invite = { interviewId: b.dataset.interviewId, candidateId: b.dataset.candidateId };
  document.getElementById('invite_candidate_id').value = b.dataset.candidateId;
  document.getElementById('invite_message').value = `Hi ${b.dataset.candidateName}, please join interview.`;
  document.getElementById('inviteModal').classList.remove('d-none');
});

document.getElementById('inviteCancelBtn').addEventListener('click', ()=> document.getElementById('inviteModal').classList.add('d-none'));

document.getElementById('inviteSendBtn').addEventListener('click', async () => {
  const interviewId = window.__invite?.interviewId;
  ...
  if (res.ok) {
    ...
    alert('Invite sent');
    // update UI: show badge near application row (find by candidate id)
    const el = document.querySelector(`.application-row[data-candidate-id="${candidate_id}"] .invite-badge`);
...
// static/recruiter_dashboard.js
// make sure <script type="module" src="{% static 'recruiter_dashboard.js' %}"> is in template

(function () {
  console.log('recruiter dashboard js initialised');

  function getToken() {
    return localStorage.getItem('token') || '';
  }

  // show invite modal and prefill
  function openInviteModal(interviewId, candidateId, candidateName) {
    const modal = document.getElementById('inviteModal');
    document.getElementById('invite_candidate_id').value = candidateId || '';
    document.getElementById('invite_message').value = `Hi ${candidateName || ''}, you have been invited.`;
    // convert interview id to string for form usage
  ...
  // attach click to dynamic .invite-btn (works for later-added nodes)
  document.addEventListener('click', function (e) {
    ...
    if (!interviewId) {
      alert('No interview is linked to this job.');
      return;
    }
    openInviteModal(interviewId, candidateId, candidateName);
  });
  ...
  document.getElementById('inviteSendBtn').addEventListener('click', async function () {
    ...
    const candidateId = document.getElementById('invite_candidate_id').value.trim();
    const scheduled_at = document.getElementById('invite_scheduled_at').value;
    const message = document.getElementById('invite_message').value;

    if (!interviewId) { alert('Missing interview id'); return; }
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        // success - hide modal and update UI badge near that candidate row
        hideInviteModal();
        toast('Invite sent');
        // find matching app row and update invite badge
        const selector = `.application-row[data-candidate-id="${candidateId}"] .invite-badge`;
  ...
  // Export so dev console can call manually if needed
  window._recruiterDashboard = {
    openInviteModal,
    hideInviteModal
  };
})();

// Invite modal helper — paste into your recruiter JS
(function(){
  const API_PREFIX = '/api/interviews'; // base where interviews.urls is included
  // read token from localStorage (you already used this)
  function getToken(){ return localStorage.getItem('token') || ''; }

  window.showInviteModal = function(interviewId, jobTitle){
    window.__inviteInterviewId = interviewId;
    console.log('showInviteModal interviewId=', interviewId);
    document.getElementById('inviteModalTitle').innerText = `Invite for: ${jobTitle || interviewId}`;
  ...
  if (e.target.matches('.invite-btn')) {
    const jobId = e.target.dataset.jobId;            // set via template when rendering apps
    const candidateId = e.target.dataset.candidateId;
    const candidateName = e.target.dataset.candidateName;
    window.__invite_context = { jobId, candidateId };
    document.getElementById('invite_candidate_id').value = candidateId; // optional
    document.getElementById('inviteModalTitle').innerText = `Invite ${candidateName}`;
    document.getElementById('inviteModal').classList.remove('d-none');
  }
});

document.getElementById('inviteSendBtn').addEventListener('click', () => {
  const ctx = window.__invite_context || {};
  const candidateId = document.getElementById('invite_candidate_id').value || ctx.candidateId;
  const scheduledAt = document.getElementById('invite_scheduled_at').value; // "YYYY-MM-DDTHH:MM"
  const msg = document.getElementById('invite_message').value;
  // convert local datetime-local to ISO (add timezone if you want)
  let iso = scheduledAt ? new Date(scheduledAt).toISOString() : null;

  sendInviteByJob(ctx.jobId, candidateId, iso, msg);
  document.getElementById('inviteModal').classList.add('d-none');
});
  ...
  document.getElementById('inviteSendBtn').addEventListener('click', async function (){
    ...
    console.log('inviteSend clicked, interviewId=', interviewId);
    const candidate_id = document.getElementById('invite_candidate_id').value.trim();
    const scheduled_at = document.getElementById('invite_scheduled_at').value;
    const message = document.getElementById('invite_message').value;

    if(!interviewId){ alert('Interview id missing'); return; }
    if(!candidate_id){ alert('Please enter candidate id'); return; }

    // correct URL (important: trailing slash)
    const url = `${API_PREFIX}/recruiter/${interviewId}/invite/`;
    console.log('POST invite ->', url, { candidate_id, scheduled_at, message });
    ...
    try {
      ...
      console.log('invite response body', data);

      if(res.status === 201 || res.ok){
        hideInviteModal();
        alert('Invite sent');
        // update UI badge if exists
        const row = document.querySelector(`[data-application-id][data-candidate-id="${candidate_id}"]`);
        if(row){
        ...
        alert('Endpoint not found (404). Check URL: ' + url);
      } else {
        alert('Invite failed: ' + (data?.detail || JSON.stringify(data)));
      }
    } catch(err){
      console.error('Network error sending invite', err);
...
// recruiter_dashboard.js (append)
(function () {
  console.log('recruiter dashboard js loaded');

  // delegate clicks for .invite-btn
  document.addEventListener('click', function (ev) {
    const btn = ev.target.closest && ev.target.closest('.invite-btn');
    if (!btn) return;

    // defensive reads
    const interviewId = btn.dataset.interviewId || null;
    const candidateId = btn.dataset.candidateId || null;
    const candidateName = btn.dataset.candidateName || '';

    console.log('invite button clicked', { interviewId, candidateId, candidateName });

    if (!interviewId) {
      alert('No interview configured for this job. Create an interview first.');
      console.warn('Missing interviewId on invite button DOM:', btn);
      return;
    }
    if (!candidateId) {
      alert('Candidate id missing. Cannot invite.');
      console.warn('Missing candidateId on invite button DOM:', btn);
      return;
    }
    ...
    window.__inviteInterviewId = interviewId;
    document.getElementById('invite_candidate_id').value = candidateId;
    document.getElementById('invite_message').value = `Hi ${candidateName}, please schedule for interview.`;
    document.getElementById('invite_scheduled_at').value = ''; // optional preset
  ...
  // send invite
  document.getElementById('inviteSendBtn').addEventListener('click', async function () {
    const interviewId = window.__inviteInterviewId || null;
    const candidate_id = document.getElementById('invite_candidate_id').value.trim();
    const scheduled_at = document.getElementById('invite_scheduled_at').value;
    const message = document.getElementById('invite_message').value;

    if (!interviewId) { alert('Missing interview id'); return; }
    if (!candidate_id) { alert('Provide candidate id'); return; }

    const token = localStorage.getItem('token') || '';
    const url = `/api/interviews/recruiter/${interviewId}/invite/`;
    console.log('sending invite to', url, { candidate_id, scheduled_at, message });
    ...
    try {
      ...
      if (res.ok) {
        alert('Invite sent');
        document.getElementById('inviteModal').classList.add('d-none');
        // update badge near the button if present
        const btn = document.querySelector(`.invite-btn[data-candidate-id="${candidate_id}"][data-interview-id="${interviewId}"]`);
        if (btn) {
          const badge = btn.closest('.application-row').querySelector('.invite-badge');
          if (badge) badge.textContent = 'pending';
        }
      } else {
...
async function sendInviteByJob(jobId, candidateId, scheduledAt, message) {
  const token = localStorage.getItem('token') || '';
  const url = `/api/interviews/recruiter/job/${jobId}/invite/`; // matches URL above

  const headers = {'Content-Type': 'application/json'};
  if (token) headers['Authorization'] = `Bearer ${token}`;

  const body = { candidate_id: candidateId };
  if (scheduledAt) body.scheduled_at = scheduledAt; // ISO string from <input type="datetime-local">
  ...
  try {
    ...
    if (res.ok) {
      ...
      // update UI: set invite badge to Pending
      const appRow = document.querySelector(`[data-application-id="${candidateId}"]`);
      if (appRow) {
        const badge = appRow.querySelector('.invite-badge');
        if (badge) badge.innerText = (data.status || 'pending');
      }
