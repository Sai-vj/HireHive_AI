...
// wrap in async function where `wrap` and `jobId` available
async function fetchGenerateQuestions(jobId, wrap) {
  ...
  // try POST first
  try {
    let r = await fetch(`/api/quiz/generate/${jobId}/`, {
      method: 'POST',
      headers,
    ...
    // if POST succeeded
    if (r.ok) return { ok:true, data: await r.json(), method:'POST' };

    // if 405 Method Not Allowed or 403/401 etc, attempt GET fallback
    if (r.status === 405 || r.status === 404 || r.status === 403 || r.status === 401) {
      // try GET
      const r2 = await fetch(`/api/quiz/${jobId}/`); // try candidate endpoint shape /api/quiz/<jobId>/
      if (r2.ok) {
        const d = await r2.json();
        return { ok:true, data:d, method:'GET' };
      }
      // also try /api/quiz/generate/<jobId>/ with GET
      const r3 = await fetch(`/api/quiz/generate/${jobId}/`, { method:'GET', headers });
      if (r3.ok) {
        return { ok:true, data: await r3.json(), method:'GET' };
      }
      // nothing worked: return original error
      const txt = await r.text().catch(()=>null);
      return { ok:false, status:r.status, text: txt || (r.statusText||'') };
    }

    // other POST error — return body for message
    const txt = await r.text().catch(()=>null);
    return { ok:false, status: r.status, text: txt || (r.statusText||'') };
  ...
  function renderQuestionsInModal(questions) {
    ...
    wrap.innerHTML = questions.map(q => {
      const opts = (q.choices || []).map(c => {
        const val = (c.key !== undefined) ? c.key : (c.id !== undefined ? c.id : (c.value !== undefined ? c.value : 'opt'));
        return `<div><label><input type="radio" name="q-${q.id}" value="${escapeHtml(val)}"> ${escapeHtml(c.text || c.label || val)}</label></div>`;
      }).join('');
      return `<div class="quiz-question" data-qid="${q.id}" style="margin-bottom:12px;">
                <div><strong>${escapeHtml(q.text || q.question || ('Question ' + q.id))}</strong></div>
  ...
  async function onQuizSubmit(jobId, answers) {
    try {
      ...
      if (!res.ok) {
        ...
        showToast(err && err.detail ? err.detail : 'Quiz submit failed');
        return;
      }
...
async function openQuizModalAndLoad(jobId, questionPayload) {
  ...
  // call fetcher with timeout and good error handling
  const result = await fetchGenerateQuestionsWithTimeout(jobId, 8000);

  if (!result.ok) {
    console.warn('quiz generate try result', result);
    const msg = result.status ? `${result.status} ${result.text || ''}` : (result.error || 'Not Found');
    wrap.innerHTML = `<div class="text-danger">Error loading questions: ${escapeHtml(msg)}</div>`;
    return;
  }

  // Normalise questions shape
  let questions = null;
  if (Array.isArray(result.data)) questions = result.data;
  else if (result.data && result.data.questions) questions = result.data.questions;
  else if (result.data && Array.isArray(result.data)) questions = result.data;
  else questions = null;
...
// replace the old openQuizModal reference with openQuizModalAndLoad (expose)
window.openQuizModal = openQuizModalAndLoad;
