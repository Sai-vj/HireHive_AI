...
(function(){
   ...
   function renderAttemptList(attempts){
    ...
    attempts
      ...
      .forEach(at => {
        ...
        if (at.answers) {
          try {
            answersHtml = '<pre style="white-space:pre-wrap;margin:0;font-size:.9rem;">' + escapeHtml(typeof at.answers === 'string' ? at.answers : JSON.stringify(at.answers, null, 2)) + '</pre>';
  ...
  document.addEventListener('click', function(e){
    const modal = document.getElementById('attempts-modal');
    if (!modal) return;
    if (e.target.id === 'attempts-modal-close' || e.target.id === 'attempts-modal-ok' || e.target === modal) {
      modal.style.display = 'none';
      document.body.style.overflow = '';
    }
  });
