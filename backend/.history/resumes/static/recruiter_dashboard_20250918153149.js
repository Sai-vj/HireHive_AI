  ...
  jobs.forEach(j => {
    ...
    right.innerHTML = `
      ...
      <div style="margin-top:6px;">
        ...
        <button class="btn btn-sm btn-outline-secondary invite-btn" data-job-id="${j.id}" data-job-title="${escapeHtml(j.title || '')}" style="margin-left:6px">Invite</button>
      </div>
    `;
...
/* ---------------- Invite modal helpers & safe bindings ---------------- */
/* NOTE: Ensure your HTML has exactly ONE invite modal with id="inviteModal"
   and the buttons: #inviteCancelBtn, #inviteSendBtn, inputs:
   #invite_candidate_id, #invite_scheduled_at, #invite_message, #inviteModalTitle
*/

(function setupInviteBindings() {
  // keep state
  window.__inviteInterviewId = null;

  // helper to open invite modal programmatically
  window.openInviteModal = function (interviewId, title) {
    window.__inviteInterviewId = interviewId;
    const modal = qs('#inviteModal');
    if (!modal) return showToast('Invite modal not found', 'error');
    const titleEl = qs('#inviteModalTitle'); if (titleEl) titleEl.innerText = `Invite for: ${title || interviewId}`;
    const cand = qs('#invite_candidate_id'); if (cand) cand.value = '';
    const sched = qs('#invite_scheduled_at'); if (sched) sched.value = '';
    const msg = qs('#invite_message'); if (msg) msg.value = `Interview for ${title || ''}`;
  ...
  // actual send logic (uses TOKEN_KEY)
  async function sendInvite() {
    const interviewId = window.__inviteInterviewId;
    if (!interviewId) return alert('No interview/job selected');
    const candidate_id = (qs('#invite_candidate_id')?.value || '').trim();
    const scheduled_at = qs('#invite_scheduled_at')?.value || '';
    const message = qs('#invite_message')?.value || '';
    ...
    try {
      const res = await fetch(url, { method: 'POST', headers, body: JSON.stringify({ candidate_id, scheduled_at, message }) });
      let data = null;
      ...
      if (res.ok) {
        ...
        alert('Invite failed: ' + ((data && (data.detail || JSON.stringify(data))) || res.status));
      }
    } catch (err) {
      console.error('invite send err', err);
      alert('Network error sending invite');
    }
  }

  // attach modal button handlers inside DOMContentLoaded / attachUI
  const originalAttachUI = attachUI;
  attachUI = function () {
    // run original wiring first
    try { originalAttachUI(); } catch(e){ console.error('attachUI error', e); }

    // safe DOM refs
    const inviteCancel = qs('#inviteCancelBtn');
    const inviteSend = qs('#inviteSendBtn');
    if (inviteCancel && !inviteCancel._bound) {
      inviteCancel._bound = true;
      inviteCancel.addEventListener('click', (e) => { e.preventDefault(); hideInviteModal(); });
    }
    if (inviteSend && !inviteSend._bound) {
      inviteSend._bound = true;
      inviteSend.addEventListener('click', (e) => { e.preventDefault(); sendInvite(); });
    }

    // Delegated handler for any invite buttons rendered in job cards.
    // Make sure your job cards include class "invite-btn" and data-job-id & data-job-title
    document.body.addEventListener('click', function delegatedInvite(e) {
      const btn = e.target.closest && e.target.closest('.invite-btn');
      if (!btn) return;
      e.preventDefault();
      const jobId = btn.dataset.jobId || btn.datasetInterviewId || btn.datasetInterview || btn.dataset.job || btn.dataset.jobId;
      const jobTitle = btn.dataset.jobTitle || btn.dataset.title || '';
      if (!jobId) return showToast('Job id missing for invite', 'error');
      openInviteModal(jobId, jobTitle);
    });
  };
})();
