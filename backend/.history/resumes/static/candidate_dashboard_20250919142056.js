...
async function respondInvite(inviteId, answer) {
  if (!inviteId) { showToast('Invite id missing', 'error'); return; }

  // try alternate server shapes
  const tries = [
    ...
    `/api/interviews/invites/${inviteId}/` // maybe expects PATCH with {status:...}
  ];

  let finalRes = null;
  for (const u of tries) {
    try {
      console.debug('respondInvite trying', u);
      // if endpoint expects JSON payload:
      const res = await apiFetch(u, { method: 'POST', body: JSON.stringify({ response: answer }) });
      console.debug('respondInvite result', u, res);
      if (!res) continue;
      if (res.ok) { finalRes = res; break; }
      // if server says method not allowed or 404, try next
      if (res.status === 404 || res.status === 405) continue;
      finalRes = res; break;
    } catch (e) {
  ...
  if (finalRes && finalRes.ok) {
    ...
    const msg = finalRes?.data?.detail || finalRes?.data?.message || finalRes?.status || 'error';
    showToast('Failed to respond to invite: ' + (msg || 'unknown'), 'error');
    console.warn('respondInvite failed finalRes', finalRes);
  }
}
