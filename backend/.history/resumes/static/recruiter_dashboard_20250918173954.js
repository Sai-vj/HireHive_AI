...
// recruiter_dashboard.js (defensive, robust)
(function(){
  ...
  // find jobs container (try a few names)
  const jobsList = el('jobsList') || el('interviews-container') || el('interviewsContainer') || el('jobsListContainer');
  if (!jobsList) {
    console.error('recruiter_dashboard.js: cannot find jobs list container. Add <div id="jobsList"></div> to template or update script ids.');
  }

  // DOM refs (may be missing in some pages, guard)
  const inviteModal = el('inviteModal');
  const inviteSendBtn = el('inviteSendBtn');
  const inviteCancelBtn = el('inviteCancelBtn');
  const tokenInput = el('tokenInput');
  const saveTokenBtn = el('saveTokenBtn');
  const tokenStatus = el('tokenStatus');
  const refreshJobsBtn = el('refreshJobs');

  // API fetch with refresh-on-401
      ...
      if (refresh) {
        try {
          ...
          if (r.ok) {
            ...
            if (j.access) {
              localStorage.setItem('token', j.access);
              opts.headers['Authorization'] = `Bearer ${j.access}`;
              res = await fetch(url, opts); // retry
            }
          } else {
            console.warn('refresh token failed', await r.text());
          }
        } catch (err) {
  ...
  // show/hide invite modal (exposed globally so inline onclick works)
  window.showInviteModal = function(interviewId, title){
    window.__inviteInterviewId = interviewId;
    const t = el('inviteModalTitle');
    if (t) t.innerText = `Invite for: ${title || interviewId}`;
    if (el('invite_candidate_id')) el('invite_candidate_id').value = '';
    if (el('invite_scheduled_at')) el('invite_scheduled_at').value = '';
    if (el('invite_message')) el('invite_message').value = '';
  ...
  saveTokenBtn?.addEventListener('click', () => {
    ...
    if (!t) { tokenStatus && (tokenStatus.innerText = 'No token entered'); return; }
  ...
  inviteSendBtn?.addEventListener('click', async () => {
    ...
    const candidate_id = (el('invite_candidate_id')?.value || '').trim();
    const scheduled_at = el('invite_scheduled_at')?.value;
    const message = el('invite_message')?.value || '';

    if (!candidate_id) { alert('Enter candidate id'); return; }

    inviteSendBtn.disabled = true;
    try {
      const res = await apiFetch(`${API_BASE}/recruiter/${interviewId}/invite/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (res.ok) {
        hideInviteModal();
        alert('Invite sent ✓');
        const badge = document.querySelector(`[data-interview-id="${interviewId}"] .invite-status`);
  ...
  // render card with both interview and job fallback
  function renderInterviewCard(item){
    // item might be interview object or job-wrapped; normalize
    const interview = item || {};
    const job = interview.job || interview; // some APIs return job info inside job field
    const id = interview.id ?? interview.pk ?? (job && job.id);
    const title = interview.title || (job && job.title) || 'Interview';
    const company = (job && job.company) || '';
    const scheduled = interview.scheduled_at ? new Date(interview.scheduled_at).toLocaleString() : 'Not set';
    const duration = interview.duration_minutes ? `${interview.duration_minutes} min` : (interview.duration ? `${interview.duration} min` : '');
    const status = interview.is_active === false ? 'Inactive' : (interview.is_active ? 'Active' : '-');
    const inviteStatus = interview.invite_status || '-';

    return `
      <div class="job-card" data-interview-id="${escapeHtml(String(id || ''))}">
        <div class="d-flex justify-content-between">
          <div>
            <h6 class="job-title mb-0">${escapeHtml(title)}</h6>
            <div class="small-muted">${escapeHtml(company)} ${duration ? ' • '+escapeHtml(duration) : ''}</div>
            <div class="small-muted">Scheduled: ${escapeHtml(scheduled)}</div>
            <div class="small-muted">Status: <span class="invite-status">${escapeHtml(inviteStatus)}</span></div>
          </div>
          <div class="text-end">
            <button class="btn btn-sm btn-outline-primary me-1" onclick="window.location.href='/api/interviews/recruiter/${id}/'">View</button>
            <button class="btn btn-sm btn-secondary" onclick="showInviteModal(${id}, '${escapeHtml(title).replace(/'/g,"\\'")}')">Invite</button>
          </div>
        </div>
      </div>
    `;
  ...
  // load interviews - robust to response shape
  async function loadInterviews(){
    if (!jobsList) return;
    jobsList.innerHTML = '<div class="small-muted">Loading interviews…</div>';
    try {
      const res = await apiFetch(`${API_BASE}/recruiter/`, { method: 'GET' });
      if (!res.ok) {
        const txt = await res.text().catch(()=>'{no-text}');
        console.error('loadInterviews failed', res.status, txt);
        jobsList.innerHTML = `<div class="text-danger">Failed to load interviews (status ${res.status})</div>`;
        return;
      }
      const data = await res.json().catch(()=>null);
      // API might return object with results, or array
      let list = [];
      if (Array.isArray(data)) list = data;
      else if (data && Array.isArray(data.results)) list = data.results;
      else if (data && typeof data === 'object') {
        // maybe single object — try to find interviews field
        if (Array.isArray(data.interviews)) list = data.interviews;
        else list = [data];
      } else {
        list = [];
      }
      ...
      if (!list.length) {
        jobsList.innerHTML = '<div class="small-muted">No interviews yet</div>';
        return;
      }

      // build html
      const html = list.map(renderInterviewCard).join('');
      jobsList.innerHTML = html;
    } catch (e) {
      console.error('loadInterviews error', e);
      jobsList.innerHTML = '<div class="text-danger">Network error loading interviews</div>';
    }
  }

  // wire refresh
  refreshJobsBtn?.addEventListener('click', loadInterviews);

  // initial
  document.addEventListener('DOMContentLoaded', () => {
    if (tokenInput && localStorage.getItem('token')) tokenInput.value = localStorage.getItem('token');
    if (tokenStatus && localStorage.getItem('token')) tokenStatus.innerText = 'Token saved';
    loadInterviews();
  });

  // optional poll
  setInterval(() => { if (document.visibilityState === 'visible') loadInterviews(); }, 60000);
