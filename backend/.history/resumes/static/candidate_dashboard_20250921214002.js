...
async function loadInvites() {
  ...
  arr.forEach(inv => {
    ...
    const canStart = (status === 'accepted') && isScheduledNowOrPast(scheduled) && (interview.id || inv.interview_id);
    const startBtnHtml = canStart ? `<button class="btn btn-sm btn-primary start-invite" data-interview="${escapeHtml(interview.id||inv.interview_id)}" data-invite="${escapeHtml(id)}">Start</button>` : (status === 'accepted' && (interview.id||inv.interview_id) ? `<button class="btn btn-sm btn-primary start-invite" data-interview="${escapeHtml(interview.id||inv.interview_id)}" data-invite="${escapeHtml(id)}" disabled>Start</button>` : '');
