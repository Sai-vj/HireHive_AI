...
(function () {
  ...
  // ---------- modal helpers (use Bootstrap if available, fallback safely) ----------
function _showModalElement(modalEl) {
  ...
  // fallback: make element modal-like in an accessible way
  modalEl.setAttribute('tabindex', modalEl.getAttribute('tabindex') || '-1');
  modalEl.setAttribute('aria-hidden', 'false');
  modalEl.classList.add('show');
  modalEl.style.display = 'block';
  // add body class to emulate modal open (prevents background scroll)
...
async function viewInvite(inviteId) {
  ...
  // create or reuse modal (Bootstrap-friendly attributes)
  ...
  // helper to show modal using Bootstrap if available
  function showBootstrapModal() {
    try {
      ...
      // fallback simple display (last resort)
      modal.style.display = 'block';
      return false;
    }
  }

  // try HTML fragment first (if your server provides it)
  try {
