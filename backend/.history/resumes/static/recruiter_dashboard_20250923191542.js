...
function safeShowBootstrapModal(modalEl, options = {}) {
  ...
  // Ensure element is appended to body (Bootstrap expects it)
  try {
    if (!document.body.contains(modalEl)) document.body.appendChild(modalEl);
  } catch (e) { console.warn('append modal failed', e); }
    ...
    if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
      try {
        ...
        if (inst && typeof inst.show === 'function') {
          // show in next tick to avoid rare "illegal invocation" timing problems
          setTimeout(() => {
            try { inst.show(); } catch (e) { console.warn('bootstrap show failed', e); }
          }, 0);
          return inst;
        }
      } catch (inner) {
        console.warn('bootstrap modal creation failed', inner);
      }
    }
  } catch (e) { console.warn('bootstrap try failed', e); }
  ...
  try {
    ...
    console.warn('plain DOM fallback failed', e);
    return null;
  }
}
