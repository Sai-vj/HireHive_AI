...
function openInviteModal(interviewId) {
...
document.getElementById('modalSendBtn').addEventListener('click', async () => {
  ...
  const candidate_id = document.getElementById('modalCandidateId').value;
  const scheduled_at = document.getElementById('modalScheduledAt').value;
  const message = document.getElementById('modalMessage').value;
  const token = localStorage.getItem('token'); // or from your input
  ...
  if (res.ok) {
    ...
    // optionally add the invite card to UI
  } else {
...
function renderJobCard(job) {
  // job.id, job.title, job.company etc
  return `
    

        <!-- Invite button (HTMX) -->
        <button class="btn btn-sm btn-secondary invite-candidate-btn"
            hx-get="/api/interviews/recruiter/${job.id}/invite/modal/"
            hx-target="#modal-container"
            hx-swap="innerHTML">
          Invite Candidate
        </button>

        <!-- optional fallback JS handler instead of HTMX:
        <button class="btn btn-sm btn-secondary" onclick="openInviteModal(${job.id})">Invite Candidate</button>
        -->
      </div>
    </div>
  `;
...
// Utils
function showInviteModal(interviewId, title) {
  window.__inviteInterviewId = interviewId;
...
document.getElementById('inviteSendBtn').addEventListener('click', async () => {
  ...
  const url = `/api/interviews/recruiter/${interviewId}/invite/`; // match your urls
  // If you use JWT token in localStorage:
  const token = localStorage.getItem('token') || '';

  const headers = { 'Content-Type':'application/json' };
  if(token) headers['Authorization'] = `Bearer ${token}`;
  // if session-auth + csrf required: add X-CSRFToken header using cookie helper below
  ...
  try {
    ...
    if(res.ok) {
      hideInviteModal();
      // show quick toast / append small success text
      alert('Invite sent');
      // OPTIONAL: append a small badge on the job card or increment counter
      // e.g. addInviteBadgeToJobCard(interviewId)
    } else {
