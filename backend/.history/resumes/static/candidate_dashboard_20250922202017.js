...
(function () {
  ...
  async function _fetchWithAuth(url, opts = {}) {
    // normalize options and force credentials for cookie flow
    opts = Object.assign({}, opts);
    opts.credentials = opts.credentials || 'same-origin'; // ensure cookies sent
    opts.headers = Object.assign({}, opts.headers || {});
    // prefer using global fetchWithAuth wrapper if available
    if (hasFetchWithAuth) {
      try {
        return await window.fetchWithAuth(url, opts);
      } catch (e) {
        console.debug('fetchWithAuth wrapper failed, falling back to fetch', e);
      }
    }
