...
async function deleteJob(jobId) {
  ...
  try {
    ...
    if (resp.ok) {
      ...
      alert("Delete failed: " + (data.detail || resp.status));
    }
  } catch (e) {
...
async function loadJobs() {
  ...
  try {
    ...
    data.forEach(job => {
      ...
      div.innerHTML = `
        <h6>${job.title}</h6>
        <div class="small-muted">${job.company || ""}</div>
    ...
    attachJobCardEvents(); // 🔑 attach events to new buttons
  } catch (e) {
    console.error("Jobs load failed", e);
    jobsList.innerHTML = "<div class='text-danger'>Error loading jobs</div>";
  }
}
