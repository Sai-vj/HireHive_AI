...
// JS - hookup with your existing function
const btn = document.getElementById('generateBtn');
const status = document.getElementById('genStatus');

async function generateQuestionsForJobCreateInterviewThenGenerate_UI(jobPk, questionsCount = 25, autoGenerate = true) {
  if (!jobPk) {
    status.innerText = 'Missing job id';
    return { ok: false, error: 'missing job id' };
  }

  // Disable button + show generating text
  btn.disabled = true;
  ...
  try {
    const createRes = await createInterviewForJob(jobPk);
    if (!createRes?.ok) {
      status.innerText = 'Failed to create interview';
    ...
    if (!interviewId || !Number.isInteger(interviewId)) {
      status.innerText = 'No valid interview id returned';
      return { ok: false, error: 'no valid interview id', created };
    }

    // Optionally update status to show stage
    status.innerText = 'Interview created — generating questions...';

    const genRes = await generateInterviewQuestions(interviewId, questionsCount, autoGenerate);

    if (genRes?.ok) {
      const took = Math.round((Date.now() - startTs) / 1000);
      status.innerText = `Questions generated (${questionsCount}) in ${took}s`;
      return { ok: true, interviewId, createRes, genRes };
    } else {
      status.innerText = 'Question generation failed';
      return { ok: false, genRes };
    }
  } catch (err) {
    console.error(err);
    status.innerText = 'Unexpected error. Check console.';
    return { ok: false, error: err?.message || String(err) };
  } finally {
    // re-enable after small delay so user sees final text
    setTimeout(() => { btn.disabled = false; }, 400);
  }
}

// attach to button (replace jobPk with actual id from your UI)
btn.addEventListener('click', async () => {
  const jobPk = Number(btn.dataset.jobPk) || 123; // <-- set your job id source
  await generateQuestionsForJobCreateInterviewThenGenerate_UI(jobPk, 25, true);
});
