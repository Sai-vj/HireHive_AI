...
/**
 * Create or update job depending on addJobForm.dataset.editing.
 * Replaces the old submitAddJob so one function can handle both flows.
 */
async function submitAddJob(e) {
  if (e && e.preventDefault) e.preventDefault();
  const form = document.getElementById('addJobForm');
  ...
  const payload = {
    ...
    skills_required: (document.getElementById('jobSkills')?.value || '').trim(),
    experience_required: Number(document.getElementById('jobExperience')?.value || 0),
    vacancies: Number(document.getElementById('jobVacancies')?.value || 1),
    description: (document.getElementById('jobDescription')?.value || '').trim()
  };

  const editingId = form.dataset.editing; // if present -> PATCH
  try {
    let res;
    if (editingId) {
      // PATCH update existing job
      res = await apiFetch(`/api/resumes/jobs/${encodeURIComponent(editingId)}/`, {
        method: 'PATCH',
      ...
      // Create new job
      res = await apiFetch('/api/resumes/jobs/', {
        method: 'POST',
        body: JSON.stringify(payload),
        headers: { 'Content-Type': 'application/json' }
      });
    ...
    if (!res || !res.ok) {
      const errMsg = (res && res.data && (res.data.detail || res.data.message)) || (res && res.status) || 'Unknown';
    ...
    // reload jobs and optionally re-open job detail for updated job
    await loadJobs();

    // if updated, re-open details for that job so UI shows updated info
    if (editingId) {
      // small delay to let loadJobs render
      setTimeout(() => openJobDetail(editingId).catch(()=>{}), 200);
    }

  } catch (err) {
    console.error('submitAddJob err', err);
 ...
 * (This is safe to reuse; I renamed earlier function to confirmAndDeleteJob)
 */
async function confirmAndDeleteJob(jobId) {
  if (!jobId) return showToast('No job id', 'error');
  if (!confirm('Delete job permanently?')) return;
  try {
    const res = await apiFetch(`/api/resumes/jobs/${encodeURIComponent(jobId)}/`, { method: 'DELETE' });
    if (!res || !res.ok) {
    ...
    // clear job details if the deleted job was selected
    ...
    console.error('confirmAndDeleteJob err', err);
    showToast('Network error', 'error');
  }
}
/* ---------------- Matches ---------------- */
