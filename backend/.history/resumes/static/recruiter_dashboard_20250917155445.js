...
// DELETE
async function confirmAndDeleteJob(jobId, token) {
  const res = await fetch(`/api/.../recruiter/job/${jobId}/delete/`, {
    method: 'DELETE',
    headers: {
...
// PATCH
async function patchJob(jobId, payload, token) {
  const res = await fetch(`/api/.../recruiter/job/${jobId}/`, {
    method: 'PATCH',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
