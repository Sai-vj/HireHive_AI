...
async function loadMyApplications() {
  ...
  // figure out current user id from token if present (same as before)
  const token = localStorage.getItem('token') || localStorage.getItem('access_token') || '';
  ...
  apps.forEach(a => {
    ...
    // Normalize status: check many possible fields / shapes
    let rawStatus = (a.status || a.application_status || '') || '';
    // also inspect boolean flagged fields
    if (!rawStatus) {
      if (a.shortlisted === true || a.is_shortlisted === true) rawStatus = 'shortlisted';
      else if (a.rejected === true) rawStatus = 'rejected';
      else if (a.withdrawn === true) rawStatus = 'withdrawn';
    }
    // nested or alternative shapes
    if (!rawStatus && a.application && typeof a.application === 'object') {
      rawStatus = a.application.status || a.application.application_status || '';
      if (!rawStatus && (a.application.shortlisted === true || a.application.is_shortlisted === true)) rawStatus = 'shortlisted';
    }

    const s = String(rawStatus || 'pending').trim().toLowerCase();

    // Friendly label + badge class mapping
    const statusMap = {
      'shortlisted': ['Shortlisted', 'bg-success'],
      'short_list': ['Shortlisted', 'bg-success'],
      'short-list': ['Shortlisted', 'bg-success'],
      'shortlist': ['Shortlisted', 'bg-success'],
      'accepted': ['Accepted', 'bg-success'],
      'rejected': ['Rejected', 'bg-danger'],
      'withdrawn': ['Withdrawn', 'bg-secondary'],
      'pending': ['Pending', 'bg-secondary'],
      'pending_review': ['Pending review', 'bg-secondary'],
      'in_review': ['In review', 'bg-secondary'],
      'hired': ['Hired', 'bg-success'],
      'declined': ['Declined', 'bg-danger']
    };
    const mapped = statusMap[s] || [ (s ? (s.charAt(0).toUpperCase() + s.slice(1)) : 'Pending'), (s==='pending' ? 'bg-secondary' : 'bg-secondary') ];
    const statusLabel = mapped[0];
    const badgeClass = mapped[1];
    ...
    div.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:start">
      <div style="min-width:0">
        ...
        <div class="small-muted">Applied: ${escapeHtml(appliedAt)} • Status: <span class="badge ${badgeClass}">${escapeHtml(statusLabel)}</span></div>
      ...
      <div style="text-align:right">
        <button class="btn btn-sm btn-outline-danger remove-app-btn" data-id="${escapeHtml(id)}">Withdraw</button>
      </div>
    </div>`;
    el.appendChild(div);
