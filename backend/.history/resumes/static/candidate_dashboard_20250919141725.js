...
async function loadInvites() {
  ...
  listEl.querySelectorAll('.start-interview-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      // go to dedicated interview page (candidate facing)
...
async function respondInvite(inviteId, answer) {
  ...
  // note: some servers expect /api/interviews/candidate/invites/<id>/respond/ vs /api/interviews/candidate/<id>/respond/
  // we'll try the "invites" path first then fallback
  const tries = [
  ...
  for (const u of tries) {
    try {
      ...
      if (res && res.ok) { finalRes = res; break; }
      // if server returned 404, try next
  ...
  if (finalRes && finalRes.ok) {
    ...
    showToast('Failed: ' + (finalRes?.data?.detail || finalRes?.status || 'error'), 'error');
  }
}
