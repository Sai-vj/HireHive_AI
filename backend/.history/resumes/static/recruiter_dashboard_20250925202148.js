...
async function generateInterviewQuestions(interviewPk, nQuestions = 25, autoPublish = false) {
  ...
  try {
    const body = { count: Number(nQuestions) || 25, auto_publish: !!autoPublish, params: {} };
...
document.addEventListener('DOMContentLoaded', () => {
  const createBtn = document.getElementById('rdashCreateInterviewBtn');
  if (createBtn) {
    createBtn.addEventListener('click', async () => {
      // open fallback add job modal (so user can create a Job); if you want create interview directly, call createInterviewForJob(...)
      const modal = document.getElementById('addJobModal') || createFallbackAddJobModal();
      safeShowBootstrapModal(modal);
    });
  }

  const inviteBtn = document.getElementById('rdashInviteCandidateBtn');
  if (inviteBtn) {
    inviteBtn.addEventListener('click', () => {
      // show invite modal; if you have a selectedJob use its id
      const jobId = window.selectedJob?.id || null;
      // showInviteModal is provided by recruiter_dashboard.js
      window.showInviteModal({ jobId });
    });
  }
});
