...
function createQuizModal() {
  ...
  document.querySelector('#quiz-submit-btn').addEventListener('click', async () => {
    ...
    // ✅ build answers object (not array)
    const answers = {};
    ...
    await onQuizSubmit(jobId, answers);  // send to backend
    document.querySelector('#quiz-submit-btn').disabled = false;
  });
}

// candidate_dashboard.js (replace your current file with this)

// small helper escape (used when rendering text)
function escapeHtml(s) {
  if (s === null || s === undefined) return '';
  return String(s)
    .replace(/&/g, '&amp;')
...
function openQuizModal(jobId, questionPayload) {
  createQuizModal(); // assume this builds #quiz-modal structure
  const modal = document.querySelector('#quiz-modal');
  if (!modal) { console.error('Quiz modal markup missing (createQuizModal)'); return; }
  modal.style.display = 'flex';
  modal.dataset.jobId = jobId;
  const wrap = modal.querySelector('#quiz-questions-wrap');
  wrap.innerHTML = 'Loading questions…';

  // If caller passed questions directly (rare)
  ...
  (async () => {
    try {
      // prefer fetchWithAuth if provided
      let resp = null;
      ...
      // fallback: generate endpoint (some backends need POST)
      if (!resp || !resp.ok) {
        try {
          let r2 = null;
          if (typeof fetchWithAuth === 'function') {
            ...
            r2 = await fetch(`/api/quiz/generate/${jobId}/`, { method: 'POST', headers, body: JSON.stringify({ job_id: jobId }) });
          }
          if (r2 && r2.ok) resp = r2;
        } catch (e) { /* ignore */ }
      }
      ...
      if (!resp || !resp.ok) {
        ...
        const msg = parsed?.detail || parsed?.message || text || `${resp ? (resp.status + ' ' + resp.statusText) : 'No response'}`;
        wrap.innerHTML = `<div class="text-danger">Error loading questions: ${escapeHtml(msg)}</div>`;
        console.warn('quiz GET error', resp && resp.status, parsed || text);
        return;
      }
...
async function loadAttemptHistoryAndRender(jobId, fallbackButton) {
  try {
    ...
    if (!resp) return null;
    if (resp.error) {
      // populate job card with error note
    ...
    // take most recent (sort by finished_at)
...
async function onQuizSubmit(jobId, answers) {
  try {
    ...
    // refresh attempt count in modal
...
/* ---------- attachQuizButtons: hook up job cards ---------- */
function attachQuizButtons() {
  document.querySelectorAll('.take-quiz-btn').forEach(btn => {
    ...
    btn.addEventListener('click', () => {
      ...
      if (!jid) {
        console.warn('No job id found on button');
        return;
      }
      openQuizModal(jid);
    });
...
// Run attachQuizButtons after DOM ready
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', attachQuizButtons);
} else {
  attachQuizButtons();
}
// 🔥 Force attach click listeners after page load
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.take-quiz-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const jid = Number(btn.dataset.jobId || btn.getAttribute('data-job-id'));
      console.log('Take Quiz clicked for jobId=', jid);
      if (typeof openQuizModal === 'function') {
        openQuizModal(jid);
      } else {
        alert('openQuizModal not available');
      }
    });
  });
});
console.log("DEBUG: force attach starting...");

setTimeout(() => {
  const btns = document.querySelectorAll('.take-quiz-btn');
  console.log("DEBUG: found", btns.length, "take-quiz buttons");
  
  btns.forEach(btn => {
    btn.onclick = () => {
      const jid = Number(btn.dataset.jobId || btn.getAttribute('data-job-id'));
      console.log("DEBUG: Take Quiz clicked → jobId=", jid);
      if (typeof openQuizModal === 'function') {
        openQuizModal(jid);
      } else {
        alert("openQuizModal not found");
      }
    };
  });

  console.log("DEBUG: force attach completed.");
...
/* --------------------------
   small toast helper if missing
   (if you already have showToast, ignore this)
   -------------------------- */
