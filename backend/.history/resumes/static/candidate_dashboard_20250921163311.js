...
(function () {
  ...
  async function loadInvites() {
    ...
    if (!res.ok) {
      ...
      try {
        ...
        if (alt.ok) { processInvitesData(alt.data); return; }
      } catch (e){}
      if (wrap) wrap.innerHTML = `<div class="small-muted">Failed to load invites (${res.status})</div>`;
      if (modalWrap) modalWrap.innerHTML = `<div class="text-muted small">Failed to load invites (${res.status})</div>`;
      return;
    }
  ...
  function processInvitesData(raw) {
        ...
        arr.forEach(inv => {
          ...
          div.querySelectorAll('.accept-invite').forEach(b => b.addEventListener('click', async () => {
            ...
            try {
              ...
              if (r.ok) { showToast('Accepted', 'success'); await loadInvites(); }
          ...
          div.querySelectorAll('.decline-invite').forEach(b => b.addEventListener('click', async () => {
            ...
            try {
              ...
              if (r.ok) { showToast('Declined', 'success'); await loadInvites(); }
    ...
    const modalWrap = document.getElementById('invitesListModal');
    if (modalWrap) {
      if (!arr.length) modalWrap.innerHTML = '<div class="text-muted small">No invites.</div>';
      else {
        modalWrap.innerHTML = '';
        arr.forEach(inv => {
          const id = inv.id;
          ...
          row.querySelectorAll('.accept-invite-modal').forEach(b => b.addEventListener('click', async () => {
            ...
            try {
              ...
              if (r.ok) { showToast('Accepted', 'success'); await loadInvites(); } else { showToast('Accept failed', 'error'); b.disabled = false; }
          ...
          row.querySelectorAll('.decline-invite-modal').forEach(b => b.addEventListener('click', async () => {
            ...
            try {
              ...
              if (r.ok) { showToast('Declined', 'success'); await loadInvites(); } else { showToast('Decline failed', 'error'); b.disabled = false; }
  ...
  async function viewInvite(inviteId) {
    ...
    try {
      ...
      `;
      modal.querySelector('#inviteAcceptBtn')?.addEventListener('click', async () => { await respondInvite(inviteId, 'accept'); modal.style.display='none'; });
      modal.querySelector('#inviteDeclineBtn')?.addEventListener('click', async () => { await respondInvite(inviteId, 'decline'); modal.style.display='none'; });
