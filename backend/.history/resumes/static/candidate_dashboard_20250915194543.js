...
document.addEventListener('submit', async function (e) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      // show useful error
// ===== replace existing 409 branch with this =====
if (res.status === 409) {
  // res.data may be object or string; handle common shapes
  const body = res.data || {};
  // 1) direct detail/message
  let msg = body.detail || body.message || body.error || null;

  // 2) check nested application object
  let appInfo = null;
  ...
  if (appInfo) {
    ...
    // optional: open application details or link
    console.log('Existing application object:', appInfo);
  }

  // fallback: if nothing useful, stringfy entire body
