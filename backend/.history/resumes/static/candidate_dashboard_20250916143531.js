...
// ---------- Quiz timer helper (paste once, near other helpers) ----------
...
function startQuizTimer(seconds, onExpire) {
  ...
  // find header to attach pill
  let header = modal.querySelector('h4#quiz-modal-title') || modal.querySelector('h4');
  ...
  if (!pill) {
    ...
    pill.id = 'quiz-timer';
    pill.style = 'margin-left:12px;font-weight:600;color:#333';
  ...
  function tick() {
    ...
    if (__quizTimerRemaining <= 30) pill.classList.add('warning'); else pill.classList.remove('warning');
...
// ---------- end timer helper ----------
  ...
  fetcher(`/api/quiz/${jobId}/`, { method: 'GET' })
    ...
    /* ---------- Quiz: timer + max attempts (copy into candidate_dashboard.js near quiz modal code) ---------- */
/*
  Adds:
   - timer in quiz modal (countdown)
   - auto-submit on time up
   - checks attempts count and blocks > 3 attempts
...
(function(){ 
  ...
  // attach a small results panel below questions
  function renderResultPanel(modal, result) {
    // result: { attempt_id, score, passed, correct, total }
    let panel = modal.querySelector('#quiz-result-panel');
    if (!panel) {
      panel = document.createElement('div');
      panel.id = 'quiz-result-panel';
      panel.style = 'margin-top:12px;padding:12px;border-radius:8px;background:#f8f9fa;border:1px solid #e9ecef;';
      modal.querySelector('#quiz-questions-wrap').after(panel);
    }
    panel.innerHTML = `
      <div style="font-weight:600;margin-bottom:6px;">Result</div>
      <div>Score: <strong>${result.score}%</strong> (${result.correct}/${result.total})</div>
      <div>Status: <strong style="color:${result.passed ? 'green' : 'crimson'}">${result.passed ? 'Passed' : 'Failed'}</strong></div>
      <div style="margin-top:6px;font-size:.9rem;color:#666">${new Date().toLocaleString()}</div>
    `;
  }
  ...
  function startQuizTimer(modal, seconds, onTick, onExpire) {
    // show timer area
    let tEl = modal.querySelector('#quiz-timer');
    if (!tEl) {
    ...
    function fmt(s){ const m = Math.floor(s/60); const se = s%60; return `${m}:${String(se).padStart(2,'0')}`; }
    tEl.textContent = `Time: ${fmt(remaining)}`;
    // clear any previous interval
    if (modal._quizTimerInterval) { clearInterval(modal._quizTimerInterval); modal._quizTimerInterval = null; }
    modal._quizTimerInterval = setInterval(() => {
      remaining -= 1;
      if (remaining < 0) {
        clearInterval(modal._quizTimerInterval);
        modal._quizTimerInterval = null;
        tEl.textContent = `Time: 0:00`;
        if (typeof onExpire === 'function') onExpire();
      } else {
        tEl.textContent = `Time: ${fmt(remaining)}`;
    ...
    return {
      stop(){ if (modal._quizTimerInterval) { clearInterval(modal._quizTimerInterval); modal._quizTimerInterval = null; } },
      set( sec ){ remaining = Number(sec) || 0; tEl.textContent = `Time: ${fmt(remaining)}`; }
    };
  }

  // fetch attempt history and return count (uses fetchWithAuth if available)
  async function fetchAttemptCount(jobId) {
    try {
      // Try endpoints previously used in code
      const tries = [
        `/api/quiz/attempts/?job_id=${jobId}`,
        `/api/quiz/attempts/?job=${jobId}`,
        `/api/quiz/attempts/` // fallback - server may filter
      ];
      for (const u of tries) {
        try {
          const r = await fetchWithAuth ? await fetchWithAuth(u, { method:'GET' }) : await fetch(u, { method:'GET' });
          if (!r) continue;
          // fetchWithAuth returns Response sometimes or wrapper - support both
          let ok = r.ok; let data = null;
          if (r.json) { data = await r.json(); ok = r.ok; }
          else if (r.data !== undefined) { data = r.data; ok = r.ok; }
          else {
            // if it was a wrapper with text
            try { data = await r.json(); } catch(e){ data = null; }
          }
          if (r.ok && data) {
            // if API returns list directly
            if (Array.isArray(data)) return data.length;
            // else if returns results
            if (Array.isArray(data.results)) return data.results.length;
            if (Array.isArray(data.attempts)) return data.attempts.length;
            // if not, continue trying
          } else {
            // continue on 404
            if (r.status && (r.status === 404 || r.status === 405)) continue;
          }
        } catch (e) {
          // try next
          continue;
    ...
    } catch (e) { console.warn('fetchAttemptCount error', e); }
    return 0;
  }

  // auto submit helper used when time is up
  async function autoSubmitQuiz(modal) {
    try {
      ...
      // call existing onQuizSubmit if present
      ...
      if (r.ok) {
        ...
        showToast('Auto-submit failed', 'error', 4000);
        console.warn('autoSubmit failed', r.status, txt);
      }
    } catch (e) { console.error('autoSubmit error', e); showToast('Auto-submit error', 'error'); }
  }

  // enhance existing openQuizModal to start timer and check attempts
  const origOpenQuizModal = window.openQuizModal;
  window.openQuizModal = async function(jobId, questionPayload) {
    // call original to render modal and questions (if it exists)
    if (typeof origOpenQuizModal === 'function') origOpenQuizModal(jobId, questionPayload);

    // now find modal and set dataset (may be created by original)
    const modal = document.querySelector('#quiz-modal');
    if (!modal) return;
    modal.dataset.jobId = jobId;

    const wrap = modal.querySelector('#quiz-questions-wrap');
    // show loading while we check attempts/time
    wrap.innerHTML = 'Loading questions…';

    // 1) fetch quiz info (to get time limit & questions via candidate endpoint)
    let quizData = null;
    try {
      // candidate GET shape: /api/quiz/<jobId>/
      const rQuiz = await fetchWithAuth ? await fetchWithAuth(`/api/quiz/${jobId}/`, { method:'GET' }) : await fetch(`/api/quiz/${jobId}/`);
      if (rQuiz && rQuiz.ok) {
        quizData = (rQuiz.json ? await rQuiz.json() : rQuiz.data) || null;
      } else {
        // if GET fails, try generate endpoint (server may accept POST only)
        const r2 = await (fetchWithAuth ? fetchWithAuth(`/api/quiz/generate/${jobId}/`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ job_id: jobId }) }) : fetch(`/api/quiz/generate/${jobId}/`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ job_id: jobId }) }));
        if (r2 && (r2.ok || r2.status === 200)) {
          quizData = (r2.json ? await r2.json() : r2.data) || null;
        }
      }
    } catch (e) {
      console.warn('quiz fetch attempt failed', e);
    }

    // If we got quizData, extract questions/time
    const questions = quizData?.questions || quizData?.questions_json || quizData?.questions || null;
    const timeLimit = quizData?.time_limit_seconds || quizData?.time_limit || null;
    // render questions (hide answers)
    if (questions && Array.isArray(questions) && questions.length) {
      // reuse existing renderer if available
      if (typeof renderQuestionsInModal === 'function') renderQuestionsInModal(questions);
      else {
        // minimal rendering
        wrap.innerHTML = questions.map(q=>{
          const opts = Object.entries(q.choices||{}).map(([k,v])=>`<div><label><input type="radio" name="q-${q.id}" value="${k}"> ${k}. ${escapeHtml(v)}</label></div>`).join('');
          return `<div class="quiz-question" data-qid="${q.id}"><div style="font-weight:600;margin-bottom:6px">${escapeHtml(q.question)}</div>${opts}</div>`;
        }).join('');
      }
    } else {
    ...
    // 2) load attempt count and enforce max attempts
    const attemptsCount = await fetchAttemptCount(jobId);
    const attemptsSummaryEl = modal.querySelector('#attempt-summary');
    if (!modal.querySelector('#attempt-summary')) {
      const s = document.createElement('div');
      s.id = 'attempt-summary';
      s.style = 'margin-top:10px;color:#666;font-size:.9rem';
      wrap.after(s);
    }
    modal.querySelector('#attempt-summary').innerText = `Attempts: ${attemptsCount} / ${QUIZ_MAX_ATTEMPTS}`;

    if (attemptsCount >= QUIZ_MAX_ATTEMPTS) {
      // disable all radios and submit
      modal.querySelectorAll('input[type="radio"]').forEach(i=>i.disabled = true);
    ...
    // 4) when modal closed make sure to stop timer
    const closeBtn = modal.querySelector('#quiz-modal-close');
    if (closeBtn) {
      closeBtn.onclick = function(){
        if (timerControl && timerControl.stop) timerControl.stop();
        modal.style.display = 'none';
      };
    }
  };

  // Enhance submit button behaviour so we show result panel and update attempts UI
  const origOnQuizSubmit = window.onQuizSubmit;
  window.onQuizSubmit = async function(jobId, answers) {
    // call original if exists and returns data
    if (typeof origOnQuizSubmit === 'function') {
      await origOnQuizSubmit(jobId, answers);
      // orig handler already calls handleQuizSubmitResponse etc.
      return;
    }
    ...
    try {
      ...
      if (!r.ok) {
        const txt = await r.text().catch(()=>null);
        showToast('Quiz submit failed', 'error', 4000);
        console.warn('quiz submit failed', r.status, txt);
        return;
      }
      const data = await r.json();
      // show results panel
      const modal = document.querySelector('#quiz-modal');
      renderResultPanel(modal, {
      ...
      // update history UI + badges (existing function)
      if (typeof handleQuizSubmitResponse === 'function') handleQuizSubmitResponse(jobId, data);
      // stop timer if running
      if (modal && modal._quizTimerInterval) { clearInterval(modal._quizTimerInterval); modal._quizTimerInterval = null; }
    } catch (e) {
...
// ---------- start timer and setup auto-submit ----------
try {
  // use backend-provided time_limit if available in quizData, else default 120 sec
  const timeLimit = (typeof quizData !== 'undefined' && quizData && (quizData.time_limit_seconds || quizData.time_limit)) ?
                      Number(quizData.time_limit_seconds || quizData.time_limit) : 120;

  // start timer and auto-submit on expiry
  const timerCtrl = startQuizTimer(timeLimit, async () => {
    try {
      showToast('Time up — auto submitting quiz', 'info', 3000);
      const modalEl = document.querySelector('#quiz-modal');
      if (!modalEl) return;
      const jobIdNow = parseInt(modalEl.dataset.jobId, 10);
      const answers = {};
      modalEl.querySelectorAll('.quiz-question').forEach(q => {
      ...
      // call existing onQuizSubmit if present
      if (typeof onQuizSubmit === 'function') {
        await onQuizSubmit(jobIdNow, answers);
      } else {
        // fallback direct POST
        const token = localStorage.getItem('token') || '';
        const headers = { 'Content-Type': 'application/json' };
        if (token) headers['Authorization'] = 'Bearer ' + token;
        const r = await fetch(`/api/quiz/${jobIdNow}/attempt/`, {
          method: 'POST',
          headers,
          body: JSON.stringify({ answers })
        });
        if (r.ok) {
          const data = await r.json();
          if (typeof handleQuizSubmitResponse === 'function') handleQuizSubmitResponse(jobIdNow, data);
        } else {
          console.warn('Auto-submit failed', r.status);
        }
      }
    } catch (e) { console.error('auto-submit error', e); }
  });
} catch (e) {
  console.warn('start timer failed', e);
