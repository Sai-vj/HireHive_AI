...
if (inviteSendBtn) inviteSendBtn.addEventListener('click', async () => {
  const jobId = inviteJobIdInput.value || inviteContext.jobId;
  ...
  if (!candidateId || !jobId) { alert('Candidate / Job missing'); return; }

  inviteSendBtn.disabled = true;
  try {
    const res = await apiFetchSimple(`/api/interviews/recruiter/${jobId}/invite/`, {
      method: 'POST',
    ...
    if (res.ok) {
      ...
      console.error(res);
      alert('Invite failed: ' + (res.data?.detail || res.status));
    }
  } finally {
