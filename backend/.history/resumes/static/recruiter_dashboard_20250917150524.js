...
async function confirmAndDeleteJob(jobId) {
  ...
  // try get token (if you save in localStorage)
  const token = localStorage.getItem('token') || localStorage.getItem('recruiter_token_v1') || '';

  // try get CSRF token from cookie (if using session auth)
  ...
  // possible URLs to try (adjust according to your backend)
  const tryUrls = [
  ...
  for (const url of tryUrls) {
    try {
      ...
      if (resp.ok) {
        ...
        if (card) card.remove();
        if (window.selectedJob && String(window.selectJob.id) === String(jobId)) {
      ...
      if (resp.status === 405) {
        // method not allowed, try next URL or fallback to POST delete below
        continue;
      }
      // non-ok but not 401/405 -> try parse server message for dev
      const text = await resp.text().catch(()=>null);
      console.warn('Delete failed', resp.status, text);
    } catch (e) {
      console.warn('Network error deleting', e);
    }
  }

  // 2) fallback: try POST to /delete/ endpoints (some backends expect POST)
  for (const base of tryUrls) {
    const alt = base.endsWith('/delete/') ? base : (base.endsWith('/') ? base + 'delete/' : base + '/delete/');
    try {
      const resp = await fetch(alt, { method: 'POST', headers, credentials: 'include', body: JSON.stringify({ id: jobId }) });
      if (resp.ok) {
      ...
      if (resp.status === 401 || resp.status === 403) { showToast('Unauthorized — check token', 'error'); return; }
      const txt = await resp.text().catch(()=>null);
      console.warn('POST-delete failed', resp.status, txt);
    } catch (e) {
      console.warn('Network error POST-delete', e);
    }
  }

  // if reached here -> failed
  showToast('Delete failed — check server/CORS/auth', 'error', 6000);
  btn.addEventListener('click',()=>confirmAndDeleteJob(j.id))
}
/* ---------------- Matches ---------------- */
