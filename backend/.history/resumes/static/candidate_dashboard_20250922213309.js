...
import { apiFetchAsJson, fetchWithAuth, clearTokens } from "./utils.js";
...
(function () {
  ...
  // ----------------- safe wrappers (cookie-first) -----------------
  // ----------------- safe wrappers (cookie-first, hardened) -----------------
...
async function _fetchWithAuth(url, opts = {}) {
  ...
  // always send cookies for same-origin APIs
  opts.credentials = opts.credentials || 'same-origin';

  // normalize headers object
  opts.headers = Object.assign({}, opts.headers || {});

  // If an access cookie exists, prefer cookie auth: remove Authorization header
  try {
    // simple cookie check — HttpOnly cookie won't be readable, but name presence is enough to decide
    const hasAccessCookie = document.cookie.split(';').some(c => c.trim().startsWith('access='));
    if (hasAccessCookie && opts.headers['Authorization']) {
      // remove Authorization so server uses cookie instead
  ...
  // prefer wrapper if provided by page
  if (typeof window.fetchWithAuth === 'function') {
    try { return await window.fetchWithAuth(url, opts); } catch (e) { console.debug('fetchWithAuth failed', e); }
  }
  ...
  async function _apiFetch(url, opts = {}) {
    // prefer global apiFetch wrappers if available (they may return normalized objects)
    try {
    ...
    try {
      ...
      // If wrapper returned normalized object, pass it through
      if (r && typeof r.ok === 'boolean' && 'status' in r) return r;
...
(async function autoCookieAuthCookieOnly() {
  // minimal selectors (use sidebar area to insert logout button/status)
  const sidebar = document.querySelector('.sidebar') || document.querySelector('#sidebar') || document.body;
  let statusEl = document.getElementById('cookieAuthStatus');
  if (!statusEl) {
    statusEl = document.createElement('div');
    statusEl.id = 'cookieAuthStatus';
    statusEl.className = 'small-muted mb-2';
    sidebar.prepend(statusEl);
  }
  ...
  // safe-run helper for optional refresh functions
  async function safeCall(fn) {
    if (typeof fn === 'function') {
      try { await fn(); }
      catch (e) { console.debug(`${fn.name || 'anonymous'} failed`, e); }
    }
  }

  // run known refreshers if available
  async function runRefreshes() {
    await safeCall(typeof refreshJobs !== 'undefined' ? refreshJobs : null);
    await safeCall(typeof refreshResumes !== 'undefined' ? refreshResumes : null);
    await safeCall(typeof refreshMyApplications !== 'undefined' ? refreshMyApplications : null);
    await safeCall(typeof refreshInvites !== 'undefined' ? refreshInvites : null);
    // add any other refresh functions you use
  }

  // check auth using your wrapper if available, otherwise fetch with credentials
  async function checkAuth() {
    try {
      ...
      // fallback fetch: ensure backend accepts cookies for this origin
      const r = await fetch('/api/resumes/my-resumes/', { method: 'GET', credentials: 'same-origin' });
      return r.ok;
    } catch (e) {
  ...
  // Logout: call server logout endpoint to clear cookie (use wrapper if present)
  async function doLogout() {
    setStatus('Logging out...');
    try {
      if (typeof _fetchWithAuth === 'function') {
        // some wrappers return normalized { ok, status, data } — adapt if needed
        const r = await _fetchWithAuth('/accounts/token/logout/', { method: 'POST', credentials: 'same-origin' });
        if (r && typeof r.ok === 'boolean' && !r.ok) throw new Error('logout failed');
      } else {
        const r = await fetch('/accounts/token/logout/', { method: 'POST', credentials: 'same-origin' });
  ...
  // create logout button next to status
  ...
  // on authenticated -> hide any token-input elements in DOM (in case existed)
  function removeTokenUIIfAny() {
    const tokenEls = document.querySelectorAll('#token, #tokenInput, .token-card, .token-ui');
    tokenEls.forEach(el => el.remove());
  }

  // init flow
  async function init() {
    setStatus('Checking authentication (cookies)...');
    ...
    if (ok) {
      ...
      setStatus('Not authenticated — please login (server cookie required).', true);
      // Keep token UI removed by design; show short hint only.
    }
    createLogoutBtn();
  }

  // run immediately then re-check periodically
  await init();
  setInterval(async () => {
    const ok = await checkAuth();
    if (ok) {
