// candidate_dashboard.js -- Candidate-only dashboard (full, updated)
// Minimal changes: robust modal show/hide helpers to avoid aria-hidden/focus freezes.
...
(function () {
  ...
  // ----------------- Modal helpers (NEW) -----------------
  // Use Bootstrap API when available; otherwise fallback to accessible class toggles.
  function _showModalElement(modalEl) {
    if (!modalEl) return;
    try {
      // If it's already managed by Bootstrap, get or create instance and show
      const inst = (window.bootstrap && bootstrap.Modal) ? bootstrap.Modal.getOrCreateInstance(modalEl, { backdrop: true }) : null;
    ...
    // fallback: add accessible attributes + show
    try {
      ...
      // add backdrop element if missing
      let bb = document.querySelector('.__custom_modal_backdrop');
      if (!bb) {
        bb = document.createElement('div');
        bb.className = '__custom_modal_backdrop';
        bb.style.position = 'fixed';
        bb.style.inset = '0';
        bb.style.background = 'rgba(0,0,0,0.45)';
        bb.style.zIndex = 99997;
        document.body.appendChild(bb);
      }
    } catch (e) { /* ignore */ }
  }
  function _hideModalElement(modalEl) {
    if (!modalEl) return;
    try {
    ...
    try {
      ...
      const bb = document.querySelector('.__custom_modal_backdrop');
      if (bb) bb.remove();
    } catch (e) { /* ignore */ }
  }
  ...
  function openJobDetailModal(job) {
    ...
    applyBtn.onclick = () => { try { _hideModalElement(modal); } catch(e){}; openApplyModal(job.id || job.pk); };
  ...
  async function openQuizModal(jobId) {
    ...
    // use our _showModalElement so aria-hidden/focus handled consistently
    _showModalElement(modal);
  ...
  function closeQuizModal() { try { stopQuizTimer(); const modal = document.getElementById('quizModal'); if (modal) _hideModalElement(modal); } catch (e) {} }
  ...
  async function viewInvite(inviteId) {
    ...
    try {
      const fragUrl = `/interviews/fragments/invite_row/${inviteId}/`;
  ...
  async function respondInvite(inviteId, action) {
    ...
    try {
      ...
      await loadInvites();
      const detailModal = document.getElementById('inviteDetailModal'); if (detailModal) _hideModalElement(detailModal);
      const invitesModal = document.getElementById('invitesModal'); if (invitesModal) _hideModalElement(invitesModal);
    } catch (e) {
  ...
  function init() {
    ...
    document.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          try { const m = document.getElementById('applyModal'); _hideModalElement(m); } catch(e){}
          loadMyApplications();
        } else {
    ...
    // invites modal auto-create if missing (only if page template missing it)
    if (!document.getElementById('invitesModal')) {
      ...
      m.querySelector('#invitesClose').addEventListener('click', ()=> _hideModalElement(m));
    } else {
      // if invitesModal exists (from template), hook its close button to our helper
      const m = document.getElementById('invitesModal');
      const btn = m?.querySelector('#invitesModalClose') || m?.querySelector('[data-bs-dismiss="modal"]');
      if (btn) btn.addEventListener('click', () => _hideModalElement(m));
    }

    // view invites button wiring
    document.getElementById('viewInvitesBtn')?.addEventListener('click', () => {
      const m = document.getElementById('invitesModal');
      if (m) _showModalElement(m);
      loadInvites();
    });
