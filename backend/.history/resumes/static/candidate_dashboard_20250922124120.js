...
(function () {
  ...
  async function loadJobs() {
    ...
    jobs.forEach(j => {
      ...
      card.innerHTML = `<div style="min-width:0"><strong>${escapeHtml(j.title||`Job ${id}`)}</strong><div class="small-muted">${escapeHtml(j.company||'')} • ${escapeHtml(j.skills_required||j.skills||'')}</div></div>
        <div style="display:flex;flex-direction:column;gap:6px;align-items:flex-end">
          <div>
            <button class="btn btn-sm btn-outline-primary view-job-btn" data-id="${escapeHtml(id)}">View</button>
            <button class="btn btn-sm btn-outline take-quiz-btn" data-id="${escapeHtml(id)}">Take Quiz</button>
            <button class="btn btn-sm btn-success apply-btn" data-id="${escapeHtml(id)}" disabled>Apply</button>
          </div>
          <div style="text-align:right"><span id="quiz-status-${escapeHtml(id)}" class="small text-muted">Not attempted</span></div>
        </div>`;
      el.appendChild(card);
      const viewBtn = card.querySelector('.view-job-btn'); if (viewBtn && !viewBtn._bound) { viewBtn._bound = true; viewBtn.addEventListener('click', ()=> viewJob(id)); }
