...
(function () {
  ...
  function processInvitesData(raw) {
    ...
    if (sidebarWrap) {
      ...
      if (!invites.length) {
        ...
        invites.forEach(inv => {
          // resolve id robustly
          const id = inv.id || inv.raw?.id || inv.raw?.invite_id || inv.raw?.pk || '';
          ...
          // --- robust wiring using resolved id/interview ---
          const resolvedId = id || inv.raw?.id || inv.raw?.invite_id || '';

          // Accept
          div.querySelectorAll('.accept-invite').forEach(b => {
          ...
          // Decline
          div.querySelectorAll('.decline-invite').forEach(b => {
            b.dataset.id = resolvedId;
            b.addEventListener('click', async () => {
              b.disabled = true;
              try {
          ...
          // Start
          div.querySelectorAll('.start-invite').forEach(b => {
            // ensure dataset contains a valid interview + invite id fallback
            const iidFallback = inv.interview && (inv.interview.id || inv.interview_id) || inv.raw?.interview || inv.raw?.interview_id || '';
            b.dataset.interview = b.dataset.interview || iidFallback;
            b.dataset.invite = b.dataset.invite || resolvedId;
            b.addEventListener('click', async () => {
              b.disabled = true;
          ...
          // View
          div.querySelectorAll('.view-invite').forEach(b => {
            b.dataset.id = b.dataset.id || resolvedId;
  ...
  async function viewInvite(inviteId) {
    console.log('viewInvite called with inviteId=', inviteId);
