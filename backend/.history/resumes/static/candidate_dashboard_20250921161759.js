...
(function () {
  ...
  function safeShowModal(modal) {
    ...
    try {
      ...
      console.warn('bootstrap show failed, fallback to plain display', e);
    }
  ...
  function attachGlobalModalCloseHandlers() {
    // attach to all elements that should dismiss modals
      ...
      btn.addEventListener('click', function () {
        ...
        if (modal) safeHideModal(modal);
          ...
          document.querySelectorAll('.modal').forEach(m => {
            if (m && (m.style.display === 'block' || m.classList.contains('show'))) safeHideModal(m);
          });
        }
      });
    });
    // also wire any explicit "close" buttons with class .modal-close (optional)
    document.querySelectorAll('.modal-close').forEach(b => {
      if (b.__closeHandlerAttached) return;
      b.__closeHandlerAttached = true;
