...
console.log("candidate_dashboard.js (improved) loaded");
...
async function loadJobs() {
  ...
  jobs.forEach(j => {
    ...
    card._job = j;
    // use addEventListener to avoid accidental navigation
    const left = document.createElement('div');
    left.innerHTML = `<strong>${escapeHtml(j.title)}</strong><div class="small-muted">${escapeHtml(j.company || '')} • ${escapeHtml(j.skills_required || '')}</div>`;
    const right = document.createElement('div');
    const btnView = document.createElement('button');
    btnView.className = 'btn btn-sm btn-outline-primary me-1';
    btnView.type = 'button';
    btnView.textContent = 'View';
    btnView.addEventListener('click', (ev)=>{ ev.preventDefault(); selectJobById(j.id); });

    const btnApply = document.createElement('button');
    btnApply.className = 'btn btn-sm btn-success';
    btnApply.type = 'button';
    btnApply.textContent = 'Apply';
    btnApply.addEventListener('click', (ev)=>{ ev.preventDefault(); openApplyModal(j.id); });
...
async function loadMyApplications() {
  ...
  // candidate endpoints tries - prefer specific 'my' endpoint, else fall back to generic and filter
  const tries = [
    '/api/resumes/applications/my/',
  ...
  for (const url of tries) {
    try {
      ...
      // Accept success OR array returned (some endpoints return 200 with array)
  ...
  // if endpoint returned ALL applications, try filter by current user (if server returned candidate id or username)
  // NOTE: server filtering is preferred; this is fallback only
  if (apps.length && !tries[0].endsWith('/my/') && !tries.includes('/api/resumes/applications/my/')) {
    // attempt to filter by token user cannot be easily determined client-side - skip filtering
  }
    ...
    card.innerHTML = `
      <div class="d-flex justify-content-between align-items-start">
        ...
        <div style="min-width:120px;text-align:right;">
          ${a.job ? `<a class="btn btn-sm btn-outline-primary me-1" href="/jobs/${a.job.id || a.job}/" target="_blank">View Job</a>` : ''}
