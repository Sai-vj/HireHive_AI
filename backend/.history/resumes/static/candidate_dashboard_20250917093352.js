...
window.refreshJobAttempt(jobId);
...
// Show Attempt History Modal
async function openAttemptHistoryModal(jobId) {
  const modal = document.getElementById("attempt-history-modal");
  modal.style.display = "flex";
  document.getElementById("attempt-history-title").textContent = `Quiz Attempts – Job #${jobId}`;
  const wrap = document.getElementById("attempt-history-content");
  ...
  try {
    ...
    const r = await fetch(`/api/quiz/${jobId}/attempts/`, { method: "GET", headers });
    if (!r.ok) {
      wrap.innerHTML = `<div class="text-danger">Error loading attempts</div>`;
      return;
    }
    ...
    if (!attempts.length) {
      wrap.innerHTML = `<div>No attempts found.</div>`;
      return;
    }
    ...
    const rows = attempts.map(a => {
      ...
      const passTxt = a.passed ? `<span style="color:green">Passed</span>` : `<span style="color:crimson">Failed</span>`;
      return `
        <tr>
          <td>${a.id}</td>
          <td>${a.score}</td>
          <td>${a.total || a.total_questions}</td>
          <td>${passTxt}</td>
          <td>${when}</td>
          <td><button class="btn btn-sm btn-info view-details-btn" data-attempt-id="${a.id}">View</button></td>
        </tr>
        <tr id="attempt-detail-${a.id}" style="display:none;"><td colspan="6">
          <div style="padding:8px;border:1px solid #ddd;background:#fafafa;">
            ${renderAttemptAnswers(a)}
          </div>
        </td></tr>
      `;
    }).join("");
    ...
    wrap.innerHTML = `
      <table class="table table-bordered">
        <thead>
          <tr><th>ID</th><th>Score</th><th>Total</th><th>Result</th><th>Finished</th><th></th></tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
    `;

    // attach detail togglers
    wrap.querySelectorAll(".view-details-btn").forEach(btn => {
      btn.addEventListener("click", () => {
        const id = btn.dataset.attemptId;
        const row = document.getElementById(`attempt-detail-${id}`);
...
// Render per-attempt answers
function renderAttemptAnswers(attempt) {
  if (!attempt.answers) return `<div>No answers recorded</div>`;
  return `
    <table class="table table-sm">
      <thead><tr><th>QID</th><th>Your Answer</th></tr></thead>
      <tbody>
        ${Object.entries(attempt.answers).map(([qid, ans]) => {
          return `<tr><td>${qid}</td><td>${ans || "-"}</td></tr>`;
        }).join("")}
      </tbody>
    </table>
  `;
...
// Close modal
document.getElementById("attempt-history-close").addEventListener("click", () => {
  document.getElementById("attempt-history-modal").style.display = "none";
});
