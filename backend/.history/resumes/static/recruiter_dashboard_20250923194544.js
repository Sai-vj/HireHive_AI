...
function safeShowBootstrapModal(modalEl, options = {}) {
  ...
  try {
    if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
      ...
      try {
        // call show on next frame with correct binding, fallback to plainShow on error
        requestAnimationFrame(() => {
          try {
            if (typeof inst.show === 'function') {
              try { inst.show.call(inst); return; } catch (callErr) { console.warn('inst.show.call failed', callErr); }
            }
...
function safeHideBootstrapModal(modalEl) {
    ...
    if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
      try {
        ...
        if (inst && typeof inst.hide === 'function') {
          inst.hide();
          // remove any leftover custom backdrop if present
  ...
  // remove custom backdrop(s)
  document.querySelectorAll('.modal-backdrop.custom-rdash-backdrop').forEach(el=>el.remove());
  // also remove any bootstrap backdrops if present
  document.querySelectorAll('.modal-backdrop').forEach(el=>{
    if (!el.classList.contains('custom-rdash-backdrop')) el.remove();
  });
  document.body.style.overflow = '';
}
