...
/* ---- Replace safeShowBootstrapModal with this safer version ---- */
function safeShowBootstrapModal(modalEl, options = {}) {
  if (!modalEl) return null;

  // If modal element is not in DOM, append it to body first
  if (!document.body.contains(modalEl)) {
    document.body.appendChild(modalEl);
  }

  // Ensure element has modal class & attributes to satisfy bootstrap internals
  try {
  ...
  if (typeof bootstrap !== 'undefined' && typeof bootstrap.Modal === 'function') {
    try {
      // Use getInstance when possible, else construct new instance (element must be in DOM)
      let inst = null;
      try { inst = bootstrap.Modal.getInstance(modalEl); } catch(e) { inst = null; }
      if (!inst) {
        inst = new bootstrap.Modal(modalEl, Object.assign({ backdrop: 'static' }, options));
      }
      inst.show();
      return inst;
    } catch (e) {
      // Defensive fallback: don't let bootstrap errors break app
      console.warn('bootstrap.Modal show failed (fallback to plain display):', e);
    }
  }

  // Fallback: simple show by toggling styles (non-bootstrap)
  try {
...
/* ---- Replace ensureInviteModal with this bootstrap-friendly creator ---- */
function ensureInviteModal() {
  ...
  // Bootstrap canonical classes/attributes
  ...
  // append to body (important: in DOM before bootstrap uses it)
  document.body.appendChild(modal);

  // wire small behaviour: bootstrap will wire dismissal via data-bs-dismiss, but we add click listener for send
  const sendBtn = modal.querySelector('#inviteSendBtn');
  if (sendBtn) {
    sendBtn.addEventListener('click', async (ev) => {
      ev.preventDefault();
      // reuse existing logic you already had in ensureInviteBindings/send handler
      ...
      const jobId = modal.dataset.jobId || (selectedJob && selectedJob.id) || null;
      if (!candidateId) { alert('Enter candidate id'); return; }
      if (!jobId) { alert('No job selected'); return; }

      sendBtn.disabled = true;
      try {
        const res = await apiFetchSimple(INTERVIEWS_INVITE_BY_JOB(jobId), { method: 'POST', body: JSON.stringify({ candidate_id: candidateId, scheduled_at: scheduled, message }) });
        if (res.ok) {
          showToast('Invite sent', 'success');
          // hide bootstrap modal properly
          safeHideBootstrapModal(modal);
          // optionally refresh apps
...
async function submitAddJob(e) {
  ...
  try {
    ...
    // after you have a valid `res` variable
const createdJob = res?.data || null;
console.log("Job API response (res):", res);               // entire wrapper object
console.log("Created Job object (res.data):", createdJob); // actual job object (may be null)
const newId = createdJob?.id || editingId || null;
console.log("New job id:", newId);
    ...
    if (newId) {
      ...
      // Optional: auto-generate quiz for the newly created job
      // uncomment next line if you want auto-generation:
 ...
 // show list (fixed: use s.job.id / s.resume.id and better display)
async function showShortlistsForSelectedJob() {
  ...
  list.forEach(s => {
    ...
    div.innerHTML = `
      <div class="d-flex justify-content-between">
        <div>
          <strong>Resume #${escapeHtml(String(resumeDisplay))}</strong>
          <div class="small-muted">${escapeHtml(String(shortlistedBy))}</div>
        </div>
        <div>
          <button class="btn btn-sm btn-outline-primary resend-btn" data-job="${escapeHtml(String(jobId))}" data-resume="${escapeHtml(String(resumeDisplay))}">Resend</button>
...
// current delete (works with your POST-decode DELETE route)
// ensure apiFetch sets Content-Type: application/json (if not, see alt below)
async function removeShortlist(id) {
  ...
  const res = await apiFetch('/api/resumes/shortlist/', {
    ...
    headers: { 'Content-Type': 'application/json' } // add if apiFetch doesn't set it
  });
  if (res.ok) { showToast('Removed', 'success'); showShortlistsForSelectedJob(); }
  else showToast('Remove failed', 'error');
}

// preferred: DELETE-by-path (if you added endpoint /shortlist/<id>/)
...
// resend (send numeric ids; ensure backend expects job_id & resume_id ints)
async function resend(job_id, resume_id) {
  const payload = { job_id: Number(job_id), resume_id: Number(resume_id), resend: true };
  const res = await apiFetch('/api/resumes/shortlist/', {
    method: 'POST',
    body: JSON.stringify(payload),
    headers: { 'Content-Type': 'application/json' } // add if apiFetch doesn't
  });
