...
// Uses imported helpers if available; otherwise falls back to safe wrappers.
// Do not change server URLs in this file.
...
(function () {
  ...
  // ----------------- helpers: safe wrappers (do not overwrite imported names) -----------------
  // If your ./utils.js exported fetchWithAuth and apiFetchAsJson, we will use them.
  // Otherwise fallback implementations will be used.

  // safe fetchWithAuth wrapper
  async function fetchWithAuthFallback(url, options = {}) {
    if (typeof fetchWithAuth === 'function') return fetchWithAuth(url, options);

    const headers = Object.assign({}, options.headers || {});
    const token = localStorage.getItem('token') || localStorage.getItem('access_token') || '';
    if (token && !headers['Authorization']) headers['Authorization'] = 'Bearer ' + token;
    const opts = Object.assign({}, options, { headers, credentials: options.credentials || 'include' });
    return fetch(url, opts);
  }

  // safe apiFetch wrapper returning normalized { ok, status, data }
  async function apiFetchFallback(url, opts = {}) {
    if (typeof apiFetch === 'function') {
      // if imported apiFetch is a function, call it
      try {
  ...
  // canonical functions used throughout this file
  const _fetchWithAuth = (typeof fetchWithAuth === 'function') ? fetchWithAuth : fetchWithAuthFallback;
  const _apiFetch = (typeof apiFetch === 'function') ? apiFetch : apiFetchFallback;
  ...
  async function submitQuizAttempt(auto = false) {
    ...
    try {
      ...
      try {
        ...
        // fallback using apiFetch normalized wrapper
        res = await _apiFetch(API.QUIZ_ATTEMPT_SUBMIT, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      }
      // normalize response
      let body = null;
      if (res && res.ok !== undefined) { body = res.data; }
      else {
  ...
  function _attachGlobalModalCloseHandlers() {
    if (typeof window.attachGlobalModalCloseHandlers === 'function') return window.attachGlobalModalCloseHandlers();
  }

  function attachGlobalModalCloseHandlers() {
    document.querySelectorAll('.modal').forEach(modalEl => {
      if (modalEl._globalCloseBound) return;
      modalEl._globalCloseBound = true;
      modalEl.addEventListener('click', (ev) => {
        if (ev.target === modalEl) {
          try { _hideModal(modalEl); } catch(e){ modalEl.style.display='none'; }
        }
      });
      window.addEventListener('keydown', (ev) => {
        if (ev.key === 'Escape') {
          if (modalEl.style && modalEl.style.display && modalEl.style.display !== 'none') {
            try { _hideModal(modalEl); } catch(err){ modalEl.style.display='none'; }
          }
        }
      });
    });
  ...
  async function loadInvites() {
    ...
    if (!res || !res.ok) {
      try {
        const alt = await _apiFetch('/interviews/candidate/invites/');
  ...
  function processInvitesData(raw) {
    ...
    if (sidebarWrap) {
      ...
      // ensure buttons have robust id fallbacks (use raw id fields if normalized id missing)
      const resolvedId = id || inv.raw?.id || inv.raw?.invite_id || '';
      // wire handlers - attach to the elements and ensure dataset values are set
      const acceptBtns = div.querySelectorAll('.accept-invite');
      acceptBtns.forEach(b => { b.dataset.id = resolvedId; b.addEventListener('click', async () => {
        b.disabled = true;
        try {
          const r = await _apiFetch(API.INVITE_RESPOND(resolvedId), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ response: 'accept' })
          });
          if (r && r.ok) { showToast('Accepted', 'success'); await loadInvites(); const detailModal = document.getElementById('inviteDetailModal'); if (detailModal) _hideModal(detailModal); }
      ...
      const declineBtns = div.querySelectorAll('.decline-invite');
      declineBtns.forEach(b => { b.dataset.id = resolvedId; b.addEventListener('click', async () => {
        b.disabled = true;
        try {
          const r = await _apiFetch(API.INVITE_RESPOND(resolvedId), {
            method: 'POST',
      ...
      const startBtns = div.querySelectorAll('.start-invite');
      startBtns.forEach(b => { 
        // ensure dataset contains a valid interview + invite id fallback
        const iid = b.dataset.interview || (inv.interview && (inv.interview.id || inv.interview_id)) || inv.raw?.interview || inv.raw?.interview_id || '';
        b.dataset.interview = iid;
        b.dataset.invite = resolvedId;
        b.addEventListener('click', async () => {
          b.disabled = true;
          const iidLocal = b.dataset.interview;
          const inviteIdLocal = b.dataset.invite;
          try {
            const r = await _apiFetch(API.START_INTERVIEW(iidLocal), { method: 'POST', headers: { 'Content-Type':'application/json' }, body: JSON.stringify({ invite: inviteIdLocal }) });
            if (r && r.ok && (r.data?.redirect_url || r.data?.join_url || r.data?.url)) {
              ...
              window.location.href = `${INTERVIEWS_BASE}/page/candidate/${iidLocal}?invite=${encodeURIComponent(inviteIdLocal)}`;
            }
          } catch (e) {
      ...
      const viewBtns = div.querySelectorAll('.view-invite');
      viewBtns.forEach(b => {
        // ensure dataset.id present
        b.dataset.id = resolvedId;
        b.addEventListener('click', () => viewInvite(b.dataset.id));
      });}


    // modal wrap rendering:
    const modalWrap = document.getElementById('invitesListModal');
    if (modalWrap) {
      modalWrap.innerHTML = '';
      if (!invites.length) {
  ...
  // --- startInterview fallback (ensures defined)
  if (!window.startInterview) {
    window.startInterview = async function (interviewId, inviteId = null) {
      console.warn('Fallback startInterview called.', { interviewId, inviteId });
      try {
        const base = (typeof INTERVIEWS_BASE !== 'undefined') ? INTERVIEWS_BASE : '/interviews';
  ...
  (function autoSaveTokenFromInput() {
    try {
      ...
      if ((typeof getAccessToken === 'function' && getAccessToken()) || localStorage.getItem('token')) {
        if (tokenInputEl) tokenInputEl.style.display = 'none';
        if (saveBtn) saveBtn.style.display = 'none';
      }
    } catch (err) {
  ...
  (function autoAuthInit() {
    try {
      const tokenInputEl = document.getElementById('tokenInput');
      const saveBtn = document.getElementById('saveTokenBtn');
      if ((typeof getRefreshToken === 'function' && getRefreshToken()) || (typeof getAccessToken === 'function' && getAccessToken()) || localStorage.getItem('token')) {
        if (tokenInputEl) tokenInputEl.style.display = 'none';
        if (saveBtn) saveBtn.style.display = 'none';
      }
      if (typeof refreshAccessToken === 'function') refreshAccessToken().catch(()=>{});
    } catch (e) {
  ...
  // expose some functions to console for debugging (single assignment)
  window.cdb = {
