...
async function showShortlistsForSelectedJob() {
  ...
  list.forEach(s => {
    ...
    // defensive: s.job and s.resume may be objects; prefer ids
    const jobId = (s.job && (s.job.id || s.job)) || s.job_id || s.job;
    const resumeId = (s.resume && (s.resume.id || s.resume)) || s.resume_id || s.resume;
    const shortlistedBy = (s.shortlisted_by && (s.shortlisted_by.username || s.shortlisted_by)) || s.shortlisted_by || '';
    div.innerHTML = `<div class="d-flex justify-content-between align-items-start">
      <div><strong>Resume #${escapeHtml(String(resumeId))}</strong> — by ${escapeHtml(shortlistedBy)}<div class="small-muted">created: ${escapeHtml(s.created_at || '')}</div></div>
      <div><button class="btn btn-sm btn-outline-primary" type="button" onclick="resend(${jobId}, ${resumeId})">Resend</button> <button class="btn btn-sm btn-outline-danger" type="button" onclick="removeShortlist(${s.id})">Remove</button></div>
    </div>`;
    container.appendChild(div);
  });
