...
async function showShortlistsForSelectedJob() {
  ...
  // helper: try to read candidate name from various possible shapes
  function guessCandidateName(s) {
    // direct fields
    if (!s) return null;
    const tryList = [
      () => s.candidate_name,
      () => s.candidate || s.candidate_id,
      () => s.user || s.username || s.uploader,
      () => (s.resume && (s.resume.candidate_name || s.resume.name || s.resume.owner || s.resume.uploaded_by || s.resume.user_name || s.resume.user)) ,
      () => (s.shortlisted_by || s.added_by),
      () => (s.resume && s.resume.user && (s.resume.user.full_name || s.resume.user.name || s.resume.user.username)),
      () => (s.resume && typeof s.resume === 'string' ? s.resume : null)
    ];
  ...
  list.forEach(s => {
    // resumeDisplay: prefer human name if available, else resume id or resume string
    const candidateName = guessCandidateName(s);
    let resumeDisplay = 'N/A';
    if (candidateName) {
      resumeDisplay = candidateName;
    } else if (s.resume && (s.resume.id || typeof s.resume === 'string')) {
      resumeDisplay = s.resume.id || s.resume;
    } else if (s.id) {
      resumeDisplay = `#${s.id}`;
    }
    ...
    div.innerHTML = `
      <div class="d-flex justify-content-between align-items-start">
        <div style="min-width:0">
          <strong>${escapeHtml(String(resumeDisplay))}</strong>
          ${ shortlistedBy ? `<div class="small-muted">${escapeHtml(String(shortlistedBy))}</div>` : '' }
        </div>
        <div>
