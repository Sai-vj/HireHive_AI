...
const token = localStorage.getItem('token'); // or your TOKEN_KEY
...
fetch(url, {
  method: 'DELETE',
  headers,
  credentials: 'include' // if you need cookies as well
});
...
async function confirmAndDeleteJob(jobId) {
  ...
  // candidate URLs to try (adjust/remove ones that don't apply)
  const tryUrls = [
    `/api/resumes/jobs/${encodeURIComponent(jobId)}/`,                 // typical REST DELETE
    `/api/jobs/${encodeURIComponent(jobId)}/`,                         // alternative
    `/api/jobs/recruiter/job/${encodeURIComponent(jobId)}/delete/`,    // earlier code used this
    `/api/resumes/job/${encodeURIComponent(jobId)}/delete/`,
  ...
  for (const url of tryUrls) {
    try {
      ...
      // not ok -> check status
      if (r.status === 405) {
        // method not allowed on this endpoint, try next
        lastErr = `405 at ${url}`;
        console.warn('DELETE not allowed at', url);
        continue;
      }
      if (r.status === 401 || r.status === 403) {
        lastErr = `Auth error ${r.status} at ${url}`;
        const txt = await r.text().catch(()=>null);
        console.warn('Auth/CORS error', r.status, txt);
        showToast('Unauthorized — check token', 'error', 4000);
        return;
      }

      // Some servers accept POST to a "delete" subpath
      const txt = await r.text().catch(()=>null);
      lastErr = `Failed ${r.status} ${txt} at ${url}`;
      console.warn('Delete failed', url, r.status, txt);

      // if server returned 405 or method not allowed, loop continues
    } catch (err) {
      lastErr = String(err);
      console.warn('Network error trying', url, err);
    }
  } // end for

  // fallback: try POST to <url>/delete/ where delete is explicit
  try {
    for (const base of tryUrls) {
      const altUrl = base.endsWith('/delete/') ? base : (base.endsWith('/') ? base + 'delete/' : base + '/delete/');
      try {
        const r2 = await fetch(altUrl, { method: 'POST', headers: commonHeaders, body: JSON.stringify({ id: jobId }) });
        if (r2.ok) {
          showToast('Job deleted (POST /delete/)', 'success');
          const card = document.querySelector(`.job-card[data-job-id="${jobId}"]`);
          if (card) card.remove();
          return;
        }
        if (r2.status === 401 || r2.status === 403) { showToast('Unauthorized — check token', 'error'); return; }
      } catch (err2) {
        console.warn('POST-delete try failed', altUrl, err2);
      }
    }
  } catch(e){ console.warn(e); }

  console.error('Delete attempts failed:', lastErr);
  showToast('Delete failed. Check server logs or endpoint/method.', 'error', 6000);
}
/* ---------------- Matches ---------------- */
