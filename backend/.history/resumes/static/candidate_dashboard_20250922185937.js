...
(function () {
  ...
  // NOTE: we prefer cookie auth. This wrapper ALWAYS sends credentials by default.
  ...
  async function _fetchWithAuth(url, options = {}) {
    ...
    opts.credentials = opts.credentials || 'same-origin'; // IMPORTANT -> send cookies
    opts.headers = Object.assign({}, opts.headers || {});

    // Only attach Authorization header if an explicit token is stored client-side.
    // This keeps cookie-flow the default, but still supports legacy token paste.
    const token = localStorage.getItem('token') || localStorage.getItem('access_token') || '';
    if (token && !opts.headers['Authorization']) {
      opts.headers['Authorization'] = 'Bearer ' + token;
    }

    // If you have a global fetchWithAuth wrapper that already handles credentials,
    // prefer using it (but still ensure it sets credentials)
    if (hasFetchWithAuth) {
      try {
        const res = await window.fetchWithAuth(url, opts);
        // if user wrapper returns a normal Response, return it; otherwise assume it's normalized
        return res instanceof Response ? res : res;
      } catch (e) {
        console.debug('fetchWithAuth failed, falling back', e);
      }
    }
  ...
  async function _apiFetch(url, opts = {}) {
    // prefer global apiFetch wrappers if present
    try {
      if (hasApiFetch) {
