...
function attachJobCardEvents() {
  // Delegated click handling on jobsList so both server-rendered and JS-injected cards work
  const jobsList = document.getElementById('jobsList');
  if (!jobsList) return;

  if (!jobsList._rdashDelegated) {
    jobsList._rdashDelegated = true;
    ...
    jobsList.addEventListener('click', (ev) => {
      ...
      // VIEW
      if (btn.classList.contains('view-job-btn') || btn.matches('[data-action="view"]') || btn.textContent.trim().toLowerCase() === 'view') {
        ev.stopPropagation();
        const jid = btn.dataset.jobId || btn.getAttribute('data-job-id') || btn.closest('.job-card')?.dataset?.jobId;
        if (jid) openJobDetail(jid);
        return;
      }

      // EDIT
      if (btn.classList.contains('edit-job-btn') || btn.matches('[data-action="edit"]') || btn.textContent.trim().toLowerCase() === 'edit') {
        ev.stopPropagation();
        const jid = btn.dataset.jobId || btn.getAttribute('data-job-id') || btn.closest('.job-card')?.dataset?.jobId;
        if (jid) openEditJob(jid);
        return;
      }

      // DELETE
      if (btn.classList.contains('delete-job-btn') || btn.matches('[data-action="delete"]') || btn.textContent.trim().toLowerCase() === 'delete') {
        ev.stopPropagation();
        const jid = btn.dataset.jobId || btn.getAttribute('data-job-id') || btn.closest('.job-card')?.dataset?.jobId;
        if (jid) confirmAndDeleteJob(jid);
        return;
      }

      // GENERATE QUIZ (fallback)
      if (btn.classList.contains('generate-quiz-btn') || btn.matches('[data-action="generate"]') || btn.textContent.trim().toLowerCase().includes('generate')) {
        ev.stopPropagation();
        const jid = btn.dataset.jobId || btn.getAttribute('data-job-id') || btn.closest('.job-card')?.dataset?.jobId;
        if (jid) handleGenerateFromButton({ btn, jobPk: jid, questionsCount: Number(btn.dataset.questionsCount || 5) || 5 });
        return;
      }
    });

    // also click-on-card => open detail
    jobsList.addEventListener('click', (ev) => {
      const card = ev.target.closest('.job-card');
      if (!card) return;
      const clickedButton = ev.target.closest('button');
      if (clickedButton) return; // already handled above
      const jid = card.dataset.jobId || card.getAttribute('data-job-id');
      if (jid) openJobDetail(jid);
    });
  }

  // keep previously-used binding for review-attempts if present
  ...
  // ensure generate buttons attached for dynamic elements
  try { attachGenerateInterviewButtons(); } catch(e) { console.warn('attachGenerateInterviewButtons error', e); }
}
...
async function openJobDetail(jobId) {
  ...
  async function loadApplicationsForSelectedJob(jobIdParam) {
    ...
    for (const a of apps) {
      ...
      if (resumeId) card.dataset.resumeId = String(resumeId);
    ...
    if (sendBtn && !sendBtn._bound) {
      ...
      sendBtn.addEventListener('click', async (ev) => {
        ...
        try {
          ...
          // CRITICAL: if resumeId exists, prefer it. Only send candidate_id when resume is absent.
          if (hasResume) {
  ...
  /* ---------------- Inject invite buttons ---------------- */

  // robust candidate resolver (keep this single version)
  function resolveCandidateIdFromElement(el) {
    if (!el) return null;
    // dataset keys (many variants)
    const prefer = ['candidateId','userId','candidate_id','user_id','userid','candidateid','id'];
    try {
      ...
      const attrChecks = ['data-candidate-id','data-user-id','data-resume-owner-id','data-resume-user-id','data-uid'];
  ...
  function injectInviteButtonsIntoAppRows(selector = '.application-row') {
    ...
    rows.forEach(row => {
      ...
      // Only attempt to resolve candidate id if no resume id found
      ...
      // attach candidate id only if resolved and numeric (and only when no resume id)
      if (candidateIdResolved) btn.dataset.candidateId = String(candidateIdResolved);

      // attach resume id from row if available (preferred)
      if (resumeIdFromRow) btn.dataset.resumeId = String(resumeIdFromRow);
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    /* ---------------- expose for debugging + global helpers ---------------- */
  // export internal token helpers to global to support legacy callers outside IIFE
  window.rdash = {
  ...
  // also expose simple global functions (backwards compatibility)
  // so top-level functions (outside this IIFE) can safely call savedToken() and setSavedToken()
  try {
    window.savedToken = function() { return savedToken(); };
    window.setSavedToken = function(t) { return setSavedToken(t); };
    window.rdash_savedToken = window.savedToken; // optional alias
  } catch(e) { console.warn('export savedToken failed', e); }


})();

/* ---------------- Interviews: generate questions (Recruiter) ---------------- */
/* ---------- Interviews generate helpers (permanent) ---------- */
...
async function createInterviewForJob(jobPk, payload = {}) {
  ...
  // inside createInterviewForJob (replace token resolution)
  const token = (typeof window.savedToken === 'function') ? window.savedToken() : (localStorage.getItem('recruiter_token_v1') || '');
// ... keep rest of function unchanged (but consider switching to apiFetch if desired)
...
async function generateInterviewQuestions(interviewPk, nQuestions = 25, autoPublish = false) {
  ...
  // inside generateInterviewQuestions (replace token resolution)
  const token = (typeof window.savedToken === 'function') ? window.savedToken() : (localStorage.getItem('recruiter_token_v1') || '');
...
/* ---------- Replace previous single-button / attachGenerateInterviewButtons blocks with this unified version ---------- */

/**
 * Helper: ensure a small status span exists after the button and return it
 */
function _ensureStatusSpan(btn) {
  let s = btn.nextElementSibling;
  if (!s || !s.classList || !s.classList.contains('gen-status')) {
    s = document.createElement('span');
    s.className = 'gen-status';
...
/**
 * Single flow to create interview for a job then generate or just generate for an interview.
 * It updates a status element near the triggering button.
 */
async function handleGenerateFromButton({ btn, jobPk = null, interviewPk = null, questionsCount = 25, autoPublish = true }) {
  const statusEl = _ensureStatusSpan(btn);
  // disable button and show spinner text
  btn.disabled = true;
  statusEl.innerHTML = '<span class="small-spinner" aria-hidden="true"></span> Generating...';

  try {
    // If jobPk provided -> create interview first
    let usedInterviewPk = interviewPk;
    if (!usedInterviewPk && jobPk) {
      const createRes = await createInterviewForJob(jobPk);
      console.log('createInterviewForJob:', createRes);
      if (!createRes || !createRes.ok) {
        const msg = (createRes && (createRes.data?.error || createRes.error || createRes.data)) || 'Create failed';
        statusEl.innerText = `Create failed: ${typeof msg === 'string' ? msg : JSON.stringify(msg)}`;
        if (typeof showToast === 'function') showToast('Interview creation failed', 'error');
        return { ok: false, createRes };
      }
      const created = createRes.data || {};
      usedInterviewPk = created.id ?? created.pk ?? created.pk_id ?? (created?.data && (created.data.id ?? created.data.pk));
      if (!usedInterviewPk) {
        statusEl.innerText = 'No interview id returned';
        if (typeof showToast === 'function') showToast('No interview id returned', 'error');
        return { ok: false, error: 'no interview id' };
      }
    }

    // If still no interview id -> fail
    if (!usedInterviewPk) {
      statusEl.innerText = 'Missing interview id';
      if (typeof showToast === 'function') showToast('Missing interview id', 'error');
      return { ok: false, error: 'missing interview id' };
    }

    // Generate questions
    statusEl.innerHTML = '<span class="small-spinner" aria-hidden="true"></span> Generating questions...';
    const genRes = await generateInterviewQuestions(usedInterviewPk, questionsCount, autoPublish);
    console.log('generateInterviewQuestions:', genRes);

    if (genRes && genRes.ok) {
      // try to read count from response if provided
      const countReported = (genRes.data && (genRes.data.questions?.length ?? genRes.data.questions_count ?? genRes.data.count)) ?? questionsCount;
      statusEl.innerText = `Questions generated (${countReported})`;
      if (typeof showToast === 'function') showToast('Questions generated', 'success');
      // optionally reload interview detail or jobs if functions exist
      try { if (typeof loadInterviewDetail === 'function') await loadInterviewDetail(usedInterviewPk); } catch(e){}
      try { if (typeof loadJobs === 'function') await loadJobs(); } catch(e){}
      return { ok: true, genRes };
    } else {
      const detail = genRes?.data || genRes?.text || genRes?.error || `status ${genRes?.status||'?'}`;
      statusEl.innerText = `Generation failed: ${typeof detail === 'string' ? detail : JSON.stringify(detail)}`;
      if (typeof showToast === 'function') showToast('Generate failed', 'error');
      return { ok: false, genRes };
    }

  } catch (err) {
    console.error('handleGenerateFromButton error', err);
    statusEl.innerText = 'Unexpected error — check console';
    if (typeof showToast === 'function') showToast('Unexpected error generating', 'error');
...
function attachGenerateInterviewButtons() {
  // generic selector: any element that has data-job-pk or data-interview-id or legacy classes
  const selector = '[data-job-pk], [data-interview-id], .generate-quiz-btn, .generate-questions-btn';
  document.querySelectorAll(selector).forEach(btn => {
    if (btn._genBound) return;
    btn._genBound = true;

    btn.addEventListener('click', async (ev) => {
      ev.stopPropagation(); ev.preventDefault();

      // resolve ids from multiple possible attribute names (defensive)
      const jobPk = btn.dataset.jobPk || btn.dataset.jobId || btn.getAttribute('data-job-id') || null;
      const interviewPk = btn.dataset.interviewId || btn.dataset.interviewId || btn.getAttribute('data-interview-id') || null;
      const n = Number(btn.dataset.questionsCount || btn.dataset.nQuestions || btn.getAttribute('data-n-questions') || 25) || 25;
      const autoPublish = (btn.dataset.autoPublish === 'true' || btn.getAttribute('data-auto-publish') === 'true');

      // If button has data-interview-id -> just generate for interview
      if (interviewPk) {
        await handleGenerateFromButton({ btn, interviewPk, questionsCount: n, autoPublish });
        return;
      }

      // If button has jobPk -> create interview then generate
      if (jobPk) {
        // confirmation optional for some buttons
        if (btn.classList.contains('confirm-create') && !confirm('Create interview for this job and generate questions?')) return;
        await handleGenerateFromButton({ btn, jobPk, questionsCount: n, autoPublish });
        return;
      }

      // No id found
      const statusEl = _ensureStatusSpan(btn);
      statusEl.innerText = 'Missing job/interview id';
      if (typeof showToast === 'function') showToast('Missing job/interview id', 'error');
    });
  });
}

// expose so other scripts can call after dynamic content load
window.attachGenerateInterviewButtons = attachGenerateInterviewButtons;
window.handleGenerateFromButton = handleGenerateFromButton;
