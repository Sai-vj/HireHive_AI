...
// Replace existing startInterview with this
async function startInterview(interviewId, inviteId) {
  ...
  const candidatePage = FRONTEND_INTERVIEW_PAGE(interviewId, inviteId);

  try {
    showSpinner(true, 'Starting interview...');

    // Use apiFetch helper (it's used elsewhere in this file)
    const payload = { invite: inviteId || null }; // backend expects `invite`
const res = await apiFetch(apiStart, {
  method: 'POST',
  ...
  body: JSON.stringify(payload),
    ...
    // apiFetch in your code returns an object like { ok, status, data, ... }
    // after calling apiFetch/apiFetchAsJson and getting `res`
if (!res.ok) {
  const body = res.data;
  const msg = body?.detail || body?.error || (typeof body === 'string' ? body : `Status ${res.status}`);
  const scheduled = body?.scheduled_start ? `Starts at: ${new Date(body.scheduled_start).toLocaleString()}` : '';
  // if you have showToast use it, else update a message div
  if (typeof showToast === 'function') {
    showToast(`Cannot start: ${msg}${scheduled ? ' — ' + scheduled : ''}`, 'error', 6000);
  } else {
    const m = document.getElementById('message') || document.body;
    (m).innerHTML = `<div class="alert alert-danger">Cannot start interview: ${escapeHtml(msg)}${scheduled ? '<br>'+escapeHtml(scheduled) : ''}</div>`;
  }
  return { ok: false, res };
}


    // If API returned a direct URL to join, open that
    const joinUrl = res.data?.redirect_url || res.data?.join_url || res.data?.url || null;
    if (joinUrl) {
      window.location.href = joinUrl; // same tab; use window.open(...) if you want new tab
      return { ok: true, url: joinUrl, method: 'api_join_url' };
    }

    // Otherwise open your candidate frontend page (existing view)
    window.location.href = candidatePage;
    return { ok: true, url: candidatePage, method: 'candidate_page' };
