...
async function fetchAttemptCount(jobId) {
  ...
  // Update job card after quiz submit
function handleQuizSubmitResponse(jobId, data) {
  ...
  // Add / update attempt info div
  let attemptInfo = jobCard.querySelector('.attempt-info');
  if (!attemptInfo) {
    attemptInfo = document.createElement('div');
    attemptInfo.className = 'attempt-info';
    attemptInfo.style = "margin-top:6px;font-size:0.9rem;color:#444;";
    jobCard.appendChild(attemptInfo);
  }

  attemptInfo.innerHTML = `
    Attempt ${data.attempt_id}: ${data.correct}/${data.total} — 
    <strong style="color:${data.passed ? 'green' : 'crimson'}">
      ${data.passed ? 'Passed' : 'Failed'}
    </strong>
    (${new Date().toLocaleString()})
  `;

  // Disable Take Quiz if max attempts reached
  if (data.attempts_remaining !== undefined && data.attempts_remaining <= 0) {
    const takeQuizBtn = jobCard.querySelector('.take-quiz-btn');
    if (takeQuizBtn) {
      takeQuizBtn.disabled = true;
      takeQuizBtn.innerText = "No more attempts";
    }
  }
}
async function onQuizSubmit(jobId, answers) {
