...
async function loadInvites() {
  ...
  if (!res.ok) {
    listEl.innerHTML = `<div class="text-danger">Failed to load invites (${res.status})</div>`;
    return;
  }
  ...
  invites.forEach(inv => {
    ...
    div.innerHTML = `
      <div><strong>${interview.title || 'Interview'}</strong></div>
      <div class="small-muted">Scheduled at: ${interview.scheduled_at ? new Date(interview.scheduled_at).toLocaleString() : '—'}</div>
      <div class="small-muted">Status: <span class="badge bg-${status==='accepted'?'success':status==='declined'?'danger':'secondary'}">${status}</span></div>
      <div class="mt-2">
        ${status==='pending' ? `
          <button class="btn btn-sm btn-success accept-invite-btn" data-id="${inv.id}">Accept</button>
          <button class="btn btn-sm btn-outline-danger decline-invite-btn" data-id="${inv.id}">Decline</button>
        ` : ''}
      </div>
      <div class="invite-details mt-2" style="display:${status==='accepted'?'block':'none'};">
        <strong>Details:</strong>
        <div>${interview.description || 'No description'}</div>
        <div>Mode: ${interview.mode || '—'}</div>
        <div>Duration: ${interview.duration_minutes || ''} mins</div>
      </div>
    `;
...
listEl.querySelectorAll('.start-interview-btn').forEach(btn => {
  btn.addEventListener('click', (e) => {
    ...
    window.location.href = `/interviews/candidate/${iid}/`;
  });
});
...
async function respondInvite(inviteId, answer) {
  const res = await apiFetch(`/api/interviews/candidate/invites/${inviteId}/respond/`, {
    method: 'POST',
  ...
  if (res.ok) {
    ...
    showToast('Failed: ' + (res.data?.detail || res.status), 'error');
