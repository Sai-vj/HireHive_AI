...
function safeShowBootstrapModal(modalEl, options = {}) {
  ...
  // Helper: plain DOM fallback (safe, works even if bootstrap breaks)
  function plainShow() {
    try {
  ...
  // If developer forced fallback, do that
  if (window.RDASH_FORCE_NO_BOOTSTRAP) return plainShow();

  // Try bootstrap safely
  try {
    if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
      ...
      try {
        ...
        console.warn('bootstrap instance creation error, falling back:', createErr);
        return plainShow();
      }

      // call show() guarded — try requestAnimationFrame + .call, fallback to plainShow if it fails
      try {
        requestAnimationFrame(() => {
          try {
            // try correct binding first
            if (typeof inst.show === 'function') {
              try { inst.show.call(inst); return; }
              catch (callErr) { console.warn('inst.show.call failed', callErr); }
            }
            // last attempt: direct show (old versions)
            try { inst.show(); return; } catch (directErr) { console.warn('inst.show direct failed', directErr); }
            // if everything failed, plain fallback
            plainShow();
          } catch (frameErr) {
            console.warn('requestAnimationFrame show error', frameErr);
            plainShow();
          }
        });
        return inst;
      } catch (showErr) {
        console.warn('bootstrap show wrapper failed', showErr);
        return plainShow();
      }
    }
  } catch (outerErr) {
    console.warn('bootstrap try/catch outer failed', outerErr);
    return plainShow();
  }
...
if (sendBtn && !sendBtn._bound) {
  ...
  sendBtn.addEventListener('click', async (ev) => {
    ...
    // Use hidden candidate id and readonly display
    ...
    if (scheduledRaw) {
      try {
        // datetime-local is treated as local; create date from parts to avoid browser parsing variance
