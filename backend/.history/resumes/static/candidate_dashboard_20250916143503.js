...
function openQuizModal(jobId, questionPayload) {
  ...
  // ---------- Timer helpers (single, shared definitions) ----------
  if (!window.__quizTimer) {
    window.__quizTimer = {
      handle: null,
      remaining: 0,
      start(modalEl, seconds, onExpire) {
        this.stop();
        this.remaining = Math.max(0, Math.floor(Number(seconds) || 0));
        if (!modalEl) return;
        let header = modalEl.querySelector('h4#quiz-modal-title') || modalEl.querySelector('h4');
        if (!header) return;
        ...
        const fmt = s => { const m = Math.floor(s/60); const se = s%60; return `${m}:${String(se).padStart(2,'0')}`; };
        const tick = () => {
          pill.textContent = `⏳ ${fmt(this.remaining)}`;
          if (this.remaining <= 30) pill.classList.add('warning'); else pill.classList.remove('warning');
          if (this.remaining <= 0) {
            this.stop();
      ...
      set(sec) { this.remaining = Number(sec) || 0; }
    };
  }
  // ---------- end timer helpers ----------
  ...
  fetcher(`/api/quiz/${jobId}/`, { method: 'GET' })
    .then(async (res) => {
      ...
      /* ---------- Quiz: timer + max attempts ---------- */
      const QUIZ_MAX_ATTEMPTS = 3;

      // fetch attempt count (tries a few shapes)
      async function fetchAttemptCount(jobIdLocal) {
        const tries = [
          `/api/quiz/attempts/?job_id=${jobIdLocal}`,
          `/api/quiz/attempts/?job=${jobIdLocal}`,
          `/api/quiz/attempts/`
        ];
        for (const u of tries) {
          try {
            const r = await (typeof fetchWithAuth === 'function' ? fetchWithAuth(u, { method: 'GET' }) : fetch(u, { method: 'GET' }));
            if (!r) continue;
            let data = null;
      ...
      async function autoSubmitQuiz() {
        try {
          ...
          if (r.ok) {
            ...
            if (typeof handleQuizSubmitResponse === 'function') handleQuizSubmitResponse(jobIdNow, d);
          } else {
      ...
      // start timer if provided by backend; fallback default 120s
      const timeLimit = (body?.time_limit_seconds || body?.time_limit) ? Number(body.time_limit_seconds || body.time_limit) : 120;
      try {
        window.__quizTimer.start(modal, timeLimit, async () => {
          await autoSubmitQuiz();
        });
      } catch (e) { console.warn('timer start failed', e); }

      // stop timer on modal close button
      const closeBtn = modal.querySelector('#quiz-modal-close');
      if (closeBtn) {
        closeBtn.onclick = function(){
          window.__quizTimer.stop();
