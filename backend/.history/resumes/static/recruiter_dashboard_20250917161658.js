...
async function loadApplicationsForSelectedJob() {
  ...
  `/api/recruiter/job/${encodeURIComponent(selectedJob.id)}/applications/`  // <- new recruiter-specific endpoint
];
  let res = null;
  for (const u of urls) {
  ...
  let apps = Array.isArray(res.data) ? res.data : (res.data.results || res.data.applications || res.data.data || []);
