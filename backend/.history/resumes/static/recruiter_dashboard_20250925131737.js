...
// interviews_generate_ui.js or inside recruiter_dashboard.js

const genBtn = document.getElementById('generateBtn');
const genStatus = document.getElementById('genStatus');

async function generateQuestions_UI({ jobPk, questionsCount = 25, autoPublish = true } = {}) {
  if (!jobPk) {
    genStatus.innerText = 'Missing job id';
    return { ok: false };
  }

  // disable + show spinner
  genBtn.disabled = true;
  genStatus.innerHTML = '<span class="small-spinner"></span> Generating questions...';

  const startTs = Date.now();
  try {
    const createRes = await createInterviewForJob(jobPk);
    if (!createRes?.ok) {
      genStatus.innerText = 'Interview creation failed';
    ...
    if (!interviewId) {
      genStatus.innerText = 'No interview id returned';
      return { ok: false };
    }

    genStatus.innerHTML = '<span class="small-spinner"></span> Interview created — generating questions...';

    const genRes = await generateInterviewQuestions(interviewId, questionsCount, autoPublish);

    if (genRes?.ok) {
      const took = Math.round((Date.now() - startTs) / 1000);
      genStatus.innerText = `Questions generated (${questionsCount}) in ${took}s`;
      return { ok: true, genRes };
    } else {
      genStatus.innerText = 'Generation failed';
      return { ok: false, genRes };
    }
  } catch (err) {
    console.error(err);
    genStatus.innerText = 'Unexpected error';
    return { ok: false, error: err };
  } finally {
    setTimeout(() => { genBtn.disabled = false; }, 800);
  }
}
...
if (genBtn) {
  genBtn.addEventListener('click', async () => {
    const jobPk = genBtn.dataset.jobPk;
    await generateQuestions_UI({ jobPk, questionsCount: 25, autoPublish: true });
  });
}
...
function attachGenerateInterviewButtons() {
  ...
  document.querySelectorAll('[data-job-pk]').forEach(btn => {
  btn.addEventListener('click', async () => {
    const jobPk = btn.dataset.jobPk;
    const statusEl = btn.nextElementSibling; // assume span after button
    statusEl.innerHTML = 'Generating...';

    const result = await generateQuestions_UI({
      jobPk,
      questionsCount: 25,
      autoPublish: true
    });
    ...
    if (result.ok) {
      statusEl.innerText = 'Questions generated ✅';
    } else {
...
window.generateQuestions_UI = generateQuestions_UI;
