...
function ensureInviteBindings() {
  ...
  // SANITIZE candidateId: only accept pure digits
  if (!/^\d+$/.test(candidateIdRaw)) {
    candidateIdRaw = ''; // clear anything like "User ID: 24" or "Resume ID: 24"
    // also clear the hidden input to avoid accidental sends
  ...
  try {
    ...
    if (hasCandidate) bodyObj.candidate_id = candidateId;
    else if (hasResume) bodyObj.resume_id = resumeId;
