...
// static/recruiter_dashboard.js
// Minimal recruiter logic: create interview + invite candidate modal.
// Uses localStorage['token'] as Bearer token.
...
const JOB_APPLICATIONS_URLS = (jobId) => [
  `/api/resumes/recruiter/job/${jobId}/applications/`,
  `/api/resumes/jobs/${jobId}/applications/`,
  `/api/applications/?job_id=${encodeURIComponent(jobId)}`,
  `/api/resumes/applications/?job_id=${encodeURIComponent(jobId)}`
];
...
const inviteCandidateNameInput = document.getElementById('invite_candidate_name');
const inviteJobIdInput = document.getElementById('invite_job_id');
const inviteInterviewIdInput = document.getElementById('invite_interview_id');
...
const inviteCreateThenSendBtn = document.getElementById('inviteCreateThenSendBtn');
...
const ci_job_id = document.getElementById('ci_job_id');
const ci_title = document.getElementById('ci_title');
const ci_description = document.getElementById('ci_description');
const ci_scheduled_at = document.getElementById('ci_scheduled_at');
const ci_duration = document.getElementById('ci_duration');
const ci_mode = document.getElementById('ci_mode');
const ci_create_btn = document.getElementById('ci_create');
...
function hideInviteModal(){
  ...
  inviteContext = { jobId:null, interviewId:null, candidateId:null, candidateName:null };
}
...
async function sendInvite({ interviewId=null, jobId=null, candidateId, scheduled_at=null, message='' }){
  // the endpoint implemented server-side expects job-based URL that auto-creates interview:
  // POST /api/interviews/recruiter/job/<jobId>/invite/
...
// direct Send (use existing interview or job endpoint)
if (inviteSendBtn) inviteSendBtn.addEventListener('click', async (e) => {
  ...
  inviteSendBtn.disabled = true;
  try {
    const res = await sendInvite({ jobId, candidateId, scheduled_at: scheduled, message });
    if (res.ok) {
      alert('Invite sent');
      hideInviteModal();
      // optionally refresh applications list on page
      if (typeof loadApplicationsForSelectedJob === 'function' && jobId) loadApplicationsForSelectedJob(jobId).catch(()=>{});
    } else {
      const detail = res.data?.detail || JSON.stringify(res.data) || `status ${res.status}`;
      alert('Invite failed: ' + detail);
    }
  } catch(err) {
    console.error('sendInvite error', err);
    alert('Error sending invite');
  } finally { inviteSendBtn.disabled = false; }
});

// recruiter_dashboard.js
// Single-file front-end for recruiter dashboard: load jobs, show applications,
// create interview, and invite candidate (auto-create interview if missing).
// Requires HTML elements: #jobsList, #selectedJobTitle, #applicationsList, invite modal inputs/buttons.

const API_BASE = '/api'; // base prefix for your API routes
const JOBS_URL = '/api/resumes/jobs/';         // adjust if you use a different route
const CREATE_INTERVIEW_URL = (jobId) => `${API_BASE}/interviews/recruiter/job/${jobId}/create/`;
const JOB_INVITE_URL = (jobId) => `${API_BASE}/interviews/recruiter/job/${jobId}/invite/`;
const JOB_APPLICATIONS_URL = (jobId) => `/api/resumes/recruiter/job/${jobId}/applications/`; // your template used this earlier
...
async function fetchJson(url, opts = {}) {
  const headers = Object.assign({ 'Content-Type': 'application/json' }, getTokenHeader(), opts.headers || {});
  const res = await fetch(url, Object.assign({}, opts, { headers }));
  const text = await res.text();
  let json = null;
  try { json = text ? JSON.parse(text) : null; } catch(e){ json = text; }
  return { ok: res.ok, status: res.status, data: json, res };
}
...
function renderJobs() {
  ...
  jobsList.innerHTML = state.jobs.map(j => {
    const title = (j.title||'Untitled').slice(0,40);
    return `
      <div class="job-card p-2 mb-2" data-job-id="${j.id}">
        <div class="d-flex justify-content-between align-items-center">
          <div>
            <h6 class="mb-0">${title}</h6>
            <div class="small-muted">${(j.skills_required||'').slice(0,60)}</div>
          </div>
          <div>
            <button class="btn btn-sm btn-outline-primary view-job-btn" data-job-id="${j.id}">View</button>
            <button class="btn btn-sm btn-outline-success generate-quiz-btn" data-job-id="${j.id}">Generate Quiz</button>
          </div>
        </div>
      </div>
    `;
  }).join('');
  // attach view job listeners
  document.querySelectorAll('.view-job-btn').forEach(b => {
    b.addEventListener('click', (e)=>{
      const id = Number(e.currentTarget.dataset.jobId);
      selectJob(id);
    });
  });
}

/* render applications for selected job */
function renderApplications(apps = []) {
  ...
  applicationsList.innerHTML = apps.map(app => {
    ...
    const inviteBadge = app.invite_status ? `<span class="badge bg-secondary">${app.invite_status}</span>` : '';
    return `
      <div class="card p-2 mb-2 application-row" data-application-id="${app.id}" data-candidate-id="${app.candidate_id}">
        <div class="d-flex justify-content-between align-items-center">
          <div>
            <strong>${app.candidate_name || 'Candidate'}</strong>
            <div class="small-muted">Applied: ${app.applied_at || '-'}</div>
          </div>
          <div>
            <button class="btn btn-sm btn-outline-primary view-app-btn" data-candidate-id="${app.candidate_id}">View</button>
            <button class="btn btn-sm btn-success invite-app-btn" data-candidate-id="${app.candidate_id}" data-candidate-name="${app.candidate_name}">Invite</button>
            ${inviteBadge}
          </div>
        </div>
      </div>
  ...
  // attach invite click handlers
  document.querySelectorAll('.invite-app-btn').forEach(btn=>{
...
async function loadJobs() {
  ...
  const r = await fetchJson(JOBS_URL, { method: 'GET' });
  if(!r.ok) {
    jobsList.innerHTML = `<div class="text-danger">Failed to load jobs (${r.status})</div>`;
    console.error('loadJobs error', r.data);
    return;
  }
  state.jobs = Array.isArray(r.data) ? r.data : (r.data.results || []);
  renderJobs();
}
...
async function loadJobApplications(jobId) {
  ...
  const url = JOB_APPLICATIONS_URL(jobId);
  const r = await fetchJson(url, { method: 'GET' });
  if(!r.ok) {
    applicationsList.innerHTML = `<div class="text-danger">Failed to load applications (${r.status})</div>`;
    console.error('loadJobApplications', r);
    return;
  }
  const apps = Array.isArray(r.data) ? r.data : (r.data.results || []);
...
async function selectJob(jobId) {
  ...
  state.selectedJob = state.jobs.find(x=>x.id === jobId) || null;
  selectedJobTitle && (selectedJobTitle.textContent = (state.selectedJob && state.selectedJob.title) || `Job ${jobId}`);
  // show the applications section in UI if needed
...
/* -------------------- Create interview for job -------------------- */
async function createInterviewForJob(jobId, payload) {
  // payload is an object: { title, description, scheduled_at, duration_minutes, mode, is_active }
  const url = CREATE_INTERVIEW_URL(jobId);
  const r = await fetchJson(url, { method: 'POST', body: JSON.stringify(payload) });
  if (r.ok) {
    console.log('created interview', r.data);
    return r.data;
  } else {
    console.error('createInterviewForJob failed', r);
    throw r;
  }
}

/* -------------------- Invite candidate (job-based) -------------------- */
async function inviteCandidateByJob(jobId, candidateId, scheduledAt = null, message = '') {
  const url = JOB_INVITE_URL(jobId);
  const body = { candidate_id: candidateId };
  ...
  if (r.ok || r.status === 201) {
    ...
    console.error('inviteCandidateByJob failed', r);
    throw r;
  }
}
...
function openInviteModalForCandidate(candidateId = '', candidateName = '') {
  if(!inviteModal) { alert('Invite modal not found'); return; }
  // Pre-fill candidate id and message
  inviteCandidateIdInput && (inviteCandidateIdInput.value = candidateId || '');
  inviteScheduledAtInput && (inviteScheduledAtInput.value = '');
  inviteMessageInput && (inviteMessageInput.value = `Hi ${candidateName || ''}, you are invited for interview.`);
  inviteModal.classList.remove('d-none');
  inviteModal.style.display = 'flex';
  document.body.style.overflow = 'hidden';
}
...
if (inviteSendBtn) inviteSendBtn.addEventListener('click', async (e) => {
  ...
  if(!jobId) { alert('No job selected. Open job first.'); return; }
  const candidateId = inviteCandidateIdInput.value.trim();
  ...
  try {
    // Call job-invite endpoint which will auto-create interview if missing
    const invited = await inviteCandidateByJob(jobId, candidateId, scheduled_at, message);
    alert('Invite sent');
    closeInviteModal();
    // Optionally refresh applications list so invite badge appears
    await loadJobApplications(jobId);
  } catch(err) {
    // err is the fetchJson object if thrown above
    const msg = (err && (err.data && (err.data.detail || JSON.stringify(err.data)))) || 'Invite failed';
    alert(msg);
  }
});
...
async function init() {
  ...
  // Show saved token indicator (if you have token input in page)
  const token = localStorage.getItem('token') || '';
  const tokenStatus = document.getElementById('tokenStatus');
  if (tokenStatus) tokenStatus.textContent = token ? 'Token saved' : 'Token not saved.';
