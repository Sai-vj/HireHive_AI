...
async function loadJobs() {
  ...
  jobs.forEach(j => {
    ...
    // click anywhere on left to open job detail (select)
...
/* ---------------- selection helper ---------------- */
function selectJob(jobObjOrId) {
  // Accept either job object or job id
  let jobId = null;
  if (!jobObjOrId) return;
  if (typeof jobObjOrId === 'object') {
    selectedJob = jobObjOrId;
    jobId = jobObjOrId.id;
  } else {
    jobId = jobObjOrId;
  }
  ...
  document.querySelectorAll('#jobsList .job-card').forEach(el => {
    ...
    if (String(el.dataset.jobId) === String(jobId)) el.classList.add('active');
    else el.classList.remove('active');
  });
  // update selectedJob title/meta if we have the object
  if (selectedJob && document.getElementById('selectedJobTitle')) {
    document.getElementById('selectedJobTitle').innerText = selectedJob.title || '';
  }
}
...
async function openJobDetail(jobId) {
  ...
  selectedJob = r.data;
  selectJob(selectedJob); // visually mark
...
/* ---------------- Edit / Delete helpers ---------------- */
/* ---------------- Edit job using the same Add modal (full fields) ---------------- */

/**
 * Open Add Job modal in "edit" mode:
 * - fetches job details from server
 * - pre-fills form fields
 * - sets form.dataset.editing = jobId
 */
async function openEditJob(jobId) {
  if (!jobId) return showToast('No job id', 'error');
  // fetch job
  const r = await apiFetch(`/api/resumes/jobs/${jobId}/`, { method: 'GET' });
  if (!r || !r.ok) {
    return showToast('Failed to load job for edit', 'error');
  }
  const job = r.data || {};
  ...
  const form = document.getElementById('addJobForm');
  if (!modalEl || !form) {
    showToast('Add/Edit modal or form missing in HTML', 'error');
    return;
  }

  // pre-fill fields (make sure these IDs exist in your modal)
  document.getElementById('jobTitle').value = job.title || '';
  document.getElementById('jobCompany').value = job.company || job.employer || '';
  ...
  // mark form as editing so submitAddJob will do PATCH
  form.dataset.editing = String(jobId);

  // update modal title/button text if you want
...
// * Delete job and remove card from DOM after success.
// robust delete helper - tries multiple likely endpoints and handles auth/errors
// add this function anywhere in your recruiter_dashboard.js (after api helpers)
// paste into your recruiter_dashboard.js (after apiFetch helper)
async function confirmAndDeleteJob(jobId) {
  if (!jobId) return showToast('No job id', 'error');
  if (!confirm('Delete this job permanently?')) return;

  // get auth token (if you store)
  const token = localStorage.getItem('token') || localStorage.getItem('recruiter_token_v1') || '';

  // get csrftoken from cookie if present (for session auth)
  ...
  if (csrftoken) headersBase['X-CSRFToken'] = csrftoken;

  // Try to detect detail/delete URL from job object if available in DOM dataset or window
  let possibleUrls = [
    `/api/resumes/jobs/${encodeURIComponent(jobId)}/`,
    `/api/resumes/jobs/${encodeURIComponent(jobId)}`,
    `/api/resumes/jobs/${encodeURIComponent(jobId)}/delete/`,
    `/api/jobs/recruiter/job/${encodeURIComponent(jobId)}/delete/`,
    `/api/jobs/${encodeURIComponent(jobId)}/`,
    `/api/jobs/${encodeURIComponent(jobId)}/delete/`
  ];

  // If job card has dataset.url or data-delete attribute, include
  try {
    const card = document.querySelector(`.job-card[data-job-id="${jobId}"]`);
    if (card) {
      const extra = card.dataset.url || card.dataset.detail || card.dataset.deleteUrl || card.dataset.delete;
      if (extra) possibleUrls.unshift(extra);
    }
  } catch(e){}

  // Also try to find job object from last loaded list (if you store it globally)
  try {
    if (window._lastJobsList && Array.isArray(window._lastJobsList)) {
      const obj = window._lastJobsList.find(x => String(x.id) === String(jobId));
      if (obj) {
        // push common link fields if present
        ['url','detail','delete_url','resource_uri','links','self'].forEach(k=>{
          if (obj[k]) {
            if (typeof obj[k] === 'string') possibleUrls.unshift(obj[k]);
            else if (obj[k] && obj[k].href) possibleUrls.unshift(obj[k].href);
          }
        });
      }
    }
  ...
  // normalize to unique
  possibleUrls = Array.from(new Set(possibleUrls.filter(Boolean)));

  // 1) Try DELETE on each url
  for (const url of possibleUrls) {
    try {
      const r = await fetch(url, { method: 'DELETE', headers: headersBase, credentials: 'include' });
      if (r.ok) {
        showToast('Job deleted', 'success');
        const c = document.querySelector(`.job-card[data-job-id="${jobId}"]`);
        if (c) c.remove();
        if (window.selectedJob && String(window.selectedJob.id) === String(jobId)) {
          window.selectedJob = null;
          const jd = document.getElementById('jobDetails'); if (jd) jd.style.display = 'none';
          const nj = document.getElementById('noJob'); if (nj) nj.style.display = 'block';
        }
        return;
      }
      if (r.status === 401 || r.status === 403) {
        showToast('Unauthorized — paste valid token or login', 'error', 4000);
        return;
      }
      // 405 method not allowed -> try next
      if (r.status === 405) {
        console.warn('DELETE not allowed on', url);
        continue;
      }
      // other error: read text for debug
      const txt = await r.text().catch(()=>null);
      console.warn('Delete attempt failed', url, r.status, txt);
    } catch (e) {
      console.warn('Network delete error', url, e);
    }
  }

  // 2) Fallback: try POST to <url>/delete/ if DELETE not allowed
  for (const base of possibleUrls) {
    let alt = base;
    if (!alt.endsWith('/delete/')) {
      alt = base.endsWith('/') ? (base + 'delete/') : (base + '/delete/');
    }
    try {
      const r = await fetch(alt, { method: 'POST', headers: headersBase, credentials: 'include', body: JSON.stringify({ id: jobId }) });
      if (r.ok) {
        showToast('Job deleted (POST)', 'success');
        const c = document.querySelector(`.job-card[data-job-id="${jobId}"]`);
        if (c) c.remove();
        return;
      }
      if (r.status === 401 || r.status === 403) { showToast('Unauthorized — check token', 'error'); return; }
      const txt = await r.text().catch(()=>null);
      console.warn('POST-delete failed', alt, r.status, txt);
    } catch (e) {
      console.warn('Network POST-delete error', alt, e);
    }
  }

  // 3) If nothing worked, report
  showToast('Delete failed — server does not allow delete on available endpoints', 'error', 6000);
  console.log('Tried urls:', possibleUrls);
}
/* ---------------- Matches ---------------- */
...
function renderResults(rows, jobTitle) {
  ...
  qsa('.reset-attempts').forEach(b=>{ if (b._bound) return; b._bound=true; b.addEventListener('click', async ()=>{ if (!confirm('Reset attempts for this candidate?')) return; const job = b.dataset.job, cid = b.dataset.cid; const r = await apiFetch(`/api/quiz/${job}/reset_attempts/${cid}/`, { method:'POST' }); if (r.ok) { showToast('Reset OK','success'); fetchRecruiterResults(job); } else showToast('Reset failed','error'); }); });
