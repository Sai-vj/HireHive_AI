...
(function () {
  ...
  function safeShowBootstrapModal(modalEl, options = {}) {
    ...
    // ensure we are working with a real Element (not null or jQuery object)
    if (modalEl instanceof NodeList || modalEl instanceof HTMLCollection) modalEl = modalEl[0];
    if (!modalEl || modalEl.nodeType !== 1) {
      console.warn('safeShowBootstrapModal: not a DOM element', modalEl);
      return null;
    }

    // ensure appended to body so bootstrap internals can query within the document
    try {
      if (!document.body.contains(modalEl)) document.body.appendChild(modalEl);
    } catch (e) {
      console.warn('safeShowBootstrapModal: append failed', e);
    }

    // defensive attributes for accessibility
    try {
    ...
    } catch (e) { /* ignore attr errors */ }

    // Try Bootstrap modal APIs in a safe order
    try {
      // prefer getOrCreateInstance if available (Bootstrap 5.2+)
      if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
        ...
        try {
          if (typeof bootstrap.Modal.getOrCreateInstance === 'function') {
            ...
            // older fallback: try getInstance, otherwise construct new
          ...
          // if something inside bootstrap threw, log and continue to fallback
          console.warn('safeShowBootstrapModal: bootstrap instance creation failed', innerErr);
          inst = null;
        }

        if (inst && typeof inst.show === 'function') {
          // show in next tick to avoid "illegal invocation" in some edge cases
          setTimeout(() => {
            try { inst.show(); } catch (e) { console.warn('safeShowBootstrapModal: inst.show failed', e); }
          }, 0);
          return inst;
        }
      }
    } catch (e) {
      console.warn('safeShowBootstrapModal: bootstrap try-catch outer', e);
    }

    // Final plain-DOM fallback (safe, doesn't rely on bootstrap internals)
    try {
      ...
      // create and show backdrop if not present
      let backdrop = document.querySelector('.modal-backdrop.custom-rdash-backdrop');
      if (!backdrop) {
        backdrop = document.createElement('div');
        backdrop.className = 'modal-backdrop fade show custom-rdash-backdrop';
      ...
      console.warn('safeShowBootstrapModal: plain DOM fallback failed', e);
      return null;
    }
  }
...
async function loadApplicationsForSelectedJob(jobIdParam) {
  ...
  for (const a of apps) {
    ...
    if (resumeId) card.dataset.candidateId = String(resumeId);
  ...
  function ensureInviteBindings() {
    ...
    if (sendBtn && !sendBtn._bound) {
      ...
      sendBtn.addEventListener('click', async (ev) => {
        ...
        const modal = mod;
        // job id resolution: first dataset.jobId, then hidden input value, then selectedJob fallback
        ...
        try {
          // try the primary invite endpoint
          const body = JSON.stringify({ candidate_id: candidateId, scheduled_at: scheduled, message });
          const res = await apiFetchSimple(INTERVIEWS_INVITE_BY_JOB(currJobId), { method: 'POST', body });
          if (res && res.ok) {
            ...
            // try to parse server message for helpful error
 ...
 document.body.addEventListener('click', (ev) => {
  ...
  if (!btn) return;
  // prefer dataset, then attributes
  ...
  // call showInviteModal with whatever we have
  showInviteModal({
    jobId: jobId || null,
...
function showInviteModal({ jobId = null, interviewId = null, candidateId = '', candidateName = '', message = null } = {}) {
  const modal = ensureInviteModal();

  // ensure modal elements created and present
  ...
  // set dataset for modal (useful for send handler)
  if (typeof jobId !== 'undefined' && jobId !== null) modal.dataset.jobId = String(jobId);
  else delete modal.dataset.jobId;

  if (typeof interviewId !== 'undefined' && interviewId !== null) modal.dataset.interviewId = String(interviewId);
  else delete modal.dataset.interviewId;

  // fill inputs if present (defensive)
  if (idInput) idInput.value = interviewId || '';
  if (candInput) candInput.value = candidateId || '';
  if (msgInput) {
    // if caller provided message explicitly, use it; else set friendly default
    if (message !== null) msgInput.value = message;
    else msgInput.value = `Hi ${candidateName || ''}, you are invited for interview.`;
  }

  ensureInviteBindings(); // bind send button etc
