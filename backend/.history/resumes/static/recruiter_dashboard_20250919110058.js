...
// static/recruiter_dashboard.js
// Patched single-file recruiter dashboard — unified, cleaned, invite modal fixed.

// CONFIG
const API_BASE = '/api';
const JOBS_URL = `${API_BASE}/resumes/jobs/`;
const JOB_DETAIL_URL = (id) => `${API_BASE}/resumes/jobs/${id}/`;
const JOB_APPLICATIONS_URLS = (id) => [
  `/api/resumes/recruiter/job/${id}/applications/`,
  `/api/resumes/jobs/${id}/applications/`,
  `/api/applications/?job_id=${encodeURIComponent(id)}`,
  `/api/resumes/applications/?job_id=${encodeURIComponent(id)}`
];
...
// TOKEN KEY (use same key you used in Postman / other code)
const TOKEN_STORAGE_KEY = 'token';

// ---- small helpers ----


function savedToken() { return localStorage.getItem(TOKEN_STORAGE_KEY) || ''; }
function setSavedToken(t) { if (t) localStorage.setItem(TOKEN_STORAGE_KEY, t); else localStorage.removeItem(TOKEN_STORAGE_KEY); }
function authHeaders() {
  const t = savedToken();
  return t ? { Authorization: `Bearer ${t}` } : {};
}
function showToast(msg, type = 'info', ms = 3000) {
  ...
  el.style.background = (type === 'error') ? '#f8d7da' : (type === 'success') ? '#d1e7dd' : '#fff8d6';
  el.textContent = msg;
  container.appendChild(el);
  setTimeout(() => el.remove(), ms);
}
...
async function apiFetch(path, opts = {}) {
  ...
  try {
    const r = await fetch(path, opts);
    if (r.status === 401 || r.status === 403) {
      // clear saved token; prompt user
      // don't throw — return structured result
      localStorage.removeItem(TOKEN_STORAGE_KEY);
      showToast('Not authorized — paste a valid token and retry', 'error', 3500);
      return { ok: false, status: r.status, data: null };
    }
    const txt = await r.text().catch(() => null);
    let json = null;
    ...
    return { ok: false, status: 0, error: true, exception: String(err) };
  }
}

// ---- DOM refs that must exist in template ----
const jobsListEl = qs('#jobsList');
const selectedJobTitleEl = qs('#selectedJobTitle');
const applicationsListEl = qs('#applicationsList');
const inviteModalEl = qs('#inviteModal');
const inviteCandidateIdInput = qs('#invite_candidate_id');
const inviteScheduledAtInput = qs('#invite_scheduled_at');
const inviteMessageInput = qs('#invite_message');
const inviteSendBtn = qs('#inviteSendBtn');
const inviteCancelBtn = qs('#inviteCancelBtn');
const tokenInputEl = qs('#tokenInput');
const saveTokenBtn = qs('#saveTokenBtn');
const tokenStatusEl = qs('#tokenStatus');

// ---- state ----
const state = {
  jobs: [],
  selectedJob: null,
  __inviteContext: null  // { jobId, interviewId, candidateId, candidateName }
};
...
async function loadJobs() {
  ...
  const r = await apiFetch(JOBS_URL);
  if (!r.ok) { jobsListEl.innerHTML = `<div class="text-danger">Failed to load jobs (${r.status})</div>`; console.error('loadJobs', r); return; }
  state.jobs = Array.isArray(r.data) ? r.data : (r.data && r.data.results) || [];
  renderJobs();
}

function renderJobs() {
  if (!jobsListEl) return;
  if (!state.jobs.length) { jobsListEl.innerHTML = '<div class="small-muted">No jobs yet</div>'; return; }
  jobsListEl.innerHTML = '';
  state.jobs.forEach(j => {
    const card = document.createElement('div');
    card.className = 'job-card p-2 mb-2';
    card.dataset.jobId = j.id;
    card.innerHTML = `
      <div class="d-flex justify-content-between align-items-center">
        <div style="min-width:0">
          <h6 class="mb-0">${escapeHtml(j.title||'Untitled').slice(0,60)}</h6>
          <div class="small-muted">${escapeHtml(j.company||'')} ${escapeHtml(j.skills_required||j.skills||'')}</div>
        </div>
        <div>
          <button class="btn btn-sm btn-outline-primary view-job-btn" data-job-id="${j.id}">View</button>
          <button class="btn btn-sm btn-success invite-job-btn d-none" data-job-id="${j.id}">Invite</button>
        </div>
      </div>
    `;
...
async function selectJob(jobId) {
  // try to find in state.jobs first
  let job = state.jobs.find(x => x.id === jobId);
  if (!job) {
    const r = await apiFetch(JOB_DETAIL_URL(jobId));
    if (!r.ok) { showToast('Failed to load job detail', 'error'); return; }
    job = r.data;
  }
  state.selectedJob = job;
  if (selectedJobTitleEl) selectedJobTitleEl.textContent = job.title || `Job ${jobId}`;
  // show applications section
  const appsSection = qs('#applicationsSection'); if (appsSection) appsSection.style.display = 'block';
  await loadApplications(jobId);
}

async function loadApplications(jobId) {
  if (!applicationsListEl) return;
  applicationsListEl.innerHTML = '<div class="small-muted">Loading applications…</div>';
  // try multiple URLs (the backend varies)
  let res = null;
  const urls = JOB_APPLICATIONS_URLS(jobId);
  for (const u of urls) {
    try {
      res = await apiFetch(u);
      if (res && res.ok) break;
    } catch (e) { console.warn('applications fetch failed', u, e); }
  }
  if (!res || !res.ok) {
    applicationsListEl.innerHTML = `<div class="text-danger">No applications (${res ? res.status : 'network'})</div>`;
    return;
  }
  // normalize list
  let apps = [];
  ...
  apps.forEach(a => {
    const name = a.candidate_name || a.user || a.username || 'Candidate';
    const applied = a.applied_at || a.created_at || a.created || '';
    const inviteStatus = a.invite_status || '-';
    const appRow = document.createElement('div');
    appRow.className = 'card p-2 mb-2 application-row';
    appRow.dataset.applicationId = a.id || '';
    appRow.dataset.candidateId = a.candidate_id || a.user_id || '';
    appRow.dataset.candidateName = name;
    appRow.innerHTML = `
      <div class="d-flex justify-content-between align-items-center">
        <div style="min-width:0">
        ...
        <div style="min-width:200px;text-align:right;">
          <button class="btn btn-sm btn-outline-primary view-app-btn">View</button>
          <button class="btn btn-sm btn-success invite-app-btn" type="button">Invite</button>
          <span class="invite-badge small-muted" style="margin-left:8px">${escapeHtml(inviteStatus)}</span>
        </div>
      </div>
    `;
    applicationsListEl.appendChild(appRow);
  });

  // attach invite handler (delegated is also okay)
  qsa('.invite-app-btn').forEach(b => {
    if (b._bound) return; b._bound = true;
    b.addEventListener('click', (ev) => {
      const row = ev.currentTarget.closest('.application-row');
      const candidateId = row && (row.dataset.candidateId || row.getAttribute('data-candidate-id'));
...
function openInviteModalForCandidate(candidateId = '', candidateName = '') {
  if (!inviteModalEl) { alert('Invite modal not present in template'); return; }
  state.__inviteContext = { jobId: state.selectedJob ? state.selectedJob.id : null, candidateId, candidateName, interviewId: null };
  ...
  inviteModalEl.style.display = 'flex';
  document.body.style.overflow = 'hidden';
}
function closeInviteModal() {
  if (!inviteModalEl) return;
  inviteModalEl.classList.add('d-none');
  inviteModalEl.style.display = 'none';
  document.body.style.overflow = '';
  state.__inviteContext = null;
}
if (inviteCancelBtn) inviteCancelBtn.addEventListener('click', (e) => { e.preventDefault(); closeInviteModal(); });

// send invite (job-based endpoint)
async function sendInviteFromModal() {
  if (!state.__inviteContext) return alert('No invite context');
  const jobId = state.__inviteContext.jobId;
  const candidateId = (inviteCandidateIdInput && inviteCandidateIdInput.value.trim()) || state.__inviteContext.candidateId;
  const scheduled_at = inviteScheduledAtInput && inviteScheduledAtInput.value;
  const message = inviteMessageInput && inviteMessageInput.value;

  if (!candidateId) { alert('Enter candidate id'); return; }
  if (!jobId) {
    const ok = confirm('No job selected — create an interview first? Open interviews page?');
  ...
  try {
    ...
    const res = await apiFetch(JOB_INVITE_URL(jobId), { method: 'POST', body: JSON.stringify({ candidate_id: candidateId, scheduled_at, message }) });
    if (res.ok) {
      showToast('Invite created', 'success');
      closeInviteModal();
      // refresh applications so invite status updates
      await loadApplications(jobId);
    } else {
      // show server message if available
      let msg = `Invite failed (${res.status})`;
      if (res.data && res.data.detail) msg = res.data.detail;
      else if (res.data && typeof res.data === 'object') msg = JSON.stringify(res.data);
      alert(msg);
...
if (inviteSendBtn) inviteSendBtn.addEventListener('click', (e) => { e.preventDefault(); sendInviteFromModal(); });

// allow closing modal by click on backdrop / Escape
if (inviteModalEl) {
  inviteModalEl.addEventListener('click', (e) => { if (e.target === inviteModalEl) closeInviteModal(); });
  window.addEventListener('keydown', (e) => { if (e.key === 'Escape') closeInviteModal(); });
}

// ---- simple create interview helper (used by server view too) ----
async function createInterviewForJob(jobId, payload = { title: `Interview`, description: '', scheduled_at: null, duration_minutes: 45, mode: 'online', is_active: true }) {
...
// ---- token UI wiring ----
if (tokenInputEl && savedToken()) tokenInputEl.value = savedToken();
if (saveTokenBtn) saveTokenBtn.addEventListener('click', () => {
  const v = (tokenInputEl && tokenInputEl.value || '').trim();
  if (!v) { showToast('Paste token first', 'error'); return; }
  setSavedToken(v);
  if (tokenStatusEl) tokenStatusEl.textContent = 'Token saved';
...
async function init() {
  ...
  if (tokenStatusEl) tokenStatusEl.textContent = savedToken() ? 'Token saved' : 'Token not saved.';
  await loadJobs();
  // auto select first job if present
  if (state.jobs && state.jobs.length) selectJob(state.jobs[0].id);
}
...
window.createInterviewForJob = createInterviewForJob;
window.inviteCandidateByJob = async (jobId, candidateId, scheduled, message) => {
  const r = await apiFetch(JOB_INVITE_URL(jobId), { method: 'POST', body: JSON.stringify({ candidate_id: candidateId, scheduled_at: scheduled, message }) });
  return r;
};
