...
async function loadInvites() {
  const token = localStorage.getItem('token') || '';
  const res = await fetch('/api/candidate/invites/', { headers: token ? {'Authorization': `Bearer ${token}`} : {} });
  const data = await res.json();
  // render into #invitesList element
}
// respond
async function respondInvite(inviteId, action) {
  const token = localStorage.getItem('token') || '';
  const res = await fetch(`/api/candidate/invites/${inviteId}/respond/`, {
    method:'POST',
    headers: {'Content-Type':'application/json', ...(token && {'Authorization': `Bearer ${token}`})},
    body: JSON.stringify({ action }) // or {accepted:true} depending on your view
  });
  const jd = await res.json();
  if (res.ok) { loadInvites(); } else alert('Failed: ' + (jd.detail||JSON.stringify(jd)));
}
