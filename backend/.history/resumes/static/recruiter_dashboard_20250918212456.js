...
(function(){
  const API_BASE = '/api/interviews'; // base
  // token stored by your dashboard save-token flow
  function getToken(){
  ...
  const inviteScheduledAt = document.getElementById('invite_scheduled_at');
  ...
  let currentJobId = null;     // job pk we will POST to
  ...
  // attach cancel
  if(inviteCancelBtn){
    inviteCancelBtn.addEventListener('click', (e) => {
      e.preventDefault();
      hideModal();
    });
  }

  // Click handler for invite buttons (delegation)
  document.addEventListener('click', function(e){
    ...
    // update modal fields
    inviteModalTitle.innerText = `Invite: ${candidateName || candidateId}`;
    inviteCandidateIdInput.value = candidateId;
    if(inviteCandidateNameInput) inviteCandidateNameInput.value = candidateName;
    inviteScheduledAt.value = '';
    inviteMessage.value = `You are invited for an interview for job ${jobId}`;
  ...
  // Send invite
  if(inviteSendBtn){
    inviteSendBtn.addEventListener('click', async function(e){
      e.preventDefault();
      if(!currentJobId){
      ...
      const scheduled_at = inviteScheduledAt.value || null;
      const message = inviteMessage.value.trim() || 'You are invited for an interview';
      ...
      const url = `${API_BASE}/recruiter/${currentJobId}/invite/`; // matches your endpoint
      const headers = {'Content-Type': 'application/json'};
      if(token) headers['Authorization'] = `Bearer ${token}`;

      inviteSendBtn.disabled = true;
      inviteSendBtn.innerText = 'Sending...';
      ...
      try {
        ...
        if(res.ok){
          hideModal();
          toast('Invite sent');
          // update invite-badge in application row if present
          const row = document.querySelector(`.application-row[data-candidate-id="${candidate_id}"][data-job-id="${currentJobId}"]`);
          if(row){
        ...
        inviteSendBtn.disabled = false;
        inviteSendBtn.innerText = 'Send Invite';
      }
    });
  }
  ...
  function toast(msg, ms=2500){
    ...
    t.id = id;
    t.className = 'toast shadow';
    t.style.cssText = 'position:fixed;right:20px;bottom:20px;padding:10px 14px;background:#111;color:#fff;border-radius:6px;z-index:9999;opacity:0.95';
    t.innerText = msg;
    document.body.appendChild(t);
    setTimeout(()=> document.body.removeChild(t), ms);
  }

  // optional: quick keyboard ESC closes modal
  document.addEventListener('keydown', (e)=>{
    if(e.key === 'Escape') hideModal();
  });

  console.log('recruiter invite frontend initialised');
})();
