...
document.addEventListener('submit', async function (e) {
  ...
  try {
    ...
    // 2) If JSON failed with 400/415 try FormData fallback
    if (!res.ok && (res.status === 400 || res.status === 415 || res.status === 422)) {
      ...
      // use fetchWithAuth (bypasses apiFetch wrapper) so FormData not stringified
   ...
   // ---- improved response handling ----
    if (res.ok) {
      ...
      // disable apply button (prevent duplicate)
      if (submitBtn) {
        submitBtn.disabled = true;
      }
      ...
      // Already applied — try to show friendly info if server returned it
      const d = res.data || {};
      let msg = 'Already applied';
      if (d && typeof d === 'object') {
        // backend might return { application: {...} } or { detail: '...' }
        const app = d.application || d.application_data || d;
        if (app && (app.applied_at || app.created_at || app.status)) {
          const appliedAt = app.applied_at || app.created_at || '';
          const st = app.status || '';
          msg = `Already applied${appliedAt ? ' on ' + appliedAt : ''}${st ? ' — status: ' + st : ''}`;
        } else if (d.detail) {
          msg = d.detail;
        }
      ...
      // optionally refresh to show the existing application
      try { loadMyApplications(); } catch(e) {}
      try { if (selectedJob && Number(selectedJob.id) === Number(jobId)) loadApplicationsForJob(jobId); } catch(e) {}
