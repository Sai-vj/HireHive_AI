...
import { saveTokens } from '.accoutns/static/utils.js';
import { fetchWithAuth } from '../../static/utils';
// after login success
...
async function downloadShortlistCsv(jobId) {
  ...
  try {
    ...
    const resp = await fetchWithAuth(url, { headers: { 'Authorization': 'Bearer ' + token, 'Accept': 'text/csv' }});
    if (!resp.ok) {
