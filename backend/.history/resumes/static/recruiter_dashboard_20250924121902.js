...
function attachJobCardEvents() {
  ...
  qsa('.job-card').forEach(card => {
   ...
   // wire interview/job generate buttons (defensive)
  try {
...
// interviews-invite.js — updated invite handling (paste into your JS bundle)

///////////////////////////////////////////////////////////////////////////////
// Helper: ensure modal element exists (your site may already have this)
// If you already have ensureInviteModal, this will not overwrite it.
window.ensureInviteModal = window.ensureInviteModal || function() {
  const el = document.getElementById('inviteModal');
  if (el) return el;
  // Minimal fallback modal element (in case HTML missing) to avoid runtime error.
...
///////////////////////////////////////////////////////////////////////////////
// Bindings: cancel/close/send (call ensureInviteBindings() once on app init)
function ensureInviteBindings() {
  ...
  if (sendBtn && !sendBtn._bound) {
    ...
    sendBtn.addEventListener('click', async (ev) => {
      ...
      // read resume id from dataset / hidden input
      ...
      const hasCandidate = candidateIdRaw && !Number.isNaN(candidateId) && Number.isFinite(candidateId);
      ...
      try {
        // build body: prefer user id when valid, otherwise send resume_id
...
///////////////////////////////////////////////////////////////////////////////
// Show modal (fills dataset + hidden inputs)
function showInviteModal({ jobId=null, interviewId=null, candidateId='', candidateName='', resumeId='' } = {}) {
  ...
  // dataset for later usage by send handler
  ...
  if (resumeId) modal.dataset.resumeId = String(resumeId);
  else if (modal.dataset.resumeId) delete modal.dataset.resumeId;

  if (jobId) modal.dataset.jobId = String(jobId);
  else if (modal.dataset.jobId) delete modal.dataset.jobId;
...
///////////////////////////////////////////////////////////////////////////////
// Insert invite buttons into application rows (adds data-resume-id when present)
function injectInviteButtonsIntoAppRows(selector = '.application-row') {
  ...
  rows.forEach(row => {
    ...
    // attach resume id from row if available
    const resumeIdFromRow = row.dataset.resumeId || row.getAttribute('data-resume-id') || (row.querySelector && (row.querySelector('[data-resume-id]')?.dataset?.resumeId || row.querySelector('[data-resume-id]')?.getAttribute('data-resume-id')));
    ...
    btn.addEventListener('click', (e) => {
      ...
      const resumeLocal = e.currentTarget.dataset.resumeId || '';
      showInviteModal({
        jobId: jd || null,
        interviewId: e.currentTarget.dataset.interviewId || '',
        candidateId: e.currentTarget.dataset.candidateId || '',
        candidateName: e.currentTarget.dataset.candidateName || '',
...
///////////////////////////////////////////////////////////////////////////////
// small util: tries to resolve candidate id from various DOM attributes
// Keep this function if your app uses it; otherwise adapt as needed.
function resolveCandidateIdFromElement(el) {
  if (!el) return null;
  const cand = el.dataset?.candidateId || el.getAttribute && (el.getAttribute('data-candidate-id') || el.getAttribute('data-user-id') || el.getAttribute('data-uid'));
  if (cand) {
...
///////////////////////////////////////////////////////////////////////////////
// Init: call injector & bindings when DOM ready
document.addEventListener('DOMContentLoaded', () => {
  try {
    injectInviteButtonsIntoAppRows();
    ensureInviteBindings();
  } catch (e) {
    console.warn('invite-init failed', e);
  }
});

// Export for debugging / manual calls
window.injectInviteButtonsIntoAppRows = injectInviteButtonsIntoAppRows;
window.showInviteModal = showInviteModal;
window.ensureInviteBindings = ensureInviteBindings;
