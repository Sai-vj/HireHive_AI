...
async function loadJobs(){
 ...
 function openJobDetailModal(job) {
  ...
  // --- robust createQuizModalIfMissing + renderQuizQuestions ---
function createQuizModalIfMissing() {
  ...
  // small style fixes to ensure radios/labels visible
...
function renderQuizQuestions(questions) {
  ...
  questions.slice(0, 100).forEach((q, idx) => {
    ...
    // render choices or fallback input
    if (choices && Array.isArray(choices) && choices.length) {
      const list = document.createElement('div');
      choices.forEach((c, i) => {
        const id = `q-${qid}-opt-${i}`;
        const lbl = document.createElement('label');
        lbl.htmlFor = id;
        // use radio by default; if q.multi true then checkbox
        const multi = !!(q.multi || q.multiple || q.type === 'multiple_choice');
      ...
      // no choices: show an open answer textarea (user can type)
      const txt = document.createElement('textarea');
      txt.className = 'open-answer';
      txt.name = `q-${qid}-text`;
      txt.placeholder = 'Type your answer here (open-ended question)';
