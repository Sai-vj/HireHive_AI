...
(async function(){
  // adjust if you store token differently; if using session-auth remove Authorization header
  const token = localStorage.getItem('token') || '';

  const apiBase = '/api/interviews'; // adjust if prefix differs
  const invitesUrl = `${apiBase}/candidate/invites/`;
  ...
  function renderInvites(invites) {
    if(!invites || invites.length === 0) {
      container.innerHTML = '<div class="small-muted">No invites</div>';
      return;
    }
    container.innerHTML = invites.map(inv => `
      <div id="invite-${inv.id}" class="card mb-2 p-2">
        <div class="d-flex justify-content-between">
          <div>
            <strong>${inv.interview?.title || 'Interview'}</strong>
            <div class="small-muted">When: ${fmtDate(inv.scheduled_at)}</div>
            <div class="small-muted">Message: ${inv.message || ''}</div>
          </div>
          <div class="text-end">
            <div class="mb-1"><span class="badge bg-${inv.status === 'pending' ? 'warning' : inv.status === 'accepted' ? 'success' : 'secondary'}">${inv.status}</span></div>
            <div>
              <button class="btn btn-sm btn-success" onclick="respondInvite(${inv.id}, 'accept')">Accept</button>
              <button class="btn btn-sm btn-danger" onclick="respondInvite(${inv.id}, 'decline')">Decline</button>
              <a class="btn btn-sm btn-primary" href="/api/interviews/candidate/${inv.interview?.id}/start/frontend/">Start</a>
            </div>
          </div>
        </div>
      </div>
  ...
  async function loadInvites() {
    try {
      const res = await fetch(invitesUrl, {
      ...
      container.innerHTML = `<div class="text-danger">Error loading invites</div>`;
      console.error(err);
    }
  }

  // respond (accept/decline)
  window.respondInvite = async function(inviteId, action) {
    const url = `${apiBase}/candidate/invites/${inviteId}/respond/`;
    try {
      const res = await fetch(url, {
        ...
        headers: {
          ...
          // if you are using session auth and CSRF is required add X-CSRFToken header
  ...
  // optional: helper to get CSRF cookie (if using session auth)
