...
function attachJobCardEvents() {
  ...
  qsa('.job-card').forEach(card => {
    ...
    // --- ensure interview generate buttons wired ---
  try {
    // attach interview-level & job-level generate buttons (defensive)
    if (typeof attachGenerateInterviewButtons === 'function') attachGenerateInterviewButtons();

    // also wire the job-detail header button (#generateQuestionsBtn)
    ...
    if (genHeaderBtn && !genHeaderBtn._boundGenHeader) {
      ...
      genHeaderBtn.addEventListener('click', async (ev) => {
        ...
        // ask confirm then create interview and generate questions (interviews app)
        if (!confirm('Create interview for this job (if none) and generate questions?')) return;
        // default 5 questions — adjust as needed
        if (typeof generateQuestionsForJobCreateInterviewThenGenerate === 'function') {
          await generateQuestionsForJobCreateInterviewThenGenerate(jobId, 5);
        } else {
          showToast('Generate helper not loaded', 'error');
        }
      });
    }
  } catch (e) { console.warn('attach job generate buttons failed', e); }
...
function injectInviteButtonsIntoAppRows(selector = '.application-row') {
  ...
  rows.forEach(row => {
    ...
    // try to resolve proper candidate id from row attributes
    const candidateIdResolved = resolveCandidateIdFromElement(row) || resolveCandidateIdFromElement(row.querySelector('.invite-btn') || row);
    ...
    // attach candidate id only if resolved and numeric
    ...
    btn.addEventListener('click', (e) => {
      ...
      showInviteModal({ jobId: jd || null, interviewId: e.currentTarget.dataset.interviewId || '', candidateId: e.currentTarget.dataset.candidateId || '', candidateName: e.currentTarget.dataset.candidateName || '' });
    });
...
/* ---------------- Interviews: generate questions (Recruiter) ----------------
   Uses backend:
     POST /api/interviews/recruiter/<interview_pk>/generate_questions/
   If you want to generate for a Job (no interview yet), this will:
     1) POST /api/interviews/recruiter/job/<job_pk>/create/  (create interview)
     2) POST generate_questions for the created interview
*/
...
async function createInterviewForJob(jobPk, payload = {}) {
  // calls POST /api/interviews/recruiter/job/<job_pk>/create/
  ...
  const body = Object.assign({
    title: `Auto interview for job ${jobPk}`,
    description: 'Auto-created by recruiter dashboard',
    duration_minutes: 45,
    is_active: true
  }, payload);
  try {
    ...
    try { data = txt ? JSON.parse(txt) : null; } catch(e){ data = txt; }
    if (!r.ok) {
      console.warn('createInterviewForJob failed', r.status, data);
      return null;
    }
    return data; // should contain created interview (id)
  } catch (e) { console.error('createInterviewForJob err', e); return null; }
}

async function generateInterviewQuestions(interviewPk, nQuestions = 25, autoPublish = false) {
  if (!interviewPk) { showToast('Missing interview id', 'error'); return null; }
  if (!confirm('Generate questions for this interview now?')) return null;
  ...
  try {
    ...
    const payload = { n_questions: Number(nQuestions) || 25, auto_publish: !!autoPublish, params: {} };
    ...
    if (resp.status === 202) {
      showToast('Generation started in background (task queued).', 'info', 4000);
      console.log('generateInterviewQuestions enqueued', data);
      return { ok: true, enqueued: true, data };
    }
    if (resp.ok) {
      showToast('Questions generated successfully.', 'success', 3000);
      console.log('generateInterviewQuestions result', data);
      // refresh UI: reload interview questions / job detail if functions exist
      try { if (typeof loadInterviewDetail === 'function') await loadInterviewDetail(interviewPk); } catch(e){}
      try { if (typeof loadJobs === 'function') await loadJobs(); } catch(e){}
      return { ok: true, enqueued: false, data };
    }

    const errMsg = data && (data.detail || data.error) ? (data.detail || data.error) : ('Status ' + resp.status);
    showToast('Generate failed: ' + errMsg, 'error', 6000);
    console.warn('generateInterviewQuestions failed', resp.status, data);
    return { ok: false, status: resp.status, data };
  } catch (e) {
    console.error('generateInterviewQuestions exception', e);
    showToast('Network error generating questions', 'error');
    return { ok: false, exception: String(e) };
  }
}

/* ---------- Helper: generate for a JOB (create interview then generate) ---------- */
async function generateQuestionsForJobCreateInterviewThenGenerate(jobPk, questionsCount = 25) {
  if (!jobPk) { showToast('No job id', 'error'); return null; }
  // create interview first
  showToast('Creating interview for job...', 'info', 2500);
  const created = await createInterviewForJob(jobPk, { title: `Interview for job ${jobPk}` });
  if (!created || !created.id && !created.pk) {
    showToast('Failed to create interview for job', 'error');
    return null;
  }
  const interviewId = created.id || created.pk || created.pk_id || created.pk;
  return await generateInterviewQuestions(interviewId, questionsCount, true);
}

/* ---------------- Attach buttons (defensive) ----------------
   - Buttons for interviews: class .generate-questions-btn, data-interview-id
   - Buttons on job-cards to create+generate: class .generate-quiz-btn, data-job-id
*/
function attachGenerateInterviewButtons() {
  ...
  document.querySelectorAll('.generate-questions-btn').forEach(btn => {
    ...
    btn.addEventListener('click', async (ev) => {
      ...
      let n = parseInt(btn.dataset.nQuestions || btn.getAttribute('data-n-questions') || 25, 10);
      if (!n || isNaN(n)) n = 25;
      await generateInterviewQuestions(pk, n, (btn.dataset.autoPublish === 'true'));
    });
  });

  // job-level quick generate (create interview then generate)
  document.querySelectorAll('.generate-quiz-btn').forEach(btn => {
    ...
    btn.addEventListener('click', async (ev) => {
      ...
      if (!jobId) return showToast('No job id', 'error');
      if (!confirm('No interview exists — create interview for this job and generate questions?')) return;
      await generateQuestionsForJobCreateInterviewThenGenerate(jobId, parseInt(btn.dataset.questionsCount || 5, 10) || 5);
    });
  });
}

// call this after your job/interview list rendering
// e.g. inside attachJobCardEvents() after buttons created
// attachGenerateInterviewButtons();
...
// after rendering job cards
attachGenerateInterviewButtons && typeof attachGenerateInterviewButtons === 'function' && attachGenerateInterviewButtons();
