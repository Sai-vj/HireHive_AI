...
function openEditJobPrompt(jobId) { 
  ...
  if (newTitle === null) return; updateJobTitle(jobId, newTitle);
   ...
   }); if (!res.ok) { showToast('Update failed', 'error'); return; } showToast('Job updated', 'success'); // update DOM quickly const card = document.querySelector(.job-card[data-job-id="${jobId}"]); if (card) card.querySelector('h6') && (card.querySelector('h6').textContent = title); }
