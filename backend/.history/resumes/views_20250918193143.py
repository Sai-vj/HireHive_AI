...
# resumes/views.py (example)
from django.shortcuts import render
from django.db.models import Prefetch
from interviews.models import Interview
from .models import Job  # or wherever Job lives

def recruiter_dashboard(request):
    # get jobs for recruiter
    jobs = Job.objects.filter(created_by=request.user).order_by('-created_at')

    # find latest interview per job (or whichever logic you want)
    # create dict mapping job.id -> latest_interview.id
    job_default_map = {}
    interviews = Interview.objects.filter(job__in=jobs).order_by('job_id', '-created_at')
    # This loop sets first seen (latest because of order_by) per job
    for iv in interviews:
        if iv.job_id not in job_default_map:
            job_default_map[iv.job_id] = iv.id

    # build applications context as you already do (just an example)
    # keep your existing context; add job_default_map (or job_default_interview_id in template)
    context = {
        'jobs': jobs,
        'job_default_map': job_default_map,  # optional - maybe not used directly by template
        # if you prefer per-job default variable, compute job_default_interview_id in each job loop
    }

    # If your template expects job_default_interview_id in context for each job,
    # you can attach it to each job (non-persistent; just for rendering):
    for job in jobs:
        setattr(job, 'default_interview_id', job_default_map.get(job.id))

    return render(request, 'resumes/recruiter_dashboard.html', context)
