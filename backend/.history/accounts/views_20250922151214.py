...
def register_view(request):
    if request.method == 'POST':
        ...
        # create profile if you have one, e.g. Profile model with OneToOneField to user
        if hasattr(user, 'profile'):
            user.profile.role = role
            user.profile.save()
...
def dashboard_api(request):
    user = request.user
    # safe check if profile exists (depends on your signals)
    role = getattr(getattr(user, 'profile', None), 'role', None)
    # small payload: change/add fields you need
    payload = {
        "username": user.username,
        "email": user.email,
...
def role_redirect(request):
    # assume you have profile.role field with 'candidate' or 'recruiter'
    role = getattr(request.user, 'profile', None)
    if role:
        role_name = getattr(request.user.profile, 'role', '')
    else:
        role_name = ''
